...
func (d *DuOSdb) DbReadAllTypes() {
	items := make(map[string]mod2.DuOSitems)
...
func (d *DuOSdb) DbReadAll(folder string) mod2.DuOSitems {
	itemsRaw, err := d.DB.ReadAll(folder)
	if err != nil {
		fmt.Println("Error", err)
	}
	items := make(map[string]mod2.DuOSitem)
	...
	return mod2.DuOSitems{
		Slug:  folder,
		Items: items,
	}
}

func (d *DuOSdb) DbReadAllComponents() map[string]mod2.DuOScomp {
	componentsRaw, err := d.DB.ReadAll("components")
	if err != nil {
		fmt.Println("Error", err)
	}
	components := make(map[string]mod2.DuOScomp)
	for _, componentRaw := range componentsRaw {
		component := mod2.DuOScomp{}
