...
func Main(cx *conte.Xt, wg *sync.WaitGroup) {
	...
	l.Printf("glfw: created window %dx%d", width, height)
	...
	bariolBold, err := ioutil.ReadFile(cx.DataDir + "/gui/assets/fonts/bariolregular.ttf")
	//bariolBold, err := ioutil.ReadFile("/home/marcetin/go/src/github.com/p9c/pod/test/gui/assets/fonts/bariolregular.ttf")
	if err != nil {
		l.Fatal(err)
	}

	sansFont := nk.NkFontAtlasAddFromBytes(atlas, bariolBold, 16, nil)
