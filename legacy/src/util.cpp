...
#else
    const char* pszModule = "parallelcoin";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "parallelcoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No parallelcoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override parallelcoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "parallelcoind.pid"));
