...
// Copyright (c) 2014 The Parallelcoin developers
...
const string strMessageMagic = "Parallelcoin Signed Message:\n";
...
bool FindBlockPos(CValidationState &state, CDiskBlockPos &pos, unsigned int nAddSize, unsigned int nHeight, uint64 nTime, bool fKnown = false)
{
    ...
    if (fKnown) {
        if (nLastBlockFile != pos.nFile) {
            ...
            pblocktree->ReadBlockFileinf.Ln(nLastBlockFile, infoLastBlockFile);
            fUpdatedLast = true;
        }
    } else {
        while (infoLastBlockFile.nSize + nAddSize >= MAX_BLOCKFILE_SIZE) {
            ...
            nLastBlockFile++;
            infoLastBlockFile.SetNull();
            pblocktree->ReadBlockFileinf.Ln(nLastBlockFile, infoLastBlockFile); // check whether data for the new file somehow already exist; can fail just fine
            fUpdatedLast = true;
        }
    ...
    if (!pblocktree->WriteBlockFileinf.Ln(nLastBlockFile, infoLastBlockFile))
...
bool FindUndoPos(CValidationState &state, int nFile, CDiskBlockPos &pos, unsigned int nAddSize)
{
    ...
    if (nFile == nLastBlockFile) {
        ...
        CBlockFileInfo info;
        if (!pblocktree->ReadBlockFileinf.Ln(nFile, info))
            return state.Abort(_("Failed to read block info"));
        pos.nPos = info.nUndoSize;
        nNewSize = (info.nUndoSize += nAddSize);
        if (!pblocktree->WriteBlockFileinf.Ln(nFile, info))
            return state.Abort(_("Failed to write block info"));
    }
...
bool static LoadBlockIndexDB()
{
    ...
    if (pblocktree->ReadBlockFileinf.Ln(nLastBlockFile, infoLastBlockFile))
...
bool LoadExternalBlockFile(FILE* fileIn, CDiskBlockPos *dbp)
{
    ...
    try {
        ...
        if (dbp) {
            ...
            CBlockFileInfo info;
            if (pblocktree->ReadBlockFileinf.Ln(dbp->nFile, info)) {
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ParallelcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ParallelcoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    loop 
    {
        ...
        printf("Running sha265d miner with %" PRIszu " transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(CWallet *pwallet)
{
    ...
    {
        printf("Parallelcoin miner terminated\n");
        throw;
    }
}
