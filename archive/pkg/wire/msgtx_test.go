...
func TestWTxSha(t *testing.T) {
	...
	wantHashTxid, e := chainhash.NewHashFromStr(hashStrTxid)
	if e != nil {
		t.Errorf("NewShaHashFromStr: %v", e)
		return
	}
	// hashStrWTxid := "0858eab78e77b6b033da30f46699996396cf48fcf625a783c85a51403e175e74"
	// wantHashWTxid, e = chainhash.NewHashFromStr(hashStrWTxid)
	// if e != nil  {
	// 	t.Errorf("NewShaHashFromStr: %v", e)
	// 	return
	// }
...
func TestTxSerialize(t *testing.T) {
	...
	for i, test := range tests {
		...
		rbuf := bytes.NewReader(test.buf)
		if test.witness {
			e = tx.Deserialize(rbuf)
		} else {
			e = tx.DeserializeNoWitness(rbuf)
		}
		if e != nil {
...
func TestTxSerializeSizeStripped(t *testing.T) {
	...
	tests := []struct {
		...
		{multiTx, 210},
		// Transaction with an input which includes witness data, and one output. Note
		// that this uses SerializeSizeStripped which excludes the additional bytes due
		// to witness data encoding.
		{multiWitnessTx, 82},
	}
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
...
func TestTxWitnessSize(t *testing.T) {
	tests := []struct {
		...
		// Transaction with an input which includes witness data, and one output.
		{multiWitnessTx, 190},
	}
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
...
// multiTxEncoded is the wire encoded bytes for multiTx using protocol version 60002 and is used in the various tests.
...
// multiTxPkScriptLocs is the location information for the public key scripts located in multiTx.
var multiTxPkScriptLocs = []int{63, 139}

// multiWitnessTx is a MsgTx with an input with witness data, and an output used in various tests.
...
// multiWitnessTxEncoded is the wire encoded bytes for multiWitnessTx including
// inputs with witness data using protocol version 70012 and is used in the
// various tests.
...
// 	0x0, 0x0, 0x0, 0x0, // Lock time
// }
// // multiTxPkScriptLocs is the location information for the public key scripts located in multiWitnessTx.
