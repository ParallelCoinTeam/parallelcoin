...
// feeIncrement is the minimum transation fee (0.00001 DUO, measured in satoshis) added to transactions requiring a fee.
...
type (
	// OutBailmentID is the unique ID of a user's outbailment, comprising the name of the server the user connected to,
	// and the transaction number, internal to that server.
	OutBailmentID string
	// Ntxid is the normalized ID of a given bitcoin transaction, which is generated by hashing the serialized tx with
	// blank sig scripts on all inputs.
	Ntxid string
	...
	OutputRequest struct {
		...
		Transaction uint32
		// cachedHash is used to cache the hash of the outBailmentID so it only has to be calculated once.
		cachedHash []byte
	}
	...
	WithdrawalOutput struct {
		...
		status  outputStatus
		// The outpoints that fulfill the OutputRequest. There will be more than one in case we need to split the
		// request across multiple transactions.
	...
	// WithdrawalStatus contains the details of a processed withdrawal, including the status of each requested output,
	// the total amount of network fees and the next input and change addresses to use in a subsequent withdrawal
	// request.
	...
	// withdrawalInfo contains all the details of an existing withdrawal, including the original request parameters and
	// the WithdrawalStatus returned by StartWithdrawal.
	...
	// They should match the order of pubkeys in the script and an empty RawSig should be used when the private key for
...
func (tx *changeAwareTx) addSelfToStore(store *wtxmgr.Store, txmgrNs walletdb.ReadWriteBucket) (e error) {
	...
	if tx.changeIdx != -1 {
		if e = store.AddCredit(txmgrNs, rec, nil, uint32(tx.changeIdx), true); E.Chk(e) {
			return newError(ErrWithdrawalTxStorage, "error adding tx credits to store", e)
		}
	}
	return nil
}
...
// outBailmentIDHash returns a byte slice which is used when sorting OutputRequests.
...
type withdrawal struct {
	...
	// txOptions is a function called for every new withdrawalTx created as part of this withdrawal. It is defined as a
	// function field because it exists mainly so that tests can mock withdrawalTx fields.
	txOptions func(tx *withdrawalTx)
}

// withdrawalTxOut wraps an OutputRequest and provides a separate amount field. It is necessary because some requests
// may be partially fulfilled or split across transactions.
type withdrawalTxOut struct {
	// Notice that in the case of a split output, the OutputRequest here will be a copy of the original one with the
...
type withdrawalTx struct {
	...
	// how that's done. We use a struct field instead of a method so that it can be replaced in tests.
...
// isTooBig returns true if the size (in bytes) of the given tx is greater than or equal to txMaxSize.
func (tx *withdrawalTx) isTooBig() bool {
	// In bitcoind a tx is considered standard only if smaller than MAX_STANDARD_TX_SIZE; that's why we consider
	// anything >= txMaxSize to be too big.
...
func (tx *withdrawalTx) addOutput(request OutputRequest) {
	D.F("added tx output sending %s to %s %s",
...
func (tx *withdrawalTx) removeOutput() *withdrawalTxOut {
	...
	D.F("removed tx output sending %s to %s %s",
...
func (tx *withdrawalTx) addInput(input Credit) {
	D.Ln("added tx input with amount", input.Amount)
...
func (tx *withdrawalTx) removeInput() Credit {
	...
	D.Ln("removed tx input with amount", removed.Amount)
	return removed
}

// addChange adds a change output if there are any satoshis left after paying all the outputs and network fees. It
// returns true if a change output was added.
//
// This method must be called only once, and no extra inputs/outputs should be added after it's called. Also, callsites
// must make sure adding a change output won't cause the tx to exceed the size limit.
func (tx *withdrawalTx) addChange(pkScript []byte) bool {
	tx.fee = tx.calculateFee()
	change := tx.inputTotal() - tx.outputTotal() - tx.fee
	D.F("addChange: input total %v, output total %v, fee %v %s",
		tx.inputTotal(), tx.outputTotal(), tx.fee)
	if change > 0 {
		tx.changeOutput = wire.NewTxOut(int64(change), pkScript)
		D.Ln("added change output with amount", change)
	}
	return tx.hasChange()
}

// rollBackLastOutput will roll back the last added output and possibly remove inputs that are no longer needed to cover
// the remaining outputs. The method returns the removed output and the removed inputs, in the reverse order they were
// added, if any.
//
// The tx needs to have two or more outputs. The case with only one output must be handled separately (by the split
// output procedure).
func (tx *withdrawalTx) rollBackLastOutput() ([]Credit, *withdrawalTxOut, error) {
	// Chk precondition: At least two outputs are required in the transaction.
	if len(tx.outputs) < 2 {
...
// StartWithdrawal uses a fully deterministic algorithm to construct transactions fulfilling as many of the given output
// requests as possible. It returns a WithdrawalStatus containing the outpoints fulfilling the requested outputs and a
// map of normalized transaction IDs (ntxid) to signature lists (one for every private key available to this wallet) for
// each of those transaction's inputs. More details about the actual algorithm can be found at
// http://opentransactions.org/wiki/index.php/Startwithdrawal This method must be called with the address manager
// unlocked.
func (p *Pool) StartWithdrawal(ns walletdb.ReadWriteBucket,
	...
	*WithdrawalStatus, error) {
	status, e := getWithdrawalStatus(p, ns, addrmgrNs, roundID, requests, startAddress, lastSeriesID,
	...
	eligible, e := p.getEligibleInputs(ns, addrmgrNs, txStore, txmgrNs, startAddress, lastSeriesID, dustThreshold,
	...
	serialized, e := serializeWithdrawal(requests, startAddress, lastSeriesID, changeStart,
		dustThreshold, *w.status)
	if e != nil  {
				return nil, e
	}
	e = putWithdrawal(ns, p.ID, roundID, serialized)
	if e != nil  {
				return nil, e
	}
	return w.status, nil
...
// popRequest removes and returns the first request from the stack of pending requests.
...
// popInput removes and returns the first input from the stack of eligible inputs.
...
// If this returns it means we have added an output and the necessary inputs to fulfil that output plus the required
// fees. It also means the tx won't reach the size limit even after we add a change output and sign all inputs.
func (w *withdrawal) fulfillNextRequest() (e error) {
	request := w.popRequest()
	output := w.status.outputs[request.outBailmentID()]
	// We start with an output status of success and let the methods that deal with special cases change it when
	// appropriate.
	...
	for w.current.inputTotal() < w.current.outputTotal()+fee {
		if len(w.eligibleInputs) == 0 {
			D.Ln("splitting last output because we don't have enough" +
				" inputs")
			if e := w.splitLastOutput(); E.Chk(e) {
				return e
			}
			break
		}
...
// handleOversizeTx handles the case when a transaction has become too big by either rolling back an output or splitting
// it.
func (w *withdrawal) handleOversizeTx() (e error) {
	if len(w.current.outputs) > 1 {
		D.Ln("rolling back last output because tx got too big")
		inputs, output, e := w.current.rollBackLastOutput()
		if e != nil  {
						return newError(ErrWithdrawalProcessing, "failed to rollback last output", e)
		}
		...
		D.Ln("splitting last output because tx got too big...")
...
// finalizeCurrentTx finalizes the transaction in w.current, moves it to the list of finalized transactions and replaces
// w.current with a new empty transaction.
func (w *withdrawal) finalizeCurrentTx() (e error) {
	D.Ln("finalizing current transaction")
	tx := w.current
	if len(tx.outputs) == 0 {
		D.Ln("current transaction has no outputs, doing nothing")
		return nil
	}
	pkScript, e := txscript.PayToAddrScript(w.status.nextChangeAddr.addr)
	if e != nil  {
				return newError(ErrWithdrawalProcessing, "failed to generate pkScript for change address", e)
	}
	if tx.addChange(pkScript) {
		var e error
		w.status.nextChangeAddr, e = nextChangeAddress(w.status.nextChangeAddr)
		if e != nil  {
						return newError(ErrWithdrawalProcessing, "failed to get next change address", e)
		}
	}
	ntxid := tx.ntxid()
	...
	// Chk that WithdrawalOutput entries with status==success have the sum of their outpoint amounts matching the
	// requested amount.
	for _, txOut := range tx.outputs {
		// Look up the original request we received because txOut.request may represent a split request and thus have a
...
// maybeDropRequests will check the total amount we have in eligible inputs and drop requested outputs (in descending
...
func (w *withdrawal) maybeDropRequests() {
	...
	for inputAmount < outputAmount {
		request := w.popRequest()
		I.F("not fulfilling request to send %v to %v; not enough credits.",
			request.Amount, request.Address)
		outputAmount -= request.Amount
		w.status.outputs[request.outBailmentID()].status = statusPartial
	}
}
func (w *withdrawal) fulfillRequests() (e error) {
	w.maybeDropRequests()
	...
	for len(w.pendingRequests) > 0 {
		if e := w.fulfillNextRequest(); E.Chk(e) {
			return e
		}
		tx := w.current
		if len(w.eligibleInputs) == 0 && tx.inputTotal() <= tx.outputTotal()+tx.calculateFee() {
			// We don't have more eligible inputs and all the inputs in the current tx have been spent.
			break
		}
	}
	if e := w.finalizeCurrentTx(); E.Chk(e) {
		return e
	}
	...
	for _, tx := range w.transactions {
		...
		changeIdx := -1
		if tx.hasChange() {
			// When withdrawalTx has a change, we know it will be the last entry in the generated MsgTx.
...
func (w *withdrawal) splitLastOutput() (e error) {
	...
	D.Ln("splitting tx output for", output.request)
	origAmount := output.amount
	spentAmount := tx.outputTotal() + tx.calculateFee() - output.amount
	// This is how much we have left after satisfying all outputs except the last one. IOW, all we have left for the
	// last output, so we set that as the amount of the tx's last output.
	unspentAmount := tx.inputTotal() - spentAmount
	output.amount = unspentAmount
	D.Ln("updated output amount to", output.amount)
	...
	D.Ln("created a new pending output request with amount",
		newRequest.Amount)
	w.status.outputs[request.outBailmentID()].status = statusPartial
	return nil
}
func (s *WithdrawalStatus) updateStatusFor(tx *withdrawalTx) {
	for _, output := range s.outputs {
		...
		// TODO: Update outputs with status=='partial-'. For this we need an API that gives us the amount of credits in
		//  a given series. http://opentransactions.org/wiki/index.php/Update_Status
	}
}

// match returns true if the given arguments match the fields in this withdrawalInfo. For the requests slice, the order
...
func (wi *withdrawalInfo) match(requests []OutputRequest, startAddress WithdrawalAddress,
	...
	if !reflect.DeepEqual(r1, r2) {
		D.F("withdrawal requests does not match: %v != %v %s", requests,
			wi.requests)
		return false
	}
	return true
...
// getWithdrawalStatus returns the existing WithdrawalStatus for the given withdrawal parameters, if one exists. This
// function must be called with the address manager unlocked.
func getWithdrawalStatus(p *Pool, ns, addrmgrNs walletdb.ReadBucket, roundID uint32, requests []OutputRequest,
	...
	wInfo, e := deserializeWithdrawal(p, ns, addrmgrNs, serialized)
	if e != nil  {
				return nil, e
	}
		...
		for inputIdx, input := range tx.inputs {
			...
			for i, pubKey := range pubKeys {
				...
				if privKey != nil {
					...
					D.F("generating raw sig for input %d of tx %s with privkey of %s",
						inputIdx, ntxid,
						pubKey.String())
					sig, e = txscript.RawTxInSignature(
						msgtx, inputIdx, redeemScript, txscript.SigHashAll, ecPrivKey)
					if e != nil  {
												return nil, newError(ErrRawSigning, "failed to generate raw signature", e)
					}
				} else {
					D.F("not generating raw sig for input %d of %s because private"+
...
// SignTx signs every input of the given MsgTx by looking up (on the addr manager) the redeem script for each of them
// and constructing the signature script using that and the given raw signatures. This function must be called with the
// manager unlocked.
func SignTx(msgtx *wire.MsgTx, sigs TxSigs, mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, store *wtxmgr.Store, txmgrNs walletdb.ReadBucket) (e error) {
	// We use time.Now() here as we're not going to store the new TxRecord anywhere -- we just need it to pass to
	// store.PreviousPkScripts().
	rec, e := wtxmgr.NewTxRecordFromMsgTx(msgtx, time.Now())
	if e != nil  {
				return newError(ErrTxSigning, "failed to construct TxRecord for signing", e)
	}
	pkScripts, e := store.PreviousPkScripts(txmgrNs, rec, nil)
	if e != nil  {
				return newError(ErrTxSigning, "failed to obtain pkScripts for signing", e)
	}
	for i, pkScript := range pkScripts {
		if e = signMultiSigUTXO(mgr, addrmgrNs, msgtx, i, pkScript, sigs[i]); E.Chk(e) {
			return e
		}
	}
	return nil
...
func getRedeemScript(mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, addr *util.AddressScriptHash) ([]byte, error) {
	address, e := mgr.Address(addrmgrNs, addr)
	if e != nil  {
				return nil, e
	}
...
// signMultiSigUTXO signs the P2SH UTXO with the given index by constructing a script containing all given signatures
// plus the redeem (multi-sig) script. The redeem script is obtained by looking up the address of the given P2SH
// pkScript on the address manager.
//
// The order of the signatures must match that of the public keys in the multi-sig script as OP_CHECKMULTISIG expects
// that. This function must be called with the manager unlocked.
func signMultiSigUTXO(mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, tx *wire.MsgTx, idx int, pkScript []byte, sigs []RawSig) (e error) {
	class, addresses, _, e = txscript.ExtractPkScriptAddrs(pkScript, mgr.ChainParams())
	if e != nil  {
				return newError(ErrTxSigning, "unparseable pkScript", e)
	}
	...
	class, _, nRequired, e := txscript.ExtractPkScriptAddrs(redeemScript, mgr.ChainParams())
	if e != nil  {
				return newError(ErrTxSigning, "unparseable redeem script", e)
	}
	...
	// Construct the unlocking script. Start with an OP_0 because of the bug in bitcoind, then add nRequired signatures.
	...
	script, e := sigScript.Script()
	if e != nil  {
				return newError(ErrTxSigning, "error building sigscript", e)
	}
	tx.TxIn[idx].SignatureScript = script
	if e := validateSigScript(tx, idx, pkScript); E.Chk(e) {
		return e
	}
	return nil
}

// validateSigScripts executes the signature script of the tx input with the given index, returning an error if it
// fails.
func validateSigScript(msgtx *wire.MsgTx, idx int, pkScript []byte) (e error) {
	vm, e := txscript.NewEngine(pkScript, msgtx, idx,
		txscript.StandardVerifyFlags, nil, nil, 0)
	if e != nil  {
				return newError(ErrTxSigning, "cannot create script engine", e)
	}
	if e = vm.Execute(); E.Chk(e) {
		return newError(ErrTxSigning, "cannot validate tx signature", e)
	}
	return nil
}

// calculateTxSize returns an estimate of the serialized size (in bytes) of the given transaction. It assumes all tx
// inputs are P2SH multi-sig.
func calculateTxSize(tx *withdrawalTx) int {
	msgtx := tx.toMsgTx()
	// Assume that there will always be a change output, for simplicity. We simulate that by simply copying the first
	// output as all we care about is the size of its serialized form, which should be the same for all of them as
	// they're either P2PKH or P2SH..
	if !tx.hasChange() {
		msgtx.AddTxOut(msgtx.TxOut[0])
	}
	// Craft a SignatureScript with dummy signatures for every input in this tx so that we can use msgtx.SerializeSize()
	// to get its size and don't need to rely on estimations.
	for i, txin := range msgtx.TxIn {
		// 1 byte for the OP_FALSE opcode, then 73+1 bytes for each signature with their OP_DATA opcode and finally the
		// redeem script + 1 byte for its OP_PUSHDATA opcode and N bytes for the redeem script's size.
		//
		// Notice that we use 73 as the signature length as that's the maximum length they may have:
...
func nextChangeAddress(a ChangeAddress) (ChangeAddress, error) {
	...
	var e error
	if addr, e = a.pool.ChangeAddress(seriesID, index); E.Chk(e) {
...
func storeTransactions(store *wtxmgr.Store, txmgrNs walletdb.ReadWriteBucket, transactions []*changeAwareTx) (e error) {
	for _, tx := range transactions {
		if e := tx.addSelfToStore(store, txmgrNs); E.Chk(e) {
			return e
		}
	}
	return nil
