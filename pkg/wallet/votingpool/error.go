...
const (
	...
	ErrUnknownPubKey
	// ErrSeriesSerialization indicates that an error occurred while serializing or deserializing one or more series for
	...
	// ErrSeriesAlreadyEmpowered indicates that an already empowered series was used where a not empowered one was
	// expected.
	...
	// lastErr is used for testing, making it possible to iterate over the error codes in order to check that they all
	// have proper translations in errorCodeStrings.
	lastErr
)
...
// VPError satisfies the error interface and prints human-readable errors.
