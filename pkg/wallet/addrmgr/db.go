...
func maybeConvertDbError(er error) (err error) {
	// When the error is already a ManagerError, just return it.
	if _, ok := er.(ManagerError); ok {
		slog.Debug(er)
		return er
	}
	if err = managerError(ErrDatabase, er.Error(), err); slog.Check(err) {
	}
	return
}
...
// these are variables but only because they are not able to be constants
// nolint
var (
...
func fetchScopeAddrSchema(ns walletdb.ReadBucket, scope *KeyScope) (sas *ScopeAddrSchema, err error) {
	schemaBucket := ns.NestedReadBucket(scopeSchemaBucketName)
	if schemaBucket == nil {
		str := fmt.Sprintf("unable to find scope schema bucket")
		err = managerError(ErrScopeNotFound, str, nil)
		slog.Debug(err)
		return
	}
	...
	sas = scopeSchemaFromBytes(schemaBytes)
	return
}
...
// 	schema *ScopeAddrSchema) (err error) {
...
func fetchReadScopeBucket(ns walletdb.ReadBucket, scope *KeyScope) (scopedBucket walletdb.ReadBucket, err error) {
...
func fetchWriteScopeBucket(ns walletdb.ReadWriteBucket, scope *KeyScope) (scopedBucket walletdb.ReadWriteBucket, err error) {
...
func fetchManagerVersion(ns walletdb.ReadBucket) (version uint32, err error) {
...
func putManagerVersion(ns walletdb.ReadWriteBucket, version uint32) (err error) {
	bucket := ns.NestedReadWriteBucket(mainBucketName)
	verBytes := uint32ToBytes(version)
	if err = bucket.Put(mgrVersionName, verBytes); slog.Check(err) {
		str := "failed to store version"
		err = managerError(ErrDatabase, str, err)
		slog.Debug(err)
		return
	}
	return
...
// returned value can be nil, but in practice only the private key netparams will
// be nil for a watching-only database.
func fetchMasterKeyParams(ns walletdb.ReadBucket) (pubParams []byte, privParams []byte, err error) {
...
func putMasterKeyParams(ns walletdb.ReadWriteBucket, pubParams, privParams []byte) (err error) {
	bucket := ns.NestedReadWriteBucket(mainBucketName)
	if privParams != nil {
...
func fetchCoinTypeKeys(ns walletdb.ReadBucket, scope *KeyScope) (coinTypePubKeyEnc, coinTypePrivKeyEnc []byte, err error) {
	var scopedBucket walletdb.ReadBucket
	if scopedBucket, err = fetchReadScopeBucket(ns, scope); slog.Check(err) {
		return
	}
...
func putCoinTypeKeys(ns walletdb.ReadWriteBucket, scope *KeyScope,
	coinTypePubKeyEnc []byte, coinTypePrivKeyEnc []byte) (err error) {
...
func putMasterHDKeys(ns walletdb.ReadWriteBucket, masterHDPrivEnc, masterHDPubEnc []byte) (err error) {
...
func fetchMasterHDKeys(ns walletdb.ReadBucket) (masterHDPrivEnc, masterHDPubEnc []byte, err error) {
	bucket := ns.NestedReadBucket(mainBucketName)
...
func fetchCryptoKeys(ns walletdb.ReadBucket) (pubKey, privKey, scriptKey []byte, err error) {
	bucket := ns.NestedReadBucket(mainBucketName)
...
func putCryptoKeys(ns walletdb.ReadWriteBucket, pubKeyEncrypted, privKeyEncrypted,
	scriptKeyEncrypted []byte) (err error) {
	bucket := ns.NestedReadWriteBucket(mainBucketName)
	if pubKeyEncrypted != nil {
		if err = bucket.Put(cryptoPubKeyName, pubKeyEncrypted); slog.Check(err) {
	...
	if privKeyEncrypted != nil {
		if err = bucket.Put(cryptoPrivKeyName, privKeyEncrypted); slog.Check(err) {
	...
	if scriptKeyEncrypted != nil {
		if err = bucket.Put(cryptoScriptKeyName, scriptKeyEncrypted); slog.Check(err) {
...
func fetchWatchingOnly(ns walletdb.ReadBucket) (b bool, err error) {
	bucket := ns.NestedReadBucket(mainBucketName)
...
func putWatchingOnly(ns walletdb.ReadWriteBucket, watchingOnly bool) (err error) {
	bucket := ns.NestedReadWriteBucket(mainBucketName)
	var encoded byte
	if watchingOnly {
		encoded = 1
...
// the common parts.
func deserializeAccountRow(accountID []byte, serializedAccount []byte) (row *dbAccountRow, err error) {
...
func serializeAccountRow(row *dbAccountRow) []byte {
	...
	binary.LittleEndian.PutUint32(buf[1:5], uint32(l))
	copy(buf[5:5+l], row.rawData)
	return buf
}

// deserializeDefaultAccountRow deserializes the raw data from the passed
// account row as a BIP0044-like account.
func deserializeDefaultAccountRow(accountID []byte, row *dbAccountRow) (retRow *dbDefaultAccountRow, err error) {
...
func forEachKeyScope(ns walletdb.ReadBucket, fn func(KeyScope) error) (err error) {
	bucket := ns.NestedReadBucket(scopeBucketName)
	return bucket.ForEach(func(k, v []byte) (err error) {
		// skip non-bucket
		if len(k) != 8 {
			return nil
		}
...
func forEachAccount(ns walletdb.ReadBucket, scope *KeyScope,
	...
	return acctBucket.ForEach(func(k, v []byte) (err error) {
		// Skip buckets.
		if v == nil {
			return nil
		}
...
func fetchLastAccount(ns walletdb.ReadBucket, scope *KeyScope) (account uint32, err error) {
	var scopedBucket walletdb.ReadBucket
	if scopedBucket, err = fetchReadScopeBucket(ns, scope); slog.Check(err) {
		return
	}
...
// database.
func fetchAccountName(ns walletdb.ReadBucket, scope *KeyScope, account uint32) (acctName string, err error) {
	var scopedBucket walletdb.ReadBucket
	if scopedBucket, err = fetchReadScopeBucket(ns, scope); slog.Check(err) {
		return
	}
	...
	if val == nil {
		...
		err = managerError(ErrAccountNotFound, str, nil)
		slog.Debug(err)
		return
	}
	offset := uint32(0)
...
// the database.
func fetchAccountByName(ns walletdb.ReadBucket, scope *KeyScope, name string) (u uint32, err error) {
	var scopedBucket walletdb.ReadBucket
	scopedBucket, err = fetchReadScopeBucket(ns, scope)
	if err != nil {
		slog.Error(err)
...
// database.
func fetchAccountInfo(ns walletdb.ReadBucket, scope *KeyScope, account uint32) (i interface{}, err error) {
	var scopedBucket walletdb.ReadBucket
	scopedBucket, err = fetchReadScopeBucket(ns, scope)
	if slog.Check(err) {
		return
	...
	if row, err = deserializeAccountRow(accountID, serializedRow); slog.Check(err) {
		return
	}
...
func deleteAccountNameIndex(ns walletdb.ReadWriteBucket, scope *KeyScope, name string) (err error) {
	var scopedBucket walletdb.ReadWriteBucket
	if scopedBucket, err = fetchWriteScopeBucket(ns, scope); slog.Check(err) {
		return
	}
	bucket := scopedBucket.NestedReadWriteBucket(acctNameIdxBucketName)
	// Delete the account name key
	if err = bucket.Delete(stringToBytes(name)); slog.Check(err) {
...
func deleteAccountIDIndex(ns walletdb.ReadWriteBucket, scope *KeyScope, account uint32) (err error) {
	var scopedBucket walletdb.ReadWriteBucket
	if scopedBucket, err = fetchWriteScopeBucket(ns, scope); slog.Check(err) {
		return
	}
	...
	if err = bucket.Delete(uint32ToBytes(account)); slog.Check(err) {
...
func putAddrAccountIndex(ns walletdb.ReadWriteBucket, scope *KeyScope,
	account uint32, addrHash []byte) (err error) {
	scopedBucket, err := fetchWriteScopeBucket(ns, scope)
	if err != nil {
		slog.Error(err)
		return err
...
func putAccountRow(ns walletdb.ReadWriteBucket, scope *KeyScope,
	account uint32, row *dbAccountRow) (err error) {
	scopedBucket, err := fetchWriteScopeBucket(ns, scope)
	if err != nil {
		slog.Error(err)
		return err
...
func deserializeAddressRow(serializedAddress []byte) (row *dbAddressRow, err error) {
	...
	rowDataLen := binary.LittleEndian.Uint32(serializedAddress[14:18])
	row.rawData = make([]byte, rowDataLen)
	copy(row.rawData, serializedAddress[18:18+rowDataLen])
	return
}
...
func deserializeChainedAddress(row *dbAddressRow) (retRow *dbChainAddressRow, err error) {
...
func deserializeImportedAddress(row *dbAddressRow) (retRow *dbImportedAddressRow, err error) {
...
func deserializeScriptAddress(row *dbAddressRow) (retRow *dbScriptAddressRow, err error) {
...
func fetchAddressByHash(ns walletdb.ReadBucket, scope *KeyScope, addrHash []byte) (res interface{}, err error) {
	var scopedBucket walletdb.ReadBucket
	if scopedBucket, err = fetchReadScopeBucket(ns, scope); slog.Check(err) {
		return
	}
	...
	if row, err = deserializeAddressRow(serializedRow); slog.Check(err) {
		return
	}
	switch row.addrType {
	case adtChain:
...
// failure.
func fetchAddress(ns walletdb.ReadBucket, scope *KeyScope, addressID []byte) (res interface{}, err error) {
	addrHash := sha256.Sum256(addressID)
...
func putAddress(ns walletdb.ReadWriteBucket, scope *KeyScope, addressID []byte, row *dbAddressRow) (err error) {
	var scopedBucket walletdb.ReadWriteBucket
	if scopedBucket, err = fetchWriteScopeBucket(ns, scope); slog.Check(err) {
		return
	}
	...
	addrHash := sha256.Sum256(addressID)
	if err = bucket.Put(addrHash[:], serializeAddressRow(row)); slog.Check(err) {
...
func putChainedAddress(
	...
	if err = putAddress(ns, scope, addressID, &addrRow); slog.Check(err) {
		return err
	}
	...
	var row *dbAccountRow
	if row, err = deserializeAccountRow(accountID, serializedAccount); slog.Check(err) {
		return
	}
	var defaultRow *dbDefaultAccountRow
	if defaultRow, err = deserializeDefaultAccountRow(accountID, row); slog.Check(err) {
		return
	}
	// Increment the appropriate next index depending on whether the branch
	// is internal or external.
	nextExternalIndex := defaultRow.nextExternalIndex
	nextInternalIndex := defaultRow.nextInternalIndex
	...
	row.rawData = serializeDefaultAccountRow(
		defaultRow.pubKeyEncrypted, defaultRow.privKeyEncrypted, nextExternalIndex,
		nextInternalIndex, defaultRow.name,
	)
	if err = bucket.Put(accountID, serializeAccountRow(row)); slog.Check(err) {
...
func existsAddress(ns walletdb.ReadBucket, scope *KeyScope, addressID []byte) (b bool) {
	var scopedBucket walletdb.ReadBucket
	var err error
	if scopedBucket, err = fetchReadScopeBucket(ns, scope); slog.Check(err) {
		return
...
func fetchAddrAccount(ns walletdb.ReadBucket, scope *KeyScope, addressID []byte) (u uint32, err error) {
	var scopedBucket walletdb.ReadBucket
	if scopedBucket, err = fetchReadScopeBucket(ns, scope); slog.Check(err) {
		return
	}
	...
	if val == nil {
		str := "address not found"
		err = managerError(ErrAddressNotFound, str, nil)
		slog.Debug(err)
		return
	}
...
func deletePrivateKeys(ns walletdb.ReadWriteBucket) (err error) {
	bucket := ns.NestedReadWriteBucket(mainBucketName)
	// Delete the master private key netparams and the crypto private and
	// script keys.
	...
	if err = scopeBucket.ForEach(func(scopeKey, _ []byte) (err error) {
		if len(scopeKey) != 8 {
			return
		}
		...
		if err = managerScopeBucket.Delete(coinTypePrivKeyName); err != nil {
			str := "failed to delete coin type private key"
			err = managerError(ErrDatabase, str, err)
			slog.Debug(err)
			return
		}
		...
		if err = bucket.ForEach(func(k, v []byte) (err error) {
			// Skip buckets.
			if v == nil {
				return nil
			}
			...
			var row *dbAccountRow
			if row, err = deserializeAccountRow(k, v); slog.Check(err) {
				return
			}
			if row.acctType == accountDefault {
				var defaultAccountRow *dbDefaultAccountRow
				if defaultAccountRow, err = deserializeDefaultAccountRow(k, row); slog.Check(err) {
					return
				}
				// Reserialize the account without the private key and
				// store it.
				row.rawData = serializeDefaultAccountRow(
					defaultAccountRow.pubKeyEncrypted, nil,
					defaultAccountRow.nextExternalIndex, defaultAccountRow.nextInternalIndex,
					defaultAccountRow.name,
				)
				if err = bucket.Put(k, serializeAccountRow(row)); slog.Check(err) {
		...
		if err = bucket.ForEach(func(k, v []byte) (err error) {
			...
			var row *dbAddressRow
			if row, err = deserializeAddressRow(v); slog.Check(err) {
				return
			}
			switch row.addrType {
			case adtImport:
				var importedAddressRow *dbImportedAddressRow
				if importedAddressRow, err = deserializeImportedAddress(row); slog.Check(err) {
					return
				}
				// Reserialize the imported address without the private
				// key and store it.
				row.rawData = serializeImportedAddress(
					importedAddressRow.encryptedPubKey, nil)
				if err = bucket.Put(k, serializeAddressRow(row)); slog.Check(err) {
			...
			case adtScript:
				var scriptAddressRow *dbScriptAddressRow
				if scriptAddressRow, err = deserializeScriptAddress(row); slog.Check(err) {
					return
				}
				// Reserialize the script address without the script
				// and store it.
				row.rawData = serializeScriptAddress(scriptAddressRow.encryptedHash,
					nil)
				if err = bucket.Put(k, serializeAddressRow(row)); slog.Check(err) {
			...
			if err = maybeConvertDbError(err); slog.Check(err) {
			}
			return
		}
		return
...
// database.
func fetchSyncedTo(ns walletdb.ReadBucket) (bs *BlockStamp, err error) {
	...
	if len(buf) == 40 {
		bs.Timestamp = time.Unix(int64(binary.LittleEndian.Uint32(buf[36:])), 0)
	}
	return
}

// putSyncedTo stores the provided synced to blockstamp to the database.
func putSyncedTo(ns walletdb.ReadWriteBucket, bs *BlockStamp) (err error) {
	...
	if bs.Height > 0 {
		if _, err = fetchBlockHash(ns, bs.Height-1); err != nil {
			err = managerError(ErrDatabase, errStr, err)
			slog.Debug(err)
			return
		}
	}
	...
	if err = bucket.Put(height, bs.Hash[0:32]); slog.Check(err) {
		err = managerError(ErrDatabase, errStr, err)
		slog.Debug(err)
		return
	}
	...
	if err = bucket.Put(syncedToName, buf); slog.Check(err) {
		err = managerError(ErrDatabase, errStr, err)
		slog.Debug(err)
		return
	}
...
// database.
func fetchBlockHash(ns walletdb.ReadBucket, height int32) (hash *chainhash.Hash, err error) {
	bucket := ns.NestedReadBucket(syncBucketName)
	...
	if err = hash.SetBytes(hashBytes); slog.Check(err) {
		err = managerError(ErrDatabase, errStr, err)
		slog.Debug(err)
		return
	}
...
// database.
func fetchStartBlock(ns walletdb.ReadBucket) (bs *BlockStamp, err error) {
	bucket := ns.NestedReadBucket(syncBucketName)
...
func putStartBlock(ns walletdb.ReadWriteBucket, bs *BlockStamp) (err error) {
	...
	binary.LittleEndian.PutUint32(buf[0:4], uint32(bs.Height))
	copy(buf[4:36], bs.Hash[0:32])
	if err = bucket.Put(startBlockName, buf); slog.Check(err) {
...
func fetchBirthday(ns walletdb.ReadBucket) (t time.Time, err error) {
	bucket := ns.NestedReadBucket(syncBucketName)
...
func putBirthday(ns walletdb.ReadWriteBucket, t time.Time) (err error) {
	...
	if err = bucket.Put(birthdayName, buf); slog.Check(err) {
...
func createScopedManagerNS(ns walletdb.ReadWriteBucket, scope *KeyScope) (err error) {
	...
	if scopeBucket, err = ns.CreateBucket(scopeKey[:]); slog.Check(err) {
		str := "failed to create sync bucket"
		err = managerError(ErrDatabase, str, err)
		slog.Check(err)
		return
	}
	if _, err = scopeBucket.CreateBucket(acctBucketName); slog.Check(err) {
		str := "failed to create account bucket"
		err = managerError(ErrDatabase, str, err)
		slog.Debug(err)
		return
	}
	if _, err = scopeBucket.CreateBucket(addrBucketName); slog.Check(err) {
	...
	if _, err = scopeBucket.CreateBucket(usedAddrBucketName); slog.Check(err) {
		str := "failed to create used addresses bucket"
		err = managerError(ErrDatabase, str, err)
		slog.Debug(err)
		return
	}
	if _, err = scopeBucket.CreateBucket(addrAcctIdxBucketName); slog.Check(err) {
		str := "failed to create address index bucket"
		err = managerError(ErrDatabase, str, err)
		slog.Debug(err)
		return
	}
	if _, err = scopeBucket.CreateBucket(acctNameIdxBucketName); slog.Check(err) {
		str := "failed to create an account name index bucket"
		err = managerError(ErrDatabase, str, err)
		slog.Debug(err)
		return
	}
	if _, err = scopeBucket.CreateBucket(acctIDIdxBucketName); slog.Check(err) {
		str := "failed to create an account id index bucket"
		err = managerError(ErrDatabase, str, err)
		slog.Debug(err)
		return
	}
	if _, err = scopeBucket.CreateBucket(metaBucketName); slog.Check(err) {
...
func createManagerNS(ns walletdb.ReadWriteBucket, defaultScopes map[KeyScope]ScopeAddrSchema) (err error) {
...
// func upgradeToVersion2(// 	ns walletdb.ReadWriteBucket) (err error) {
...
// versions as neeeded.
func upgradeManager(db walletdb.DB, namespaceKey []byte, pubPassPhrase []byte, chainParams *netparams.Params,
	cbs *OpenCallbacks) (err error) {
	var version uint32
	...
	if version < latestMgrVersion {
		...
		err = managerError(ErrUpgrade, str, nil)
		slog.Debug(err)
		return
	}
	return
...
func upgradeToVersion5(ns walletdb.ReadWriteBucket, pubPassPhrase []byte) (err error) {
	...
	if err = putManagerVersion(ns, 5); slog.Check(err) {
		return
	}
	// First, we'll need to create the new buckets that are used in the new
	// database version.
	var scopeBucket walletdb.ReadWriteBucket
	if scopeBucket, err = ns.CreateBucket(scopeBucketName); slog.Check(err) {
		str := "failed to create scope bucket"
		err = managerError(ErrDatabase, str, err)
		slog.Debug(err)
		return
	...
	if scopeSchemas, err = ns.CreateBucket(scopeSchemaBucketName); slog.Check(err) {
		str := "failed to create scope schema bucket"
		err = managerError(ErrDatabase, str, err)
		slog.Debug(err)
		return
	...
	schemaBytes := scopeSchemaToBytes(&scopeSchema)
	if err = scopeSchemas.Put(scopeKey[:], schemaBytes); slog.Check(err) {
		return
	}
	if err = createScopedManagerNS(scopeBucket, &KeyScopeBIP0044); slog.Check(err) {
		return
	}
	...
	if err = bip44Bucket.Put(coinTypePrivKeyName, encCoinPrivKeys); slog.Check(err) {
		return
	}
	if err = bip44Bucket.Put(coinTypePubKeyName, encCoinPubKeys); slog.Check(err) {
		return
	}
	if err = mainBucket.Delete(coinTypePrivKeyName); slog.Check(err) {
		return
	}
	if err = mainBucket.Delete(coinTypePubKeyName); slog.Check(err) {
		return
	}
	...
	if err = scopedMetaBucket.Put(lastAccountName, lastAccount); slog.Check(err) {
		return
	}
	...
	for _, bucketKey := range keysToMigrate {
		if err = migrateRecursively(ns, bip44Bucket, bucketKey); slog.Check(err) {
			return
		}
	}
	return
...
func migrateRecursively(src, dst walletdb.ReadWriteBucket, bucketKey []byte) (err error) {
	...
	if newBucket, err = dst.CreateBucketIfNotExists(bucketKey); slog.Check(err) {
		return
	}
	if err = bucketToMigrate.ForEach(func(k, v []byte) (err error) {
	...
	if err = src.DeleteNestedBucket(bucketKey); slog.Check(err) {
		return
	}
	return
}
