...
func (s *NeutrinoClient) onFilteredBlockConnected(height int32,
	...
	if err != nil {
		log.L.Error(err)
		log.L.Error("can't get chain service's best block:", err)
		return
	}
...
func (s *NeutrinoClient) notificationHandler() {
	...
	if err != nil {
		log.L.Error(err)
		log.L.Errorf("failed to get best block from chain service:", err)
		s.Stop()
		s.wg.Done()
		return
	}
...
out:
	for {
		...
		case err := <-rescanErr:
			if err != nil {
				log.L.Error(err)
				log.L.Error("neutrino rescan ended with error:", err)
			}
