...
// Interface allows more than one backing blockchain source, such as a pod RPC chain server, or an SPV library, as long
// as we write a driver for it.
...
// Notification types.
//
// These are defined here and processed from from reading a notificationChan to avoid handling these notifications
// directly in rpcclient callbacks, which isn't very Go-like and doesn't allow blocking client calls.
type (
	...
	// FilteredBlockConnected is an alternate notification that contains both block and relevant transaction information
	// in one struct, which allows atomic updates.
	FilteredBlockConnected struct {
		Block       *wtxmgr.BlockMeta
		RelevantTxs []*wtxmgr.TxRecord
	}
	// FilterBlocksRequest specifies a range of blocks and the set of internal and external addresses of interest,
	// indexed by corresponding scoped-index of the child address. A global set of watched outpoints is also included to
	// monitor for spends.
	...
	// that can modify the wallet's balance. The index of the block within the FilterBlocksRequest is returned, such
	// that the caller can reinitiate a request for the subsequent block after updating the addresses of interest.
