...
func (s secretSource) GetKey(addr util.Address) (privKey *ec.PrivateKey, cmpr bool, e error) {
	...
	if privKey, e = mpka.PrivKey(); dbg.Chk(e) {
		return nil, false, e
	}
...
func (w *Wallet) txToOutputs(
	...
	if chainClient, e = w.requireChainClient(); dbg.Chk(e) {
		return nil, e
	}
	e = walletdb.Update(
		w.db, func(dbtx walletdb.ReadWriteTx) (e error) {
			...
			if bs, e = chainClient.BlockStamp(); dbg.Chk(e) {
				return
			}
			var eligible []wtxmgr.Credit
			if eligible, e = w.findEligibleOutputs(dbtx, account, minconf, bs); dbg.Chk(e) {
				return
			}
			...
			if tx, e = txauthor.NewUnsignedTransaction(outputs, feeSatPerKb, inputSource, changeSource); dbg.Chk(e) {
				return
			}
	...
	if e = validateMsgTx(tx.Tx, tx.PrevScripts, tx.PrevInputValues); dbg.Chk(e) {
		return
	}
...
func (w *Wallet) findEligibleOutputs(
	...
	for i := range unspent {
		...
		); dbg.Chk(e) || len(addrs) != 1 {
			continue
		}
		var addrAcct uint32
		if _, addrAcct, e = w.Manager.AddrAccount(addrmgrNs, addrs[0]); dbg.Chk(e) ||
			addrAcct != account {
			continue
		}
