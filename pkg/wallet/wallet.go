...
func (w *Wallet) Start() {
	...
	default:
		if w.started {
			...
			log.L.Info("wallet already started")
			w.quitMu.Unlock()
			return
		}
		w.started = true
...
func (w *Wallet) syncWithChain() error {
	...
	if err != nil {
		log.L.Warn("error starting sync", err)
		return err
	}
	...
	if isInitialSync || isRecovery {
		...
		log.L.Debug("bestHeight", bestHeight)
...
func (w *Wallet) resendUnminedTxs() {
	...
	if err != nil {
		log.L.Error(err)
		log.L.Error("no chain server available to resend unmined transactions", err)
		return
	}
	var txs []*wire.MsgTx
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
	...
	for _, tx := range txs {
		...
		log.L.Debug("resent unmined transaction", resp)
	}
}
...
func (w *Wallet) NewAddress(account uint32, scope waddrmgr.KeyScope,
	nochain bool) (util.Address, error) {
	var (
		chainClient chain.Interface
		err         error
		addr        util.Address
...
func Open(db walletdb.DB, pubPass []byte, cbs *waddrmgr.OpenCallbacks,
	...
	log.L.Trace("opened wallet") // TODO: log balance? last sync height?
	w := &Wallet{
