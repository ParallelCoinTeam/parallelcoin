...
func (w *Wallet) handleChainNotifications() {
	...
	if err != nil {
		slog.Error(err)
		slog.Error("handleChainNotifications called without RPC client", err)
		return
	}
	sync := func(w *Wallet) {
		if w.db != nil {
			...
			if err != nil && !w.ShuttingDown() {
				slog.Warn("unable to synchronize wallet to chain:", err)
			}
		}
	}
	catchUpHashes := func(w *Wallet, client chain.Interface,
		...
		if err = walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) (err error) {
			...
			slog.Errorf("failed to update address manager sync state for height %d: %v", height, err)
		}
	...
	for {
		...
		case n, ok := <-chainClient.Notifications():
				...
				if len(n.RelevantTxs) > 0 {
					err = walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) (err error) {
						for _, rec := range n.RelevantTxs {
							if err = w.addRelevantTx(tx, rec, n.Block); slog.Check(err) {
								return
							}
						}
						return
			...
			if err != nil {
				...
				errStr := "failed to process consensus server notification (name: `%s`, detail: `%v`)"
				if notificationName == "blockconnected" &&
					strings.Contains(err.Error(), "couldn't get hash from database") {
					slog.Debugf(errStr, notificationName, err)
				} else {
					slog.Errorf(errStr, notificationName, err)
				}
			}
		case <-w.quit:
			return
...
func (w *Wallet) connectBlock(dbtx walletdb.ReadWriteTx, b tm.BlockMeta) (err error) {
	...
	if err = w.Manager.SetSyncedTo(addrmgrNs, &bs); slog.Check(err) {
		return
	}
...
func (w *Wallet) disconnectBlock(dbtx walletdb.ReadWriteTx, b tm.BlockMeta) (err error) {
	addrmgrNs := dbtx.ReadWriteBucket(waddrmgrNamespaceKey)
	...
	if b.Height <= w.Manager.SyncedTo().Height {
		...
		if hash, err = w.Manager.BlockHash(addrmgrNs, b.Height); slog.Check(err) {
			return
		}
		if bytes.Equal(hash[:], b.Hash[:]) {
			bs := wm.BlockStamp{Height: b.Height - 1}
			if hash, err = w.Manager.BlockHash(addrmgrNs, bs.Height); slog.Check(err) {
				return
			}
			b.Hash = *hash
			client := w.ChainClient()
			var header *wire.BlockHeader
			if header, err = client.GetBlockHeader(hash); slog.Check(err) {
				return
			}
			...
			if err = w.TxStore.Rollback(txmgrNs, b.Height); slog.Check(err) {
				return
			}
		}
	}
...
func (w *Wallet) addRelevantTx(dbtx walletdb.ReadWriteTx, rec *tm.TxRecord, block *tm.BlockMeta) (err error) {
	addrmgrNs := dbtx.ReadWriteBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadWriteBucket(wtxmgrNamespaceKey)
	...
	if err = w.TxStore.InsertTx(txmgrNs, rec, block); slog.Check(err) {
		return
	}
	...
	for i, output := range rec.MsgTx.TxOut {
		...
		for _, addr := range addrs {
			var ma wm.ManagedAddress
			if ma, err = w.Manager.Address(addrmgrNs, addr); slog.Check(err) {
				// TODO: Credits should be added with the account they belong to, so tm is able to track per-account
				//  balances.
				if err = w.TxStore.AddCredit(txmgrNs, rec, block, uint32(i), ma.Internal()); slog.Check(err) {
					return
				}
				if err = w.Manager.MarkUsed(addrmgrNs, addr); slog.Check(err) {
					return
				}
				slog.Trace("marked address used:", addr)
				continue
			}
	...
	if block == nil {
		...
		if err != nil {
			slog.Error(err)
			slog.Error("cannot query transaction details for notification:", err)
		}
		...
		var details *tm.TxDetails
		if details, err = w.TxStore.UniqueTxDetails(txmgrNs, &rec.Hash, &block.Block); slog.Check(err) {
			slog.Error("cannot query transaction details for notification:", err)
		}
