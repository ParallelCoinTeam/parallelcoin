...
// of the blocks to search.
type RecoveryManager struct {
	// recoveryWindow defines the key-derivation lookahead used when attempting to recover the set of used addresses.
	recoveryWindow uint32
	// started is true after the first block has been added to the batch.
	started bool
	// blockBatch contains a list of blocks that have not yet been searched for recovered addresses.
...
func (rm *RecoveryManager) Resurrect(
	...
	for keyScope, scopedMgr := range scopedMgrs {
		// Load the current account properties for this scope, using the the default account number.
		//
		...
		// Walk through all indexes through the last external key, deriving each address and adding it to the external
		...
		// Fetch the internal key count, which bounds the indexes we will need to rederive.
		internalCount := acctProperties.InternalKeyCount
		// Walk through all indexes through the last internal key, deriving each address and adding it to the internal
		// branch recovery state's set of addresses to look for.
		...
		// The key counts will point to the next key that can be derived, so we subtract one to point to last known key.
		// If the key count is zero, then no addresses have been found.
		if externalCount > 0 {
...
//
// In order to ensure that all addresses are properly recovered, the window should be sized as the sum of maximum
...
type RecoveryState struct {
	// recoveryWindow defines the key-derivation lookahead used when attempting to recover the set of used addresses.
	// This value will be used to instantiate a new RecoveryState for each requested scope.
	recoveryWindow uint32
	// scopes maintains a map of each requested key scope to its active RecoveryState.
...
// subsequently initialized for a particular key scope will receive the same recoveryWindow.
...
func (rs *RecoveryState) StateForScope(
	...
	// Otherwise, initialize the recovery state for this scope with the chosen recovery window.
...
// ScopeRecoveryState is used to manage the recovery of addresses generated under a particular BIP32 account. Each
// account tracks both an external and internal branch recovery state, both of which use the same recovery window.
...
// BranchRecoveryState maintains the required state in-order to properly recover addresses derived from a particular
// account's internal or external derivation branch.
//
...
type BranchRecoveryState struct {
	// recoveryWindow defines the key-derivation lookahead used when attempting to recover the set of addresses on this
	// branch.
	recoveryWindow uint32
	// horizion records the highest child index watched by this branch.
	horizon uint32
	// nextUnfound maintains the child index of the successor to the highest index that has been found during recovery
	// of this branch.
	nextUnfound uint32
...
// NewBranchRecoveryState creates a new BranchRecoveryState that can be used to track either the external or internal
// branch of an account's derivation path.
...
// ExtendHorizon returns the current horizon and the number of addresses that must be derived in order to maintain the
// desired recovery window.
func (brs *BranchRecoveryState) ExtendHorizon() (uint32, uint32) {
	// Compute the new horizon, which should surpass our last found address by the recovery window.
	curHorizon := brs.horizon
	nInvalid := brs.NumInvalidInHorizon()
	minValidHorizon := brs.nextUnfound + brs.recoveryWindow + nInvalid
	// If the current horizon is sufficient, we will not have to derive any new keys.
	if curHorizon >= minValidHorizon {
		return curHorizon, 0
	}
	// Otherwise, the number of addresses we should derive corresponds to the delta of the two horizons, and we update
	// our new horizon.
...
func (brs *BranchRecoveryState) ReportFound(index uint32) {
	if index >= brs.nextUnfound {
		...
		// Prune all invalid child indexes that fall below our last found index. We don't need to keep these entries any
		// longer, since they will not affect our required look-ahead.
...
// MarkInvalidChild records that a particular child index results in deriving an invalid address. In addition, the
// branch's horizon is increment, as we expect the caller to perform an additional derivation to replace the invalid
// child. This is used to ensure that we are always have the proper lookahead when an invalid child is encountered.
...
// NumInvalidInHorizon computes the number of invalid child indexes that lie between the last found and current horizon.
// This informs how many additional indexes to derive in order to maintain the proper number of valid addresses within
// our horizon.
