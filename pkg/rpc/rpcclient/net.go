...
// FutureAddNodeResult is a future promise to deliver the result of an AddNodeAsync RPC invocation (or an applicable
// error).
...
// Node attempts to perform the passed node command on the host. For example, it can be used to add or a remove a
...
// FutureGetAddedNodeInfoResult is a future promise to deliver the result of a GetAddedNodeInfoAsync RPC invocation (or
// an applicable error).
...
// GetAddedNodeInfoAsync returns an instance of a type that can be used to get the result of the RPC at some future time
// by invoking the Receive function on the returned instance. See GetAddedNodeInfo for the blocking version and more
// details.
...
// FutureGetAddedNodeInfoNoDNSResult is a future promise to deliver the result of a GetAddedNodeInfoNoDNSAsync RPC
...
// GetAddedNodeInfoNoDNSAsync returns an instance of a type that can be used to get the result of the RPC at some future
// time by invoking the Receive function on the returned instance. See GetAddedNodeInfoNoDNS for the blocking version
// and more details.
...
// GetAddedNodeInfoNoDNS returns a list of manually added (persistent) peers. This works by setting the dns flag to
// false in the underlying RPC. See GetAddedNodeInfo to obtain more information about each added (persistent) peer.
func (c *Client) GetAddedNodeInfoNoDNS(peer string) ([]string, error) {
	return c.GetAddedNodeInfoNoDNSAsync(peer).Receive()
}

// FutureGetConnectionCountResult is a future promise to deliver the result of a GetConnectionCountAsync RPC invocation
...
// GetConnectionCountAsync returns an instance of a type that can be used to get the result of the RPC at some future
// time by invoking the Receive function on the returned instance. See GetConnectionCount for the blocking version and
// more details.
...
// the Receive function on the returned instance. See Ping for the blocking version and more details.
...
// FutureGetPeerInfoResult is a future promise to deliver the result of a GetPeerInfoAsync RPC invocation (or an
// applicable error).
...
// GetPeerInfoAsync returns an instance of a type that can be used to get the result of the RPC at some future time by
...
// FutureGetNetTotalsResult is a future promise to deliver the result of a GetNetTotalsAsync RPC invocation (or an
// applicable error).
...
// GetNetTotalsAsync returns an instance of a type that can be used to get the result of the RPC at some future time by
// invoking the Receive function on the returned instance.
//
