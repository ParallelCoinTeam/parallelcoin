...
out:
	for {
		...
		select {
		case msg := <-c.sendChan:
			Trace("### sendChan received message to send")
			var err error
			if err = c.wsConn.WriteMessage(websocket.TextMessage, msg);Check(err){
				Trace("### sendChan disconnecting client")
				c.Disconnect()
				break out
			}
			Trace("### message sent")
