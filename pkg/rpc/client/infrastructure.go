...
var (
	// ErrInvalidAuth is an error to describe the condition where the client is either unable to authenticate or the
	// specified endpoint is incorrect.
	ErrInvalidAuth = errors.New("authentication failure")
	// ErrInvalidEndpoint is an error to describe the condition where the websocket handshake failed with the specified
	// endpoint.
	ErrInvalidEndpoint = errors.New("the endpoint either does not support websockets or does not exist")
	// ErrClientNotConnected is an error to describe the condition where a websocket client has been created, but the
	...
	// ErrClientDisconnect is an error to describe the condition where the client has been disconnected from the RPC
	// server.
	//
	// When the DisableAutoReconnect option is not set, any outstanding futures when a client disconnect occurs will
	// return this error as will any new requests.
	ErrClientDisconnect = errors.New("the client has been disconnected")
	// ErrClientShutdown is an error to describe the condition where the client is either already shutdown, or in the
	// process of shutting down.
	//
	// Any outstanding futures when a client shutdown occurs will return this error as will any new requests.
	ErrClientShutdown = errors.New("the client has been shutdown")
	// ErrNotWebsocketClient is an error to describe the condition of calling a Client method intended for a websocket
	// client when the client has been configured to run in HTTP POST mode instead.
	ErrNotWebsocketClient = errors.New("client is not configured for websockets")
	// ErrClientAlreadyConnected is an error to describe the condition where a new client connection cannot be
	// established due to a websocket client having already connected to the RPC server.
	ErrClientAlreadyConnected = errors.New("websocket client has already connected")
)

const (
	// sendBufferSize is the number of elements the websocket send channel can queue before blocking.
	sendBufferSize = 50
	// sendPostBufferSize is the number of elements the HTTP POST send channel can queue before blocking.
	sendPostBufferSize = 100
	// connectionRetryInterval is the amount of time to wait in between retries when automatically reconnecting to an
	// RPC server.
	connectionRetryInterval = time.Second * 5
)

// sendPostDetails houses an HTTP POST request to send to an RPC server as well as the original JSON-RPC command and a
// channel to reply on when the server responds with the result.
type sendPostDetails struct {
	httpRequest *http.Request
	jsonRequest *jsonRequest
}

// jsonRequest holds information about a json request that is used to properly detect, interpret, and deliver a reply to
// it.
...
// Client represents a Bitcoin RPC client which allows easy access to the various RPC methods available on a Bitcoin RPC
// server.
//
// Each of the wrapper functions handle the details of converting the passed and return types to and from the underlying
// JSON types which are required for the JSON-RPC invocations
//
// The client provides each RPC in both synchronous ( blocking) and asynchronous (non-blocking) forms.
//
// The asynchronous forms are based on the concept of futures where they return an instance of a type that promises to
// deliver the result of the invocation at some future time.
//
// Invoking the Receive method on the returned future will block until the result is available if it's not already.
type Client struct {
...
// NextID returns the next id to be used when sending a JSON-RPC message. This ID allows responses to be associated with
// particular requests per the JSON-RPC specification.
//
// Typically the consumer of the client does not need to call this function, however, if a custom request is being
// created and used this function should be used to ensure the ID is unique amongst all requests being made.
...
//
// This allows the response from the remote server to be unmarshalled to the appropriate type and sent to the specified
// channel when it is received.
//
// If the client has already begun shutting down, ErrClientShutdown is returned and the request is not added. This
// function is safe for concurrent access.
func (c *Client) addRequest(jReq *jsonRequest) error {
	c.requestLock.Lock()
	defer c.requestLock.Unlock()
	// A non-blocking read of the shutdown channel with the request lock held avoids adding the request to the client's
...
// with the passed id or nil if there is no association. This function is safe for concurrent access.
...
// notification state that is used to automatically re-establish registered notifications on reconnects.
...
type (
	// inMessage is the first type that an incoming message is unmarshalled into. It supports both requests (for
	// notification support) and responses.
	//
	// The partially-unmarshaled message is a notification if the embedded ID (from the response) is nil. Otherwise, it
	// is a response.
	inMessage struct {
		ID *float64 `json:"id"`
		*rawNotification
	...
	// rawResponse is a partially-unmarshalled JSON-RPC response. For this to be valid (according to JSON-RPC 1.0 spec),
	// ID may not be nil.
	rawResponse struct {
...
//
// If the response is not an error, the raw bytes of the request are returned for further unmashaling into specific
// result types.
...
func (c *Client) handleMessage(msg []byte) {
	...
	// Since the command was successful, examine it to see if it's a notification, and if is, add it to the notification
	// state so it automatically be re-established on reconnect.
...
// shouldLogReadError returns whether or not the passed error, which is expected to have come from reading from the
// websocket connection in wsInHandler, should be logged.
...
// wsInHandler handles all incoming messages for the websocket connection associated with the client. It must be run as
// a goroutine.
	...
	for {
		...
		if err != nil && err != io.ErrUnexpectedEOF && err != io.EOF {
			...
			if c.shouldLogReadError(err) {
				Tracef("websocket receive error from %s: %v %s", c.config.Host, err)
			}
			break out
		}
		c.handleMessage(msg)
...
//
// The channel is read protected by the client mutex, and is safe to call while the channel is being reassigned during a
// reconnect.
...
// sendMessage sends the passed JSON to the connected server using the websocket connection. It is backed by a buffered
...
// reregisterNtfns creates and sends commands needed to re-establish the current notification state associated with the
// client.
//
...
func (c *Client) reregisterNtfns() error {
	...
	// RPCs issued below, make a copy of it and work from that.
	//
	// Also, other commands will be running concurrently which could modify the notification state (while not under the
	// lock of course) which also register it with the remote RPC server, so this prevents double registrations.
	c.ntfnStateLock.Lock()
...
func (c *Client) resendRequests() {
	...
	// Since it's possible to block on send and more requests might be added by the caller while resending, make a copy
	// of all of the requests that need to be resent now and work from the copy. This allows the lock to be released
	// quickly.
	c.requestLock.Lock()
...
// wsReconnectHandler listens for client disconnects and automatically tries to reconnect with retry interval that
// scales based on the number of retries.
//
// It also resends any commands that had not completed when the client disconnected so the disconnect/reconnect process
// is largely transparent to the caller.
//
...
// handleSendPostMessage handles performing the passed HTTP request, reading the result unmarshalling it and delivering
...
// sendPostHandler handles all outgoing messages when the client is running in HTTP POST mode.
//
// It uses a buffered channel to serialize output messages while allowing the sender to continue running asynchronously.
// It must be run as a goroutine.
...
// sendPostRequest sends the passed HTTP request to the RPC server using the HTTP client associated with the client.
//
// It is backed by a buffered channel so it will not block until the send channel is full.
...
// newFutureError returns a new future result channel that already has the passed error waiting on the channel with the
// reply set to nil. This is useful to easily return errors from the various Async functions.
...
//
// The examined errors include an error in the futureResult and the error in the reply from the server. This will block
...
// sendPost sends the passed request to the server by issuing an HTTP POST request using the provided response channel
// for the reply.
//
...
// sendRequest sends the passed json request to the associated server using the provided response channel for the reply.
//
// It handles both websocket and HTTP POST mode depending on the configuration of the client.
func (c *Client) sendRequest(jReq *jsonRequest) {
	// Choose which marshal and send function to use depending on whether the client running in HTTP POST mode or not.
	//
	// When running in HTTP POST mode, the command is issued via an HTTP client. Otherwise, the command is issued via
	// the asynchronous websocket channels.
	...
	// Add the request to the internal tracking map so the response from the remote server can be properly detected and
	// routed to the response channel. Then send the marshalled request via the websocket connection.
...
// sendCmd sends the passed command to the associated server and returns a response channel on which the reply will be
...
// sendCmdAndWait sends the passed command to the associated server, waits for the reply, and returns the result from
// it.
//
// It will return the error field in the reply if there is one.
func (c *Client) sendCmdAndWait(cmd interface{}) (interface{}, error) {
	// Marshal the command to JSON-RPC, send it to the connected server, and wait for a response on the returned
	// channel.
	return receiveFuture(c.sendCmd(cmd))
}

// Disconnected returns whether or not the server is disconnected. If a websocket client was created but never
// connected, this also returns false.
...
// doDisconnect disconnects the websocket associated with the client if it hasn't already been disconnected.
//
// It will return false if the disconnect is not needed or the client is running in HTTP POST mode. This function is
...
//
// The connection will automatically be re-established unless the client was created with the DisableAutoReconnect flag.
...
// Shutdown shuts down the client by disconnecting any connections associated with the client and, when automatic
// reconnect is enabled, preventing future attempts to reconnect. It also stops all goroutines.
func (c *Client) Shutdown() {
	// Do the shutdown under the request lock to prevent clients from adding new requests while the client shutdown
...
func dial(config *ConnConfig) (*websocket.Conn, error) {
	...
	// Create a websocket dialer that will be used to make the connection. It is modified by the proxy setting below as
	// needed.
	...
	// The RPC server requires basic authorization, so create a custom request header with the Authorization header set.
	...
	if err != nil {
		...
		// The connection was authenticated and the status response was ok, but the websocket handshake still failed, so
		// the endpoint is invalid in some way.
...
// Connect establishes the initial websocket connection. This is necessary when a client was created after setting the
// DisableConnectOnNew field of the Config struct.
//
// Up to tries number of connections (each after an increasing backoff) will be tried if the connection can not be
// established. The special value of 0 indicates an unlimited number of connection attempts.
//
// This method will error if the client is not configured for websockets, if the connection has already been
// established, or if none of the connection attempts were successful.
func (c *Client) Connect(tries int) error {
	...
	// Begin connection attempts. Increase the backoff after each failed attempt, up to a maximum of one minute.
	var err error
	...
	for i := 0; tries == 0 || i < tries; i++ {
		...
		// Connection was established. Set the websocket connection member of the client and start the goroutines
