...
func (r FutureGenerateResult) Receive() (convertedResult []*chainhash.Hash, err error) {
	var res []byte
	...
	for i, hashString := range result {
		if convertedResult[i], err = chainhash.NewHashFromStr(hashString); slog.Check(err) {
			return
		}
	}
	return
...
func (c *Client) Generate(numBlocks uint32) (h []*chainhash.Hash, err error) {
...
func (r FutureGetGenerateResult) Receive() (result bool, err error) {
	var res []byte
	if res, err = receiveFuture(r); slog.Check(err) {
		return
	}
...
func (c *Client) GetGenerate() (b bool, err error) {
...
func (r FutureSetGenerateResult) Receive() (err error) {
...
func (c *Client) SetGenerate(enable bool, numCPUs int) (err error) {
...
func (r FutureGetHashesPerSecResult) Receive() (result int64, err error) {
	var res []byte
	if res, err = receiveFuture(r); slog.Check(err) {
		result = -1
		return
...
func (c *Client) GetHashesPerSec() (i int64, err error) {
...
func (r FutureGetMiningInfoResult) Receive() (infoResult *btcjson.GetMiningInfoResult, err error) {
	var res []byte
	...
	if err = js.Unmarshal(res, &infoResult); slog.Check(err) {
	}
	return
}
...
func (c *Client) GetMiningInfo() (r *btcjson.GetMiningInfoResult, err error) {
...
func (r FutureGetNetworkHashPS) Receive() (result int64, err error) {
	var res []byte
	res, err = receiveFuture(r)
	if slog.Check(err) {
		result = -1
...
func (c *Client) GetNetworkHashPS() (i int64, err error) {
...
func (c *Client) GetNetworkHashPS2(blocks int) (i int64, err error) {
...
func (c *Client) GetNetworkHashPS3(blocks, height int) (i int64, err error) {
...
func (r FutureGetWork) Receive() (result *btcjson.GetWorkResult, err error) {
	var res []byte
	if res, err = receiveFuture(r); slog.Check(err) {
		return
	}
...
func (c *Client) GetWork() (i *btcjson.GetWorkResult, err error) {
...
func (r FutureGetWorkSubmit) Receive() (accepted bool, err error) {
	var res []byte
	if res, err = receiveFuture(r); slog.Check(err) {
		return
	}
	// Unmarshal result as a boolean.
	if err = js.Unmarshal(res, &accepted); slog.Check(err) {
		return
	}
	return
}
...
func (c *Client) GetWorkSubmit(data string) (b bool, err error) {
...
func (r FutureSubmitBlockResult) Receive() (err error) {
	res, err := receiveFuture(r)
	if err != nil {
		slog.Error(err)
		return err
...
func (c *Client) SubmitBlock(block *util.Block, options *btcjson.SubmitBlockOptions) (err error) {
