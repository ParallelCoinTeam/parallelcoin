...
// ToPeer returns the underlying peer instance. This function is safe for concurrent access and is part of the
...
// IsTxRelayDisabled returns whether or not the peer has disabled transaction relay. This function is safe for
// concurrent access and is part of the RPCServerPeer interface implementation.
func (p *Peer) IsTxRelayDisabled() bool {
	return (*NodePeer)(p).DisableRelayTx
}

// GetBanScore returns the current integer value that represents how close the peer is to being banned. This function is
// safe for concurrent access and is part of the RPCServerPeer interface implementation.
func (p *Peer) GetBanScore() uint32 {
	return (*NodePeer)(p).BanScore.Int()
}

// GetFeeFilter returns the requested current minimum fee rate for which transactions should be announced. This function
// is safe for concurrent access and is part of the RPCServerPeer interface implementation.
...
// return an error. This function is safe for concurrent access and is part of the RPCServerConnManager interface
// implementation.
...
// RemoveByID removes the peer associated with the provided id from the list of persistent peers. Attempting to remove
// an id that does not exist will return an error. This function is safe for concurrent access and is part of the
...
// RemoveByAddr removes the peer associated with the provided address from the list of persistent peers. Attempting to
// remove an address that does not exist will return an error. This function is safe for concurrent access and is part
...
// DisconnectByID disconnects the peer associated with the provided id. This applies to both inbound and outbound peers.
// Attempting to remove an id that does not exist will return an error. This function is safe for concurrent access and
// is part of the RPCServerConnManager interface implementation.
...
// peers. Attempting to remove an address that does not exist will return an error. This function is safe for concurrent
// access and is part of the RPCServerConnManager interface implementation.
...
// ConnectedCount returns the number of currently connected peers. This function is safe for concurrent access and is
// part of the RPCServerConnManager interface implementation.
func (cm *ConnManager) ConnectedCount() int32 {
	return cm.server.ConnectedCount()
}

// NetTotals returns the sum of all bytes received and sent across the network for all peers. This function is safe for
// concurrent access and is part of the RPCServerConnManager interface implementation.
func (cm *ConnManager) NetTotals() (uint64, uint64) {
	return cm.server.NetTotals()
}

// ConnectedPeers returns an array consisting of all connected peers. This function is safe for concurrent access and is
// part of the RPCServerConnManager interface implementation.
...
// PersistentPeers returns an array consisting of all the added persistent peers. This function is safe for concurrent
// access and is part of the RPCServerConnManager interface implementation.
...
// BroadcastMessage sends the provided message to all currently connected peers. This function is safe for concurrent
// access and is part of the RPCServerConnManager interface implementation.
...
// until they show up in a block. This function is safe for concurrent access and is part of the RPCServerConnManager
...
// network. This function is safe for concurrent access and is part of the RPCServerSyncManager interface
// implementation.
func (b *SyncManager) IsCurrent() bool {
	return b.syncMgr.IsCurrent()
}

// SubmitBlock submits the provided block to the network after processing it locally. This function is safe for
// concurrent access and is part of the RPCServerSyncManager interface implementation.
func (b *SyncManager) SubmitBlock(block *util.Block,
	flags blockchain.BehaviorFlags) (success bool, err error) {
	return b.syncMgr.ProcessBlock(block, flags)
}

// pause pauses the sync manager until the returned channel is closed. This function is safe for concurrent access and
// is part of the RPCServerSyncManager interface implementation.
func (b *SyncManager) Pause() chan<- struct{} {
	return b.syncMgr.Pause()
}

// SyncPeerID returns the peer that is currently the peer being used to sync from. This function is safe for concurrent
// access and is part of the RPCServerSyncManager interface implementation.
...
// stop hash or the current tip is reached, up to a max of wire.MaxBlockHeadersPerMsg hashes. This function is safe for
// concurrent access and is part of the RPCServerSyncManager interface implementation.
