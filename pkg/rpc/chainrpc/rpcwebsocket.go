...
func (s *Server) WebsocketHandler(conn *websocket.Conn, remoteAddr string,
	...
	Trace("new websocket client", remoteAddr)
	if s.NtfnMgr.GetNumClients()+1 > *s.Config.RPCMaxWebsockets {
		Infof("max websocket clients exceeded [%d] - disconnecting client"+
			" %s",
	...
	Trace("disconnected websocket client", remoteAddr)
}
...
func (c *WSClient) Start() {
	Trace("starting websocket client", c.Addr)
	...
	for {
		...
		if err != nil {
			...
			if err != io.EOF && err != io.ErrUnexpectedEOF {
				Tracef("websocket receive error from %s: %v",
					c.Addr, err)
			}
			break out
		}
		...
		if cmd.Err != nil {
			...
			if err != nil {
				Error(err)
				Errorf("failed to marshal parse failure reply:", err)
				continue
			}
			c.SendMessage(reply, nil)
			continue
		}
		Tracef("received command <%s> from %s", cmd.Method, c.Addr)
		// Check auth. The client is immediately disconnected if the first request of an unauthenticated websocket
		...
		case c.Authenticated && ok:
			...
			break out
		case !c.Authenticated && !ok:
			Warnf("unauthenticated websocket message received")
			break out
		case !c.Authenticated:
			...
			if cmp != 1 && limitcmp != 1 {
				Warn("authentication failure from", c.Addr)
				break out
			}
			...
			if err != nil {
				Error(err)
				Error("failed to marshal authenticate reply:", err)
				continue
			}
			c.SendMessage(reply, nil)
			continue
...
cleanup:
	...
	c.WG.Done()
	Trace("websocket client output handler done for", c.Addr)
}
...
OutPoint]map[qu.C]*WSClient, wsc *WSClient, ops []*wire.OutPoint) {
	...
	// Check if any transactions spending these outputs already exists in the mempool, if so send the notification
	// immediately.
	spends := make(map[chainhash.Hash]*util.Tx)
	for _, op := range ops {
		spend := m.Server.Cfg.TxMemPool.CheckSpend(*op)
		if spend != nil {
			Debugf("found existing mempool spend for outpoint<%v>: %v",
				op, spend.Hash())
...
func (m *WSNtfnMgr) NotifyFilteredBlockConnected(
	...
	if err != nil {
		Error("failed to serialize header for filtered block connected"+
			" notification:", err)
		return
	}
	...
	for quitChan, wsc := range clients {
		...
		marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
		if err != nil {
			Errorf("failed to marshal filtered block connected notification:", err)
			return
		}
		err = wsc.QueueNotification(marshalledJSON)
		if err != nil {
...
func (*WSNtfnMgr) NotifyFilteredBlockDisconnected(
	...
	err := block.MsgBlock().Header.Serialize(&w)
	if err != nil {
		Error("failed to serialize header for filtered block disconnected notification:", err)
		return
	}
	ntfn := btcjson.NewFilteredBlockDisconnectedNtfn(block.Height(), hex.EncodeToString(w.Bytes()))
	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
	if err != nil {
		Error("failed to marshal filtered block disconnected notification:", err)
		return
	}
	for _, wsc := range clients {
		err := wsc.QueueNotification(marshalledJSON)
...
// memory pool.
func (m *WSNtfnMgr) NotifyForNewTx(clients map[qu.C]*WSClient,
	tx *util.Tx) {
...
func (m *WSNtfnMgr) NotifyForTx(ops map[wire.OutPoint]map[qu.C]*WSClient,
...
// matching output.
func (m *WSNtfnMgr) NotifyForTxOuts(ops map[wire.OutPoint]map[qu.C]*WSClient,
	addrs map[string]map[qu.C]*WSClient, tx *util.Tx, block *util.Block) {
	...
	for i, txOut := range tx.MsgTx().TxOut {
		...
		for _, txAddr := range txAddrs {
			...
			marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
			if err != nil {
				Error("Failed to marshal processedtx notification:", err)
				continue
			}
...
func (m *WSNtfnMgr) NotifyRelevantTxAccepted(tx *util.Tx, clients map[qu.C]*WSClient) {
	...
	if len(clientsToNotify) != 0 {
		...
		if err != nil {
			Error("failed to marshal notification:", err)
			return
		}
...
func (*WSNtfnMgr) RemoveAddrRequest(
	...
	cmap, ok := addrs[addr]
	if !ok {
		Warnf("attempt to remove nonexistent addr request <%s> for websocket client %s",
			addr, wsc.Addr)
		return
	}
...
OutPoint]map[qu.C]*WSClient, wsc *WSClient, op *wire.OutPoint) {
	...
	notifyMap, ok := ops[*op]
	if !ok {
		Warn("attempt to remove nonexistent spent request for"+
...
func (m *WSNtfnMgr) GetSubscribedClients(tx *util.Tx,
	clients map[qu.C]*WSClient) map[qu.C]struct{} {
...
func HandleRescan(wsc *WSClient, icmd interface{}) (interface{}, error) {
	...
	if numAddrs == 1 {
		Debug("beginning rescan for 1 address")
	} else {
		Debugf("beginning rescan for %d addresses", numAddrs)
	}
	// Build lookup maps.
	lookups := RescanKeys{
		Fallbacks:           map[string]struct{}{},
...
fetchRange:
	for minBlock < maxBlock {
		...
		if err != nil {
			Error("error looking up block range:", err)
			return nil, &btcjson.RPCError{
				Code:    btcjson.ErrRPCDatabase,
		...
		for i := range hashList {
			...
			if err != nil {
				...
				if dbErr, ok := err.(database.DBError); !ok ||
					...
					Error("error looking up block:", err)
					return nil, &btcjson.RPCError{
						Code: btcjson.ErrRPCDatabase,
				...
				if maxBlock != math.MaxInt32 {
					Error("stopping rescan for reorged block", cmd.EndBlock)
...
func RecoverFromReorg(chain *blockchain.BlockChain, minBlock, maxBlock int32,
	...
	if err != nil {
		Error("error looking up possibly reorged block:", err)
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCDatabase,
			Message: "Database error: " + err.Error(),
		}
	...
	for _, tx := range blk.Transactions() {
		...
		for _, txin := range tx.MsgTx().TxIn {
			if _, ok := lookups.Unspent[txin.PreviousOutPoint]; ok {
				...
				marshalledJSON, err := NewRedeemingTxNotification(txHex,
					tx.Index(), blk)
				if err != nil {
					Error("failed to marshal redeemingtx notification:",
						err)
					continue
				}
				err = wsc.QueueNotification(marshalledJSON)
		...
		for txOutIdx, txout := range tx.MsgTx().TxOut {
			...
			for _, addr := range addrs {
				...
				marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
				if err != nil {
					Error("failed to marshal recvtx notification:", err)
					return
				}
				err = wsc.QueueNotification(marshalledJSON)
				// Stop the rescan early if the websocket client disconnected.
