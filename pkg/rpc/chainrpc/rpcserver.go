...
func (state *GBTWorkState) UpdateBlockTemplate(s *Server,
	useCoinbaseValue bool) (err error) {
...
func (s *Server) Start() {
	...
	rpcServeMux.HandleFunc("/ws", func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			if _, ok := err.(websocket.HandshakeError); !ok {
				slog.Error("unexpected websocket error:", err)
	...
	for _, listener := range s.Cfg.Listeners {
		...
		go func(listener net.Listener) {
			slog.Info("chain RPC server listening on ", listener.Addr())
			err := httpServer.Serve(listener)
			if err != nil {
				slog.Trace(err)
			}
			slog.Trace("chain RPC listener done for", listener.Addr())
...
func (s *Server) Stop() (err error) {
	if atomic.AddInt32(&s.Shutdown, 1) != 1 {
		slog.Warn("RPC server is already in the process of shutting down")
		return nil
	}
	slog.Trace("RPC server shutting down")

	for _, listener := range s.Cfg.Listeners {
		err := listener.Close()
		if err != nil {
			slog.Error(err)
			slog.Error("problem shutting down RPC:", err)
	...
	// close(s.Quit)
	s.WG.Wait()
	slog.Debug("RPC server shutdown complete")
	return nil
}
...
func (s *Server) CheckAuth(r *http.Request, require bool) (success bool, can bool, err error) {
	...
	if len(authhdr) == 0 {
		if require {
			slog.Warn("RPC authentication failure from", r.RemoteAddr)
...
func (s *Server) JSONRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if err != nil {
		slog.Error(err)
		slog.Warn("failed to hijack HTTP connection:", err)
	...
	if err != nil {
		slog.Error(err)
		slog.Error("failed to marshal reply:", err)
	...
	if _, err := buf.Write(msg); err != nil {
		slog.Error("failed to write marshalled reply:", err)

	}
	// Terminate with newline to maintain compatibility with Bitcoin Core.
	if err := buf.WriteByte('\n'); err != nil {
		slog.Error("failed to append terminating newline to reply:", err)
...
func GenCertPair(certFile, keyFile string) (err error) {
...
func GetDifficultyRatio(bits uint32, params *netparams.Params,
	...
	if err != nil {
		slog.Error(err)
		slog.Error("cannot get difficulty:", err)
...
// VerifyChain does?
func VerifyChain(s *Server, level, depth int32) (err error) {
	best := s.Cfg.Chain.BestSnapshot()
	...
	slog.Info("chain verify completed successfully")
	return nil
}

/*
// handleDebugLevel handles debuglevel commands.
func handleDebugLevel(	s *RPCServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, err error) {
