	...
	serveMux.Handle("/", ThrottledFn(opts.MaxPOSTClients,
		func(w http.ResponseWriter, r *http.Request) {
			...
			if err := server.CheckAuthHeader(r); err != nil {
				slog.Warn("unauthorized client connection attempt")
				JSONAuthFail(w)
				return
			}
			server.WG.Add(1)
	...
	serveMux.Handle("/ws", ThrottledFn(opts.MaxWebsocketClients,
		func(w http.ResponseWriter, r *http.Request) {
			...
			default:
				...
				slog.Warn("disconnecting improperly authorized websocket client")
				JSONAuthFail(w)
				return
			}
...
func (s *Server) Serve(lis net.Listener) {
	s.WG.Add(1)
	go func() {
		slog.Info("wallet RPC server listening on ", lis.Addr())
		err := s.HTTPServer.Serve(lis)
		slog.Trace("finished serving wallet RPC:", err)
		s.WG.Done()
	}()
}
...
out:
	...
	close(wsc.quit)
	slog.Info("disconnected websocket client", wsc.remoteAddr)
	s.WG.Done()
}
