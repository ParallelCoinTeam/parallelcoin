...
func BuildMerkleTreeStore(transactions []*util.Tx, witness bool) []*chainhash.Hash {
	// Calculate how many entries are required to hold the binary merkle tree as
	// a linear array and create an array of that size.
	nextPoT := nextPowerOfTwo(len(transactions))
	arraySize := nextPoT*2 - 1
	merkles := make([]*chainhash.Hash, arraySize)
	// Create the base transaction hashes and populate the array with them.
	for i, tx := range transactions {
		// If we're computing a witness merkle root, instead of the regular txid,
		// we use the modified wtxid which includes a transaction's witness data
		// within the digest. Additionally, the coinbase's wtxid is all zeroes.
		switch {
		case witness && i == 0:
