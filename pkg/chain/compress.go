...
// Any scripts which are not recognized as one of the aforementioned standard scripts are encoded using the general serialized format and encode the script size as the sum of the actual size of the script and the number of special cases.
// The following constants specify the special constants used to identify a special script type in the domain-specific compressed script encoding. NOTE: This section specifically does not use iota since these values are serialized and must be stable for long-term storage.
const (
	...
	// cstPayToPubKeyComp2 identifies a compressed pay-to-pubkey script to a compressed pubkey.  Bit 0 specifies which y-coordinate to use to reconstruct the full uncompressed pubkey.
	cstPayToPubKeyComp2 = 2
	// cstPayToPubKeyComp3 identifies a compressed pay-to-pubkey script to a compressed pubkey.  Bit 0 specifies which y-coordinate to use to reconstruct the full uncompressed pubkey.
	cstPayToPubKeyComp3 = 3
	// cstPayToPubKeyUncomp4 identifies a compressed pay-to-pubkey script to an uncompressed pubkey.  Bit 0 specifies which y-coordinate to use to reconstruct the full uncompressed pubkey.
	cstPayToPubKeyUncomp4 = 4
	// cstPayToPubKeyUncomp5 identifies a compressed pay-to-pubkey script to an uncompressed pubkey.  Bit 0 specifies which y-coordinate to use to reconstruct the full uncompressed pubkey.
...
func compressedScriptSize(pkScript []byte) int {
	...
	// When none of the above special cases apply, encode the script as is preceded by the sum of its size and the number of special cases encoded as a variable length quantity.
...
func decompressScript(compressedPkScript []byte) []byte {
	...
	// When none of the special cases apply, the script was encoded using the general format, so reduce the script size by the number of special cases and return the unmodified script.
...
func putCompressedScript(target, pkScript []byte) int {
	...
	// When none of the above special cases apply, encode the unmodified script preceded by the sum of its size and the number of special cases encoded as a variable length quantity.
...
// While the encoding allows arbitrarily large integers, it is artificially limited in this code to an unsigned 64-bit integer for efficiency purposes.
// Example encodings:
//           0 -> [0x00]
