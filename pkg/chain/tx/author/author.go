...
type (
	...
	// amount can not be satisified, this can be signaled by returning a total
	// amount less than the target or by returning a more detailed error
	// implementing InputSourceError.
	...
	// unspent transaction outputs to meet a target amount.  A typed error is
	// used so input sources can provide their own implementations describing
	// the reason for the error, for example, due to spendable policies or
	// locked coins rather than the wallet not having enough available input
	// value.
	...
	// ChangeSource provides P2PKH change output scripts for transaction
	// creation.
	...
	// corresponding Address for the previous output script.  Addresses for
	// lookup are created using the source's blockchain parameters and means a
	// single SecretsSource can only manage secrets for a single chain.
	//
	// TODO: Rewrite this interface to look up private keys and redeem scripts
	// for pubkeys, pubkey hashes, script hashes, etc. as separate interface
	// methods.
...
// This function must return a P2WPKH script or smaller, otherwise fee
