...
func (w *Wallet) syncWithChain() error {
	...
	if isInitialSync || isRecovery {
		...
		log.L.Infof("syncWithChain: catching up block hashes to height %d, "+
		...
		if isRecovery {
			log.L.Infof("RECOVERY MODE ENABLED -- rescanning for used"+
		...
		log.L.Info("done catching up block hashes")
...
func (w *Wallet) recoverScopedAddresses(
	...
	log.L.Infof("scanning %d blocks for recoverable addresses",
		len(batch))
expandHorizons:
...
func logFilterBlocksResp(block wtxmgr.BlockMeta,
	...
	if nFoundExternal > 0 {
		log.L.Infof("recovered %d external addrs at height=%d hash=%v", nFoundExternal,
	...
	if nFoundInternal > 0 {
		log.L.Infof("recovered %d internal addrs at height=%d hash=%v",
	...
	if nFoundOutPoints > 0 {
		log.L.Infof("found %d spends from watched outpoints at height=%d hash=%v",
...
out:
	for {
		...
		case req := <-w.unlockRequests:
			...
			if timeout == nil {
				log.L.Info("the wallet has been unlocked without a time limit")
			} else {
				log.L.Info("the wallet has been temporarily unlocked")
			}
			req.err <- nil
			continue
		...
		if err != nil && !waddrmgr.IsError(err, waddrmgr.ErrLocked) {
			log.L.Error("could not lock wallet:", err)
		} else {
			log.L.Info("the wallet has been locked")
		}
	}
	w.wg.Done()
}
...
func (w *Wallet) NextAccount(scope waddrmgr.KeyScope, name string) (uint32, error) {
	...
	if err != nil {
		log.L.Error(err)
		log.L.Error("cannot fetch new account properties for notification after"+
...
func (w *Wallet) ImportPrivateKey(scope waddrmgr.KeyScope, wif *util.WIF,
	...
	log.L.Info("imported payment address", addrStr)
	w.NtfnServer.notifyAccountProperties(props)
...
func (w *Wallet) resendUnminedTxs() {
	...
	if err != nil {
		log.L.Error(err)
		log.L.Error("cannot load unmined transactions for resending:", err)
		return
	}
	for _, tx := range txs {
		...
		if err != nil {
			log.L.Error(err)
			log.L.Debugf("could not resend transaction %v: %v %s",
				tx.TxHash(), err)
			...
			if err != nil {
				log.L.Error(err)
				log.L.Warnf("unable to remove conflicting tx %v: %v",
					tx.TxHash(), err)
				continue
			}
			log.L.Infoc(func() string {
