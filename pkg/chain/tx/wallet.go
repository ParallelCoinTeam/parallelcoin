...
func (w *Wallet) SynchronizeRPC(chainClient chain.Interface) {
	...
	w.quitMu.Unlock()
	// TODO: Ignoring the new client when one is already set breaks callers who are replacing the client, perhaps after
	//  a disconnect.
...
// activeData returns the currently-active receiving addresses and all unspent outputs. This is primarely intended to
...
func (w *Wallet) syncWithChain() error {
	...
	if isInitialSync || isRecovery {
		...
		Infof("syncWithChain: catching up block hashes to height %d, "+
...
func (w *Wallet) recoverScopedAddresses(
	...
	Infof("scanning %d blocks for recoverable addresses",
		len(batch))
expandHorizons:
...
func logFilterBlocksResp(block wtxmgr.BlockMeta,
	...
	if nFoundOutPoints > 0 {
		Infof("found %d spends from watched outpoints at height=%d hash=%v",
...
// // accountUsed returns whether there are any recorded transactions spending to a given account. It returns true if
// // atleast one address in the account was used and false if no address in the account was used.
...
func (w *Wallet) NextAccount(scope waddrmgr.KeyScope, name string) (uint32, error) {
	...
	if err != nil {
		Error(err)
		Error("cannot fetch new account properties for notification after"+
...
func (w *Wallet) ImportPrivateKey(scope waddrmgr.KeyScope, wif *util.WIF,
	...
	if rescan {
		...
		if err != nil {
			Error(err)
			return "", fmt.Errorf(
				"failed to subscribe for address ntfns for address %s: %s",
