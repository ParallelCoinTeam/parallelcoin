	...
	for {
		...
		case n := <-w.rescanNotifications:
			...
			case *chain.RescanProgress:
				if curBatch == nil {
					slog.Warn("received rescan progress" +
			...
			case *chain.RescanFinished:
				if curBatch == nil {
					slog.Warn("received rescan finished notification but no" +
...
out:
	for {
		...
		// unbuffered and are sent from same context (the batch
		// handler).
		select {
...
func (w *Wallet) rescanRPCHandler() {
	...
	if err != nil {
		slog.Error(err)
		slog.Errorf("rescanRPCHandler called without an RPC client", err)
		w.wg.Done()
		return
	}
	quit := w.quitChan()
...
// rescan.
func (w *Wallet) Rescan(addrs []util.Address, unspent []wtxmgr.Credit) (err error) {
...
func (w *Wallet) rescanWithTarget(addrs []util.Address,
	unspent []wtxmgr.Credit, startStamp *waddrmgr.BlockStamp) (err error) {
