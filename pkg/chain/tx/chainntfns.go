...
func (w *Wallet) handleChainNotifications() {
	...
	catchUpHashes := func(w *Wallet, client chain.Interface,
		...
		if err != nil {
			slog.Error(err)
			slog.Errorf("failed to update address manager sync state for height %d"+
				": %v", height, err)
		}
		...
		case n, ok := <-chainClient.Notifications():
			...
			if err != nil {
				...
				if notificationName == "blockconnected" &&
					...
					slog.Debug(errStr, notificationName, err)
				} else {
					slog.Error(errStr, notificationName, err)
				}
			}
		case <-w.quit:
			return
...
func (w *Wallet) addRelevantTx(dbtx walletdb.ReadWriteTx, rec *wtxmgr.TxRecord, block *wtxmgr.BlockMeta) error {
	...
	for i, output := range rec.MsgTx.TxOut {
		...
		for _, addr := range addrs {
			...
			if err == nil {
				...
				slog.Debugf("marked address %v used", addr)
				continue
			}
	...
	if block == nil {
		...
		if err != nil {
			slog.Error(err)
			// L.ScriptError("cannot query transaction details for notification:",				err)
