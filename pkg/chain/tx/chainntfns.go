...
func (w *Wallet) handleChainNotifications() {
	...
	catchUpHashes := func(w *Wallet, client chain.Interface,
		height int32) error {
		// TODO(aakselrod): There's a race conditon here, which happens when a reorg occurs between the rescanProgress
		...
		Infof("catching up block hashes to height %d, this might take a while, %s",
			height)
		...
		if err != nil {
			Errorf("failed to update address manager sync state for height %d: %v", height, err)
		}
		...
		case n, ok := <-chainClient.Notifications():
			...
			if err != nil {
				...
				if notificationName == "blockconnected" &&
					...
					Debug(errStr, notificationName, err)
				} else {
...
func (w *Wallet) addRelevantTx(dbtx walletdb.ReadWriteTx, rec *wtxmgr.TxRecord, block *wtxmgr.BlockMeta) error {
	...
	for i, output := range rec.MsgTx.TxOut {
		...
		for _, addr := range addrs {
			...
			if err == nil {
				// TODO: Credits should be added with the account they belong to, so wtxmgr is able to track per-account
				//  balances.
				...
				Debugf("marked address %v used", addr)
				continue
			}
