...
func SignatureScript(tx *wire.MsgTx, idx int, subscript []byte, hashType SigHashType, privKey *ec.PrivateKey, compress bool) ([]byte, error) {
...
func p2pkSignatureScript(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType, privKey *ec.PrivateKey) ([]byte, error) {
	sig, err := RawTxInSignature(tx, idx, subScript, hashType, privKey)
	if err != nil {
		log.ERROR(err)
log.ERROR(err)
...
// signMultiSig signs as many of the outputs in the provided multisig script as possible. It returns the generated script and a boolean if the script fulfils the contract (i.e. nrequired signatures are provided).  Since it is arguably legal to not be able to sign any of the outputs, no error is returned.
...
func SignTxOutput(chainParams *netparams.Params, tx *wire.MsgTx, idx int,
