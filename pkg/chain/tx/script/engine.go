...
const (
	...
	// ScriptDiscourageUpgradableNops defines whether to verify that NOP1 through NOP10 are reserved for future soft-
	// fork upgrades.  This flag must not be used for consensus critical code nor applied to blocks as this flag is only
	// for stricter standard transaction checks.  This flag is only applied when the above opcodes are executed.
	...
	// ScriptVerifyCleanStack defines that the stack must contain only one stack element after evaluation and that the
	// element must be true if interpreted as a boolean.  This is rule 6 of BIP0062. This flag should never be used
	// without the ScriptBip16 flag nor the ScriptVerifyWitness flag.
	ScriptVerifyCleanStack
	// ScriptVerifyDERSignatures defines that signatures are required to comply with the DER format.
	ScriptVerifyDERSignatures
	// ScriptVerifyLowS defines that signatures are required to comply with the DER format and whose S value is <= order
	// / 2.  This is rule 5
	// of BIP0062.
	ScriptVerifyLowS
	...
	// ScriptVerifyWitnessPubKeyType makes a script within a check-sig operation whose public key isn't serialized in a
	// compressed format non-standard.
	ScriptVerifyWitnessPubKeyType
	// MaxStackSize is the maximum combined height of stack and alt stack during execution.
	MaxStackSize = 1000
	// MaxScriptSize is the maximum allowed length of a raw script.
	MaxScriptSize = 10000
	// payToWitnessPubKeyHashDataSize is the size of the witness program's data push for a pay-to-witness-pub-key-hash
	// output.
	payToWitnessPubKeyHashDataSize = 20
	// payToWitnessScriptHashDataSize is the size of the witness program's data push for a pay-to-witness-script-hash
	// output.
...
// isBranchExecuting returns whether or not the current conditional branch is actively executing.  For example, when
// the data stack has an OP_FALSE on it and an OP_IF is encountered, the branch is inactive until an OP_ELSE or OP_ENDIF
// is encountered.  It properly handles nested conditionals.
...
// executeOpcode performs execution on the passed opcode.  It takes into account whether or not it is hidden by
// conditionals, but some rules still must be tested in this case.
func (vm *Engine) executeOpcode(pop *parsedOpcode) (err error) {
...
// isWitnessVersionActive returns true if a witness program was extracted during the initialization of the Engine, and
// the program's version matches the specified version.
...
func (vm *Engine) verifyWitnessProgram(witness [][]byte) (err error) {
	if vm.isWitnessVersionActive(0) {
		...
		case payToWitnessPubKeyHashDataSize: // P2WKH
			...
			// Set the stack to the provided witness stack, then append the pkScript generated above as the next script
			// to execute.
		...
		case payToWitnessScriptHashDataSize: // P2WSH
			...
			// Obtain the witness script which should be the last element in the passed stack. The size of the script
			...
			// The hash matched successfully, so use the witness as the stack, and set the witnessScript to be the next
			// script executed.
			vm.scripts = append(vm.scripts, pops)
		...
		// If we encounter an unknown witness program version and we aren't discouraging future unknown witness based
		// soft-forks, then we de-activate the segwit behavior within the VM for the remainder of execution.
		vm.witnessProgram = nil
	}
	if vm.isWitnessVersionActive(0) {
		// All elements within the witness stack must not be greater than the maximum bytes which are allowed to be
...
func (vm *Engine) DisasmPC() (out string, err error) {
	var scriptIdx, scriptOff int
	if scriptIdx, scriptOff, err = vm.curPC(); !slog.Check(err) {
		out = vm.disasm(scriptIdx, scriptOff)
	}
	return
}
...
func (vm *Engine) DisasmScript(idx int) (disStr string, err error) {
	if idx >= len(vm.scripts) {
		str := fmt.Sprintf("script index %d >= total scripts %d", idx,
			len(vm.scripts))
		err = scriptError(ErrInvalidIndex, str)
	} else {
...
func (vm *Engine) CheckErrorCondition(finalScript bool) (err error) {
	...
	// If we're in version zero witness execution mode, and this was the final script, then the stack MUST be clean in
	// order to maintain compatibility with BIP16.
	if finalScript && vm.isWitnessVersionActive(0) && vm.dstack.Depth() != 1 {
		return scriptError(ErrEvalFalse, "witness program must have clean stack")
	}
	...
	var v bool
	if v, err = vm.dstack.PopBool(); slog.Check(err) {
		return
	}
	if !v {
...
// enabled.
func (vm *Engine) checkHashTypeEncoding(hashType SigHashType) (err error) {
...
// enabled.
func (vm *Engine) checkPubKeyEncoding(pubKey []byte) (err error) {
...
// enabled.
func (vm *Engine) checkSignatureEncoding(sig []byte) (err error) {
	...
	//   - R is the arbitrary length big-endian encoded number which represents the R value of the signature.  DER
	//   encoding dictates that the value must be encoded using the minimum possible number of bytes.  This implies the
	//   first byte can only be null if the highest bit of the next byte is set in order to prevent it from being
	//   interpreted as a negative number.
	//   - 0x02 is once again the ASN.1 integer identifier
	//   - Length of S is 1 byte and specifies how many bytes S occupies
	//   - S is the arbitrary length big-endian encoded number which represents the S value of the signature.  The
	//   encoding rules are identical as those for R.
	const (
		asn1SequenceID = 0x30
		asn1IntegerID  = 0x02
	...
	// Calculate the offsets of the elements related to S and ensure S is inside the signature.
	// rLen specifies the length of the big-endian encoded number which represents the R value of the signature.
	// sTypeOffset is the offset of the ASN.1 identifier for S and, like its R counterpart, is expected to indicate an
	// ASN.1 integer.
	// sLenOffset and sOffset are the byte offsets within the signature of the length of S and S itself, respectively.
	...
	// sLen specifies the length of the big-endian encoded number which represents the S value of the signature.
	...
	// complement modulo the order can be used instead which is a shorter encoding by 1 byte.  Further, without
	// enforcing this, it is possible to replace a signature in a valid transaction with the complement while still
	// being a valid signature that verifies.  This would result in changing the transaction hash and thus is a source
	// of malleability.
...
func NewEngine(scriptPubKey []byte, tx *wire.MsgTx, txIdx int, flags ScriptFlags,
	sigCache *SigCache, hashCache *TxSigHashes, inputAmount int64) (vm *Engine, err error) {
	...
	// The clean stack flag (ScriptVerifyCleanStack) is not allowed without either the pay-to-script-hash (P2SH)
	// evaluation (ScriptBip16) flag or the Segregated Witness (ScriptVerifyWitness) flag.
	// Recall that evaluating a P2SH script without the flag set results in non-P2SH evaluation which leaves the P2SH
	// inputs on the stack. Thus, allowing the clean stack flag without the P2SH flag would make it possible to have a
	// situation where P2SH would not be a soft fork when it should be. The same goes for segwit which will pull in
	...
	// Check to see if we should execute in witness verification mode according to the set flags. We check both the
	// pkScript, and sigScript here since in the case of nested p2sh, the scriptSig will be a valid witness program. For
	// nested p2sh, all the bytes after the first data push should *exactly* match the witness program template.
	if vm.hasFlag(ScriptVerifyWitness) {
		...
		case len(tx.TxIn[txIdx].Witness) != 0 && vm.bip16:
			// The sigScript MUST be *exactly* a single canonical data push of the witness program, otherwise we
		...
		if witProgram != nil {
			...
			// If we didn't find a witness program in either the pkScript or as a data push within the sigScript, then
			// there MUST NOT be any witness data associated with the input being validated.
