...
// of the signature, and public key will be executed in order to ensure a
// complete match. In the occasion that two sigHashes collide, the newer
// sigHash will simply overwrite the existing entry.
...
// DoS attack wherein an attack causes a victim's client to hang due to
// worst-case behavior triggered while processing attacker crafted invalid
// transactions. A detailed description of the mitigated DoS attack can be
// found here: https://bitslog.wordpress.com/2013/01/23/fixed-bitcoin-vulnerability-explanation-why-the-signature-cache-is-a-dos-protection/.
...
// make room for new entries that would cause the number of entries in the
...
// the new entry. NOTE: This function is safe for concurrent access. Writers
// will block simultaneous readers until function execution has concluded.
func (s *SigCache) Add(sigHash chainhash.Hash, sig *ec.Signature, pubKey *ec.PublicKey) {
	...
	if uint(len(s.validSigs)+1) > s.maxEntries {
		// Remove a random entry from the map. Relying on the random starting
		// point of Go's map iteration. It's worth noting that the random
		// iteration starting point is not 100% guaranteed by the spec, however
		// most Go compilers support it. Ultimately, the iteration order isn't
		// important here because in order to manipulate which items are evicted,
		// an adversary would need to be able to execute preimage attacks on the
		// hashing function in order to start eviction at a specific entry.
