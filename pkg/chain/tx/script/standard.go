...
const (
	...
	// checks which are required for the script to be considered standard. These checks help reduce issues related to
	// transaction malleability as well as allow pay-to-script hash transactions. Note these flags are different than
	// what is required for the consensus rules in that they are more strict. TODO: This definition does not belong
	// here. It belongs in a policy package.
...
func isNullData(pops []parsedOpcode) bool {
	// A nulldata transaction is either a single OP_RETURN or an OP_RETURN SMALLDATA (where SMALLDATA is a data push up
	// to MaxDataCarrierSize bytes).
	l := len(pops)
...
// expectedInputs returns the number of arguments required by a script. If the script is of unknown type such that the
// number can not be determined then -1 is returned. We are an internal function and thus assume that class is the real
// class of pops (and we can thus assume things that were determined while finding out the type).
func expectedInputs(pops []parsedOpcode, class ScriptClass) int {
	...
	case MultiSigTy:
		...
		// small int. Also, due to the original bitcoind bug where OP_CHECKMULTISIG pops an additional item from the
		// stack, add an extra expected input for the extra push that is required to compensate.
...
type ScriptInfo struct {
	...
	NumInputs int
	// ExpectedInputs is the number of outputs required by the signature script and any pay-to-script-hash scripts. The
	// number will be -1 if unknown.
	ExpectedInputs int
...
// CalcMultiSigStats returns the number of public keys and signatures from a multi-signature transaction script. The
// passed script MUST already be known to be a multi-signature script.
func CalcMultiSigStats(script []byte) (int, int, error) {
	...
	//
	// Therefore the number of signatures is the oldest item on the stack and the number of pubkeys is the 2nd to last.
	// Also, the absolute minimum for a multi-signature script is 1 pubkey, so at least 4 items must be on the stack
	// per:
	//
...
// payToPubKeyHashScript creates a new script to pay a transaction output to a 20-byte pubkey hash. It is expected that
...
// payToScriptHashScript creates a new script to pay a transaction output to a script hash. It is expected that the
// input is a valid hash.
...
// PushedData returns an array of byte slices containing any pushed data found in the passed script. This includes OP_0,
// but not OP_1 - OP_16.
...
// ExtractPkScriptAddrs returns the type of script, addresses and required signatures associated with the passed
// PkScript. Note that it only works for 'standard' transaction script types. Any data such as public keys which are
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *netparams.Params) (ScriptClass, []util.Address, int, error) {
	...
	switch scriptClass {
	case PubKeyHashTy:
		// A pay-to-pubkey-hash script is of the form: OP_DUP OP_HASH160 <hash> OP_EQUALVERIFY OP_CHECKSIG Therefore the
		// pubkey hash is the 3rd item on the stack. Skip the pubkey hash if it's invalid for some reason.
		requiredSigs = 1
	...
	case MultiSigTy:
		...
		// public keys is the 2nd to last item on the stack.
...
//
// ExtractAtomicSwapDataPushes returns (nil, nil). Non-nil errors are returned for unparsable scripts. NOTE: Atomic
// swaps are not considered standard script types by the dcrd mempool policy and should be used with P2SH. The atomic
// swap format is also expected to change to use a more secure hash function in the future. This function is only
// defined in the txscript package due to API limitations which prevent callers using txscript to parse nonstandard
// scripts.
