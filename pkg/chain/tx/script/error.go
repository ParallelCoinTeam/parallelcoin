...
const (
	// ErrInternal is returned if internal consistency checks fail. In practice
	...
	// invalid combination.
	ErrInvalidFlags
	// ErrInvalidIndex is returned when an out-of-bounds index is passed to a
	// function.
	ErrInvalidIndex
	// ErrUnsupportedAddress is returned when a concrete type that implements a
	// util.Address is not a supported type.
	ErrUnsupportedAddress
	// ErrNotMultisigScript is returned from CalcMultiSigStats when the provided
	// script is not a multisig script.
	ErrNotMultisigScript
	// ErrTooManyRequiredSigs is returned from MultiSigScript when the specified
	// number of required signatures is larger than the number of provided public
	// keys.
	...
	// terminated with an empty top stack element.
	ErrEmptyStack
	// ErrEvalFalse is returned when the script evaluated without error but
	// terminated with a false top stack element.
	ErrEvalFalse
	// ErrScriptUnfinished is returned when CheckErrorCondition is called on a
	// script that has not finished executing.
	ErrScriptUnfinished
	// ErrInvalidProgramCounter is...
	ErrInvalidProgramCounter
	// ErrScriptDone is returned when an attempt to execute an opcode is made
	...
	ErrElementTooBig
	// ErrTooManyOperations is returned if a script has more than MaxOpsPerScript
	// opcodes that do not push data.
	ErrTooManyOperations
	// ErrStackOverflow is returned when stack and altstack combined depth is
	// over the limit.
	ErrStackOverflow
	// ErrInvalidPubKeyCount is returned when the number of public keys specified
	// for a multsig is either negative or greater than MaxPubKeysPerMultiSig.
	ErrInvalidPubKeyCount
	// ErrInvalidSignatureCount is returned when the number of signatures
	// specified for a multisig is either negative or greater than the number of
	// public keys.
	ErrInvalidSignatureCount
	// ErrNumberTooBig is returned when the argument for an opcode that expects
	// numeric input is larger than the expected maximum number of bytes. For the
	// most part, opcodes that deal with stack manipulation via offsets,
	// arithmetic, numeric comparison, and boolean logic are those that this
	...
	ErrVerify
	// ErrEqualVerify is returned when OP_EQUALVERIFY is encountered in a script
	// and the top item on the data stack does not evaluate to true.
	ErrEqualVerify
	// ErrNumEqualVerify is returned when OP_NUMEQUALVERIFY is encountered in a
	// script and the top item on the data stack does not evaluate to true.
	ErrNumEqualVerify
	// ErrCheckSigVerify is returned when OP_CHECKSIGVERIFY is encountered in a
	// script and the top item on the data stack does not evaluate to true.
	ErrCheckSigVerify
	...
	ErrReservedOpcode
	// ErrMalformedPush is returned when a data push opcode tries to push more
	// bytes than are left in the script.
	ErrMalformedPush
	// ErrInvalidStackOperation is returned when a stack operation is attempted
	// with a number that is invalid for the current stack size.
	ErrInvalidStackOperation
	// ErrUnbalancedConditional is returned when an OP_ELSE or OP_ENDIF is
	// encountered in a script without first having an OP_IF or OP_NOTIF or the
	...
	// required.
	ErrMinimalData
	// ErrInvalidSigHashType is returned when a signature hash type is not one of
	// the supported types.
	...
	// canonically-encoded DER signature does not specify the correct number of
	// remaining bytes for the R and S portions.
	ErrSigInvalidDataLen
	// ErrSigMissingSTypeID is returned a signature that should be a
	// canonically-encoded DER signature does not provide the ASN.1 type ID for
	// S.
	...
	// canonically-encoded DER signature does not specify the correct number of
	// bytes for the S portion.
	ErrSigInvalidSLen
	// ErrSigInvalidRIntID is returned when a signature that should be a
	// canonically-encoded DER signature does not have the expected ASN.1 integer
	// ID for R.
	ErrSigInvalidRIntID
	...
	// script contains any signatures whose S values are higher than the half
	// order.
	ErrSigHighS
	// ErrNotPushOnly is returned when a script that is required to only push
	// data to the stack performs other operations. A couple of cases where this
	// applies is for a pay-to-script-hash signature script when bip16 is active
	// and when the ScriptVerifySigPushOnly flag is set.
	ErrNotPushOnly
	// ErrSigNullDummy is returned when the ScriptStrictMultiSig flag is set and
	// a multisig script has anything other than 0 for the extra dummy argument.
	ErrSigNullDummy
	// ErrPubKeyType is returned when the ScriptVerifyStrictEncoding flag is set
	// and the script contains invalid public keys.
	ErrPubKeyType
	// ErrCleanStack is returned when the ScriptVerifyCleanStack flag is set, and
	// after evalution, the stack does not contain only a single element.
	ErrCleanStack
	// ErrNullFail is returned when the ScriptVerifyNullFail flag is set and
	// signatures are not empty on failed checksig or checkmultisig operations.
	ErrNullFail
	// ErrWitnessMalleated is returned if ScriptVerifyWitness is set and a native
...
// ScriptError identifies a script-related error. It is used to indicate three
// classes of errors:
//
