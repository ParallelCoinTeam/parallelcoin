...
const (
	...
	// level0MaxEntries is the maximum number of transactions that are stored in level 0 of an address index entry.  Subsequent levels store  2^n * level0MaxEntries entries, or in words, double the maximum of the previous level.
	level0MaxEntries = 8
	// addrKeySize is the number of bytes an address key consumes in the index.  It consists of 1 byte address type + 20 bytes hash160.
	addrKeySize = 1 + 20
	// levelKeySize is the number of bytes a level key in the address index consumes.  It consists of the address key + 1 byte for the level.
	levelKeySize = addrKeySize + 1
	// levelOffset is the offset in the level key which identifes the level.
	levelOffset = levelKeySize - 1
	// addrKeyTypePubKeyHash is the address type in an address key which represents both a pay-to-pubkey-hash and a pay-to-pubkey address. This is done because both are identical for the purposes of the
	// address index.
	addrKeyTypePubKeyHash = 0
	// addrKeyTypeScriptHash is the address type in an address key which represents a pay-to-script-hash address.  This is necessary because the hash of a pubkey address might be the same as that of a script
	// hash.
	addrKeyTypeScriptHash = 1
	// addrKeyTypePubKeyHash is the address type in an address key which represents a pay-to-witness-pubkey-hash address. This is required as the 20-byte data push of a p2wkh witness program may be the same data push used a p2pkh address.
	addrKeyTypeWitnessPubKeyHash = 2
	// addrKeyTypeScriptHash is the address type in an address key which represents a pay-to-witness-script-hash address. This is required, as p2wsh are distinct from p2sh addresses since they use a new script template, as well as a 32-byte data push.
	addrKeyTypeWitnessScriptHash = 3
	// Size of a transaction entry.  It consists of 4 bytes block id + 4 bytes offset + 4 bytes length.
	txEntrySize = 4 + 4 + 4
)

type AddressKey [addrKeySize]byte
...
func keyForLevel(addrKey AddressKey, level uint8) (key LevelKey) {
	copy(key[:], addrKey[:])
...
func dbPutAddrIndexEntry(bucket internalBucket, addrKey AddressKey, blockID uint32, txLoc wire.TxLoc) (err error) {
	...
	for {
		...
		if len(curLevelData) > 0 {
			mergedData = make([]byte, len(curLevelData)+len(prevLevelData))
...
func dbFetchAddrIndexEntries(bucket internalBucket, addrKey AddressKey, numToSkip, numRequested uint32,
...
func minEntriesToReachLevel(level uint8) (minRequired int) {
...
type AddrIndex struct {
	...
	// The following fields are used to quickly link transactions and addresses that have not been included into a block yet when an address index is being maintained.  The are protected by the unconfirmedLock field.
...
func (idx *AddrIndex) Create(dbTx database.Tx) (err error) {
...
func (idx *AddrIndex) indexPkScript(data writeIndexData, pkScript []byte, txIdx int) {
	...
	var err error
	_, addrs, _, err = script.ExtractPkScriptAddrs(pkScript, idx.chainParams)
	if err != nil || len(addrs) == 0 {
		return
	}
	for _, addr := range addrs {
		var addrKey AddressKey
...
func (idx *AddrIndex) indexBlock(data writeIndexData, block *util.Block, stxos []blockchain.SpentTxOut) {
	stxoIndex := 0
	for txIdx, tx := range block.Transactions() {
		...
		if txIdx != 0 {
			for range tx.MsgTx().TxIn {
				// We'll access the slice of all the transactions spent in this block properly ordered to fetch the previous input script.
...
func (idx *AddrIndex) ConnectBlock(dbTx database.Tx, block *util.Block, stxos []blockchain.SpentTxOut) (err error) {
	...
	var blockID uint32
	if blockID, err = dbFetchBlockIDByHash(dbTx, block.Hash()); slog.Check(err) {
		return
	}
	...
	for addrKey, txIdxs := range addrsToTxns {
		for _, txIdx := range txIdxs {
			if err = dbPutAddrIndexEntry(addrIdxBucket, addrKey, blockID, txLocs[txIdx]); slog.Check(err) {
				return
			}
		}
	}
...
func (idx *AddrIndex) DisconnectBlock(dbTx database.Tx, block *util.Block, stxos []blockchain.SpentTxOut) (err error) {
	// Build all of the address to transaction mappings in a local map.
	addrsToTxns := make(writeIndexData)
	idx.indexBlock(addrsToTxns, block, stxos)
	...
	for addrKey, txIdxs := range addrsToTxns {
		if err = dbRemoveAddrIndexEntries(bucket, addrKey, len(txIdxs)); slog.Check(err) {
			return
		}
	}
	return
...
// also returns the number actually skipped since it could be less in the case where there are not enough entries.
// NOTE: These results only include transactions confirmed in blocks.  See the UnconfirmedTxnsForAddress method for
// obtaining unconfirmed transactions that involve a given address. This function is safe for concurrent access.
func (idx *AddrIndex) TxRegionsForAddress(dbTx database.Tx, addr util.Address, numToSkip, numRequested uint32,
	reverse bool) (regions []database.BlockRegion, skipped uint32, err error) {
	var addrKey AddressKey
	if addrKey, err = addrToKey(addr); slog.Check(err) {
		return
	}
...
func (idx *AddrIndex) indexUnconfirmedAddresses(pkScript []byte, tx *util.Tx) {
	...
	_, addresses, _, _ := script.ExtractPkScriptAddrs(pkScript,
		idx.chainParams)
	var err error
...
// UnconfirmedTxnsForAddress returns all transactions currently in the unconfirmed (memory-only) address index that involve the passed address. Unsupported address types are ignored and will result in no results. This function is safe for concurrent access.
