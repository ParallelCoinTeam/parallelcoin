...
var (
	// txIndexKey is the key of the transaction index and the db bucket used to house it.
	txIndexKey = []byte("txbyhashidx")
	// idByHashIndexBucketName is the name of the db bucket used to house the block id -> block hash index.
	idByHashIndexBucketName = []byte("idbyhashidx")
	// hashByIDIndexBucketName is the name of the db bucket used to house the block hash -> block id index.
	hashByIDIndexBucketName = []byte("hashbyididx")
	// errNoBlockIDEntry is an error that indicates a requested entry does not exist in the block ID index.
	errNoBlockIDEntry = errors.New("no entry in the block ID index")
)

// The transaction index consists of an entry for every transaction in the main chain.  In order to significantly optimize the space requirements a separate index which provides an internal mapping between each block that has been indexed and a unique ID for use within the hash to location mappings.  The ID is simply a sequentially incremented uint32.  This is useful because it is only 4 bytes versus 32 bytes hashes and thus saves a ton of space in the index.
// There are three buckets used in total.  The first bucket maps the hash of each transaction to the specific block location.  The second bucket maps the hash of each block to the unique ID and the third maps that ID back to the block hash.
// NOTE: Although it is technically possible for multiple transactions to have the same hash as long as the previous transaction with the same hash is fully spent, this code only stores the most recent one because doing otherwise would add a non-trivial amount of space and overhead for something that will realistically never happen per the probability and even if it did, the old one must be fully spent and so the most likely transaction a caller would want for a given hash is the most recent one anyways.
...
//   -----
//   Total: 44 bytes
// dbPutBlockIDIndexEntry uses an existing database transaction to update or add the index entries for the hash to id and id to hash mappings for the provided values.
...
// dbRemoveBlockIDIndexEntry uses an existing database transaction remove index entries from the hash to id and id to hash mappings for the provided hash.
...
// dbPutTxIndexEntry uses an existing database transaction to update the transaction index given the provided serialized data that is expected to have been serialized putTxIndexEntry.
...
// dbAddTxIndexEntries uses an existing database transaction to add a transaction index entry for every transaction in the passed block.
...
// dbRemoveTxIndexEntry uses an existing database transaction to remove the most recent transaction index entry for the given hash.
...
// dbRemoveTxIndexEntries uses an existing database transaction to remove the latest transaction entry for every transaction in the passed block.
...
func (idx *TxIndex) Init() error {
	// Find the latest known block id field for the internal block id index and initialize it.  This is done because it's a lot more efficient to do a single search at initialize time than it is to write another value to the database on every update.
	err := idx.db.View(func(dbTx database.Tx) error {
		// Scan forward in large gaps to find a block id that doesn't exist yet to serve as an upper bound for the binary search below.
	...
	log <- cl.Trace{"current internal block ID:", idx.curBlockID, cl.Ine()}
	return nil
}
