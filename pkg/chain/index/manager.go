...
func (m *Manager) Init(chain *blockchain.BlockChain, interrupt <-chan struct{}) error {
	...
	L.Info("indexes caught up to height", bestHeight)
	return nil
}
...
func dropIndex(db database.DB, idxKey []byte, idxName string, interrupt <-chan struct{}) error {
	...
	if !needsDelete {
		L.Warnf("not dropping %s because it does not exist", idxName)
		return nil
	}
	...
	// complete.
	L.Infof("dropping all %s entries.  This might take a while...", idxName)
	...
	for i := range subBuckets {
		...
		for numDeleted := maxDeletions; numDeleted == maxDeletions; {
			...
			if numDeleted > 0 {
				...
				L.Infof("deleted %d keys (%d total) from %s", numDeleted,
