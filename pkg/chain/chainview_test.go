...
func TestChainView(	t *testing.T) {
	...
	tests := []struct {
		...
		unequal *chainView
		// expected locator for active view tip
...
testLoop:
	for _, test := range tests {
		...
		if test.side.Genesis() != test.genesis {
			t.Errorf("%s: unexpected side view genesis -- got %v, want %v", test.name, test.view.Genesis(), test.genesis)
			continue
		}
		...
		// Ensure all nodes contained in the view can be retrieved by height.
