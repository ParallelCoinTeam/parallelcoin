...
type (
	...
	// Start() in a goroutine. Once started, it selects peers to sync from and starts the initial block download. Once
	// the chain is in sync, the SyncManager handles incoming block and header notifications and relays announcements of
	// new blocks to peers.
	SyncManager struct {
	...
	// isCurrentMsg is a message type to be sent across the message channel for requesting whether or not the sync
	// manager believes it is synced with the currently connected peers.
	isCurrentMsg struct {
		reply chan bool
	}
	...
	// processBlockMsg is a message type to be sent across the message channel for requested a block is processed. Note
	// this call differs from blockMsg above in that blockMsg is intended for blocks that came from peers and have extra
	// handling whereas this message essentially is just a concurrent safe way to call ProcessBlock on the internal
	...
	// txMsg packages a bitcoin tx message and the peer it came from together so the block handler has access to that
	// information.
	txMsg struct {
		tx    *util.Tx
		peer  *peerpkg.Peer
...
const (
	// minInFlightBlocks is the minimum number of blocks that should be in the request queue for headers-first mode
	// before requesting more.
	minInFlightBlocks = 10
	// maxRejectedTxns is the maximum number of rejected transactions hashes to store in memory.
	maxRejectedTxns = 1000
	// maxRequestedBlocks is the maximum number of requested block hashes to store in memory.
...
// blocks are needed and how the fetching should proceed.
...
func (sm *SyncManager) current() bool {
	...
	// No matter what chain thinks, if we are below the block we are syncing to we are not current.
...
// fetchHeaderBlocks creates and sends a request to the syncPeer for the next list of blocks to be downloaded based on
// the current list of headers.
func (sm *SyncManager) fetchHeaderBlocks() {
	// Nothing to do if there is no start header.
	if sm.startHeader == nil {
		Debug("fetchHeaderBlocks called with no start header")
		return
	}
	// Build up a getdata request for the list of blocks the headers describe. The size hint will be limited to
	// wire.MaxInvPerMsg by the function, so no need to double check it here.
	gdmsg := wire.NewMsgGetDataSizeHint(uint(sm.headerList.Len()))
	numRequested := 0
	for e := sm.startHeader; e != nil; e = e.Next() {
		node, ok := e.Value.(*headerNode)
		if !ok {
			Debug("header list node type is not a headerNode")
			continue
		}
		...
		if !haveInv {
			...
			// If we're fetching from a witness enabled peer post-fork, then ensure that we receive all the witness data
			// in the blocks.
...
// findNextHeaderCheckpoint returns the next checkpoint after the passed height. It returns nil when there is not one
// either because the height is already later than the final checkpoint or some other reason such as disabled
// checkpoints.
...
func (sm *SyncManager) handleBlockMsg(workerNumber uint32, bmsg *blockMsg) {
	...
	if _, exists = state.requestedBlocks[*blockHash]; !exists {
		...
		// actually fed the duplicate blocks.
	...
	// When in headers-first mode, if the block matches the hash of the first header in the list of headers that are
	// being fetched, it's eligible for less validation since the headers have already been verified to link together
	// and are valid up to the next checkpoint. Also, remove the list entry for all blocks except the checkpoint since
	// it is needed to verify the next round of headers links properly.
	...
	// Remove block from request maps. Either chain will know about it and so we shouldn't have any more instances of
	// trying to fetch it, or we will fail the insert and thus we'll retry next time we get an inv.
	...
	if err != nil {
		Error(err)
		// When the error is a rule error, it means the block was simply rejected as opposed to something actually going
		// wrong, so log it as such. Otherwise, something really did go wrong, so log it as an actual error.
		if _, ok := err.(blockchain.RuleError); ok {
			Errorf(
				"rejected block %v from %s: %v",
				blockHash, pp, err,
			)
			// Infof("height %d", bmsg.block.Height())
		} else {
			Errorf("failed to process block %v: %v", blockHash, err)
		}
	...
	// Meta-data about the new block this peer is reporting. We use this below to update this peer's lastest block
	// height and the heights of other peers based on their last announced block hash. This allows us to dynamically
	// update the block heights of peers, avoiding stale heights when looking for a new sync peer. Upon acceptance of a
	// block or recognition of an orphan, we also use this information to update the block heights over other peers
	// who's invs may have been ignored if we are actively syncing while the chain is not yet current or who may have
	// lost the lock announcment race. Request the parents for the orphan block from the peer that sent it.
	if isOrphan {
		// We've just received an orphan block from a peer. In order to update the height of the peer, we try to extract
		// the block height from the scriptSig of the coinbase transaction. Extraction is only attempted if the block's
		// version is high enough (ver 2+).
		header := &bmsg.block.MsgBlock().Header
		if blockchain.ShouldHaveSerializedBlockHeight(header) {
			coinbaseTx := bmsg.block.Transactions()[0]
			cbHeight, err := blockchain.ExtractCoinbaseHeight(coinbaseTx)
			if err != nil {
				Errorf("unable to extract height from coinbase tx: %v", err)
			} else {
				Debugf("extracted height of %v from orphan block",
					cbHeight)
				heightUpdate = cbHeight
				blkHashUpdate = blockHash
			}
		...
		if err != nil {
			Errorf("failed to get block locator for the latest block: %v",
				err)
		} else {
	...
	// Update the block height for this peer. But only send a message to the server for updating peer heights if this is
	// an orphan or our chain is "current". This avoids sending a spammy amount of messages if we're syncing the chain
	// from scratch.
	...
	// This is headers-first mode, so if the block is not a checkpoint request more blocks using the header list when
	// the request queue is getting short.
	...
	// This is headers-first mode and the block is a checkpoint. When there is a next checkpoint, get the next round of
	// headers by asking for headers starting from the block after this one up to the next checkpoint.
	...
	// This is headers-first mode, the block is a checkpoint, and there are no more checkpoints, so switch to normal
	// mode by requesting blocks from the block after this one up to the end of the chain (zero hash).
...
// handleBlockchainNotification handles notifications from blockchain. It does things such as request orphan block
// parents and relay accepted blocks to connected peers.
func (sm *SyncManager) handleBlockchainNotification(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockAccepted:
		...
		if !ok {
			Debug("chain accepted notification is not a block")
			break
		}
	...
	case blockchain.NTBlockConnected:
		block, ok := notification.Data.(*util.Block)
		if !ok {
			Debug("chain connected notification is not a block")
			break
		}
		// Remove all of the transactions (except the coinbase) in the connected block from the transaction pool.
		// Secondly, remove any transactions which are now double spends as a result of these new transactions. Finally,
	...
	case blockchain.NTBlockDisconnected:
		block, ok := notification.Data.(*util.Block)
		if !ok {
			Debug("chain disconnected notification is not a block.")
			break
		}
		...
		for _, tx := range block.Transactions()[1:] {
			...
			if err != nil {
				Error(err)
				// Remove the transaction and all transactions that depend on it if it wasn't accepted into the
				// transaction pool.
...
func (sm *SyncManager) handleDonePeerMsg(peer *peerpkg.Peer) {
	...
	if !exists {
		Trace("received done peer message for unknown peer", peer)
		return
	}
	// Remove the peer from the list of candidate peers.
	delete(sm.peerStates, peer)
	Trace("lost peer ", peer)
	// Remove requested transactions from the global map so that they will be fetched from elsewhere next time we get an
	// inv.
	for txHash := range state.requestedTxns {
		delete(sm.requestedTxns, txHash)
	}
	// Remove requested blocks from the global map so that they will be fetched from elsewhere next time we get an inv.
	// TODO: we could possibly here check which peers have these blocks and request them now to speed things up a
	//  little.
	for blockHash := range state.requestedBlocks {
		delete(sm.requestedBlocks, blockHash)
	}
	// Attempt to find a new peer to sync from if the quitting peer is the sync peer. Also, reset the headers-first
	// state if in headers-first mode so
	if sm.syncPeer == peer {
		sm.syncPeer = nil
		if sm.headersFirstMode {
			best := sm.chain.BestSnapshot()
...
// handleHeadersMsg handles block header messages from all peers. Headers are requested when performing a headers-first
// sync.
func (sm *SyncManager) handleHeadersMsg(hmsg *headersMsg) {
	peer := hmsg.peer
	_, exists := sm.peerStates[peer]
	if !exists {
		Trace("received headers message from unknown peer", peer)
		return
	}
	...
	for _, blockHeader := range msg.Headers {
		...
		// Ensure the header properly connects to the previous one and add it to the list of headers.
	...
	if receivedCheckpoint {
		// Since the first entry of the list is always the final block that is already in the database and is only used
		// to ensure the next header links properly, it must be removed before fetching the blocks.
	...
	// This header is not a checkpoint, so request the next batch of headers starting from the latest known header and
	// ending with the next checkpoint.
	locator := blockchain.BlockLocator([]*chainhash.Hash{finalHash})
	err := peer.PushGetHeadersMsg(locator, sm.nextCheckpoint.Hash)
	if err != nil {
		Error(err)
		Errorf("failed to send getheaders message to peer %s: %v", peer,
			err)
		return
	}
}
...
func (sm *SyncManager) handleInvMsg(imsg *invMsg) {
	...
	state, exists := sm.peerStates[peer]
	if !exists {
		Trace("received inv message from unknown peer", peer)
		return
	}
	...
	// Request the advertised inventory if we don't already have it. Also, request parent blocks of orphans if we
	// receive one we already have. Finally, attempt to detect potential stalls due to long side chains we already have
	// and request more blocks to prevent them.
	for i, iv := range invVects {
		...
		haveInv, err := sm.haveInventory(iv)
		if err != nil {
			Error("unexpected failure when checking for existing inventory during inv message processing:", err)
			continue
		}
		if !haveInv {
		...
		if iv.Type == wire.InvTypeBlock {
			// The block is an orphan block that we already have. When the existing orphan was processed, it requested
			// the missing parent blocks. When this scenario happens, it means there were more blocks missing than are
			// allowed into a single inventory message. As a result, once this peer requested the final advertised
			// block, the remote peer noticed and is now resending the orphan block as an available block to signal
			// there are more missing blocks that need to be requested.
			if sm.chain.IsKnownOrphan(&iv.Hash) {
				...
				locator, err := sm.chain.LatestBlockLocator()
				if err != nil {
					Error("failed to get block locator for the latest block:", err)
					continue
				}
	...
	// Request as much as possible at once. Anything that won't fit into request will be requested on the next inv
	// message.
	numRequested := 0
...
func (sm *SyncManager) handleNewPeerMsg(peer *peerpkg.Peer) {
	...
	Tracef("new valid peer %s (%s)", peer, peer.UserAgent())
...
func (sm *SyncManager) handleTxMsg(tmsg *txMsg) {
	...
	// NOTE: BitcoinJ, and possibly other wallets, don't follow the spec of sending an inventory message and allowing
	// the remote peer to decide whether or not they want to request the transaction via a getdata message.
	// Unfortunately, the reference implementation permits unrequested data, so it has allowed wallets that don't follow
	// the spec to proliferate. While this is not ideal, there is no check here to disconnect peers for sending
	// unsolicited transactions to provide interoperability.
	txHash := tmsg.tx.Hash()
	// Ignore transactions that we have already rejected. Do not send a reject message here because if the transaction
	// was already rejected, the transaction was unsolicited.
	...
	acceptedTxs, err := sm.txMemPool.ProcessTransaction(sm.chain, tmsg.tx,
		true, true, mempool.Tag(peer.ID()))
	// Remove transaction from request maps. Either the mempool/chain already knows about it and as such we shouldn't
	// have any more instances of trying to fetch it, or we failed to insert and thus we'll retry next time we get an
	// inv.
	...
	if err != nil {
		...
		// going wrong, so log it as such. Otherwise, something really did go wrong, so log it as an actual error.
...
// checking all of the various places inventory can be when it is in different states such as blocks that are part of
// the main chain, on a side chain, in the orphan pool, and transactions that are in the memory pool (either the main
// pool or orphan pool).
func (sm *SyncManager) haveInventory(invVect *wire.InvVect) (bool, error) {
	...
	case wire.InvTypeWitnessBlock:
		fallthrough
	case wire.InvTypeBlock:
		// Ask chain if the block is known to it in any form (main chain, side chain, or orphan).
		return sm.chain.HaveBlock(&invVect.Hash)
	case wire.InvTypeWitnessTx:
		fallthrough
	case wire.InvTypeTx:
		// Ask the transaction memory pool if the transaction is known to it in any form (main pool or orphan).
		if sm.txMemPool.HaveTransaction(&invVect.Hash) {
			return true, nil
		}
		// Check if the transaction exists from the point of view of the end of the main chain. Note that this is only a
		// best effort since it is expensive to check existence of every output and the only purpose of this check is to
		// avoid downloading already known transactions. Only the first two outputs are checked because the vast
		// majority of transactions consist of two outputs where one is some form of "pay-to-somebody-else" and the
		// other is a change output.
	...
	// The requested inventory is is an unsupported type, so just claim it is known to avoid requesting it.
	return true, nil
}

// isSyncCandidate returns whether or not the peer is a candidate to consider syncing from.
func (sm *SyncManager) isSyncCandidate(peer *peerpkg.Peer) bool {
	// Typically a peer is not a candidate for sync if it's not a full node, however regression test is special in that
	// the regression tool is not a full node and still needs to be considered a sync candidate.
	if sm.chainParams == &netparams.RegressionTestParams {
		// The peer is not a candidate if it's not coming from localhost or the hostname can't be determined for some
		// reason.
...
// limitMap is a helper function for maps that require a maximum limit by evicting a random transaction if adding a new
// value would cause it to overflow the maximum allowed.
func (sm *SyncManager) limitMap(m map[chainhash.Hash]struct{}, limit int) {
	if len(m)+1 > limit {
		// Remove a random entry from the map. For most compilers, Go's range statement iterates starting at a random
		// item although that is not 100% guaranteed by the spec. The iteration order is not important here because an
		// adversary would have to be able to pull off preimage attacks on the hashing function in order to target
		// eviction of specific entries anyways.
		for txHash := range m {
			delete(m, txHash)
			return
		}
...
func (sm *SyncManager) resetHeaderState(newestHash *chainhash.Hash, newestHeight int32) {
	...
	sm.headerList.Init()
	sm.startHeader = nil
	// When there is a next checkpoint, add an entry for the latest known block into the header pool. This allows the
	// next downloaded header to prove it links to the chain properly.
	if sm.nextCheckpoint != nil {
...
// startSync will choose the best peer among the available candidate peers to download/sync the blockchain from. When
// syncing is already running, it simply returns. It also examines the candidates for any which are no longer candidates
...
func (sm *SyncManager) startSync() {
	...
	for peer, state := range sm.peerStates {
		...
		if segwitActive && !peer.IsWitnessEnabled() {
			Debug("peer", peer, "not witness enabled, skipping")
			continue
		}
	...
	if bestPeer != nil {
		...
		// When the current height is less than a known checkpoint we can use block headers to learn about which blocks
		// comprise the chain up to the checkpoint and perform less validation for them. This is possible since each
		// header contains the hash of the previous header and a merkle root.
		//
		// Therefore if we validate all of the received headers link together properly and the checkpoint hashes match,
		// we can be sure the hashes for the blocks in between are accurate. Further, once the full blocks are
		// downloaded, the merkle root is computed and compared against the value in the header which proves the full
		// block hasn't been tampered with. Once we have passed the final checkpoint, or checkpoints are disabled, use
		// standard inv messages learn about the blocks and fully validate them. Finally, regression test mode does not
		// support the headers-first approach so do normal block downloads when in regression test mode.
		if sm.nextCheckpoint != nil &&
