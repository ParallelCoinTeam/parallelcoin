...
type (
	...
	// SyncManager is started as by executing Start() in a goroutine. Once
	// started, it selects peers to sync from and starts the initial block
	// download. Once the chain is in sync, the SyncManager handles incoming
	// block and header notifications and relays announcements of new blocks to
	// peers.
	SyncManager struct {
	...
	// getSyncPeerMsg is a message type to be sent across the message channel
	// for retrieving the current sync peer.
	getSyncPeerMsg struct {
		reply chan int32
	}
	// headerNode is used as a node in a list of headers that are linked
	// together between checkpoints.
	headerNode struct {
		height int32
		hash   *chainhash.Hash
	}
	...
	// above in that blockMsg is intended for blocks that came from peers and
	// have extra handling whereas this message essentially is just a concurrent
	// safe way to call ProcessBlock on the internal block chain instance.
	...
	// txMsg packages a bitcoin tx message and the peer it came from together so
...
const (
	...
	// maxRequestedBlocks is the maximum number of requested block hashes to
	// store in memory.
	maxRequestedBlocks = wire.MaxInvPerMsg
	// maxRequestedTxns is the maximum number of requested transactions hashes
	// to store in memory.
...
// pause pauses the sync manager until the returned channel is closed. Note
// that while paused, all peer and block processing is halted.  The message
...
func (sm *SyncManager) ProcessBlock(block *util.Block, flags blockchain.BehaviorFlags) (bool, error) {
	log.L.Trace("processing block")
	// log.L.Traces(block)
	reply := make(chan processBlockResponse, 1)
	//log.L.Trace("sending to msgChan")
	sm.msgChan <- processBlockMsg{block: block, flags: flags, reply: reply}
	//log.L.Trace("waiting on reply")
	response := <-reply
...
func (sm *SyncManager) Start() {
	...
	log.L.Trace("starting sync manager")
	sm.wg.Add(1)
...
func (sm *SyncManager) Stop() error {
	if atomic.AddInt32(&sm.shutdown, 1) != 1 {
		log.L.Warn("sync manager is already in the process of shutting down")
		return nil
	}
		...
		case m := <-sm.msgChan:
			...
			case processBlockMsg:
				...
				if blockchain.ShouldHaveSerializedBlockHeight(header) {
					...
					if err != nil {
						log.L.Error(err)
						log.L.Trace("unable to extract height from coinbase tx:",
							err)
					} else {
						heightUpdate = cbHeight
					}
				}
				//log.L.Trace("passing to chain.ProcessBlock")
				_, isOrphan, err := sm.chain.ProcessBlock(workerNumber, msg.
					block, msg.flags, heightUpdate)
				if err != nil {
					log.L.Error("error processing new block ", err)
					msg.reply <- processBlockResponse{
						isOrphan: false,
						err:      err,
					}
				}
				//log.L.Trace("sending back message on reply channel")
				msg.reply <- processBlockResponse{
					isOrphan: isOrphan,
					err:      nil,
				}
			...
			case pauseMsg:
				...
				<-msg.unpause
			default:
				log.L.Tracef("invalid message type in block handler: %T", msg)
			}
		case <-sm.quit:
			break out
		}
...
func (sm *SyncManager) current() bool {
	...
	// if blockChain thinks we are current and we have no syncPeer it is
	// probably right.
	if sm.syncPeer == nil {
		return true
	}
	// No matter what chain thinks, if we are below the block we are syncing to
	// we are not current.
...
func (sm *SyncManager) fetchHeaderBlocks() {
	...
	if sm.startHeader == nil {
		log.L.Warn("fetchHeaderBlocks called with no start header")
		return
	}
	// Build up a getdata request for the list of blocks the headers describe.
	// The size hint will be limited to wire.MaxInvPerMsg by the function, so no
	// need to double check it here.
	gdmsg := wire.NewMsgGetDataSizeHint(uint(sm.headerList.Len()))
	numRequested := 0
	for e := sm.startHeader; e != nil; e = e.Next() {
		node, ok := e.Value.(*headerNode)
		if !ok {
			log.L.Warn("header list node type is not a headerNode")
			continue
		}
		...
		if !haveInv {
			...
			// If we're fetching from a witness enabled peer post-fork, then
			// ensure that we receive all the witness data in the blocks.
...
// already later than the final checkpoint or some other reason such as
...
func (sm *SyncManager) handleBlockMsg(workerNumber uint32, bmsg *blockMsg) {
	...
	if _, exists = state.requestedBlocks[*blockHash]; !exists {
		// The regression test intentionally sends some blocks twice to test
		// duplicate block insertion fails.  Don't disconnect the peer or ignore
		// the block when we're in regression test mode in this case so the chain
		// code is actually fed the duplicate blocks.
	...
	// When in headers-first mode, if the block matches the hash of the first
	// header in the list of headers that are being fetched, it's eligible for
	// less validation since the headers have already been verified to link
	// together and are valid up to the next checkpoint. Also, remove the list
	// entry for all blocks except the checkpoint since it is needed to verify
	// the next round of headers links properly.
	...
	// shouldn't have any more instances of trying to fetch it, or we will fail
	// the insert and thus we'll retry next time we get an inv.
	...
	if err != nil {
		...
		if _, ok := err.(blockchain.RuleError); ok {
			...
			// log.L.Infof("height %d", bmsg.block.Height())
		} else {
			log.L.Errorf("failed to process block %v: %v", blockHash, err)
		}
	...
	if isOrphan {
		...
		// the block's version is high enough (ver 2+).
		header := &bmsg.block.MsgBlock().Header
		if blockchain.ShouldHaveSerializedBlockHeight(header) {
			coinbaseTx := bmsg.block.Transactions()[0]
			cbHeight, err := blockchain.ExtractCoinbaseHeight(coinbaseTx)
			if err != nil {
				log.L.Errorf("unable to extract height from coinbase tx: %v", err)
			} else {
				log.L.Debugf("extracted height of %v from orphan block",
					cbHeight)
				heightUpdate = cbHeight
				blkHashUpdate = blockHash
			}
		...
		if err != nil {
			log.L.Errorf("failed to get block locator for the latest block: %v",
				err)
		} else {
		...
		// When the block is not an orphan, log information about it and update
		// the chain state.
	...
	// server for updating peer heights if this is an orphan or our chain is
	// "current". This avoids sending a spammy amount of messages if we're
	// syncing the chain from scratch.
	...
	// This is headers-first mode, so if the block is not a checkpoint request
	// more blocks using the header list when the request queue is getting short.
	...
	// This is headers-first mode and the block is a checkpoint.  When there is
	...
	// This is headers-first mode, the block is a checkpoint, and there are no
	// more checkpoints, so switch to normal mode by requesting blocks from the
	// block after this one up to the end of the chain (zero hash).
...
func (sm *SyncManager) handleBlockchainNotification(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockConnected:
		block, ok := notification.Data.(*util.Block)
		if !ok {
			log.L.Warn("chain connected notification is not a block")
			break
		}
		// Remove all of the transactions (except the coinbase) in the connected
		// block from the transaction pool.  Secondly, remove any transactions
		// which are now double spends as a result of these new transactions.
	...
	case blockchain.NTBlockDisconnected:
		...
		block, ok := notification.Data.(*util.Block)
		if !ok {
			log.L.Warn("chain disconnected notification is not a block.")
			break
		}
		// Reinsert all of the transactions (except the coinbase) into the
		// transaction pool.
		for _, tx := range block.Transactions()[1:] {
...
func (sm *SyncManager) handleDonePeerMsg(peer *peerpkg.Peer) {
	...
	if !exists {
		log.L.Trace("received done peer message for unknown peer", peer)
		return
	}
	// Remove the peer from the list of candidate peers.
	delete(sm.peerStates, peer)
	log.L.Trace("lost peer ", peer)
	// Remove requested transactions from the global map so that they will be
	// fetched from elsewhere next time we get an inv.
	for txHash := range state.requestedTxns {
		delete(sm.requestedTxns, txHash)
	}
	// Remove requested blocks from the global map so that they will be fetched
	// from elsewhere next time we get an inv.
	// TODO: we could possibly here check which peers have these blocks and
	// request them now to speed things up a little.
...
func (sm *SyncManager) handleHeadersMsg(hmsg *headersMsg) {
	...
	if !exists {
		log.L.Trace("received headers message from unknown peer", peer)
		return
	}
	...
	for _, blockHeader := range msg.Headers {
		...
		// Ensure the header properly connects to the previous one and add it to
		// the list of headers.
	...
	// When this header is a checkpoint, switch to fetching the blocks for all
	// of the headers since the last checkpoint.
	if receivedCheckpoint {
		// Since the first entry of the list is always the final block that is
		// already in the database and is only used to ensure the next header
		// links properly, it must be removed before fetching the blocks.
	...
	// This header is not a checkpoint, so request the next batch of headers starting from the latest known header and ending with the next checkpoint.
	locator := blockchain.BlockLocator([]*chainhash.Hash{finalHash})
	err := peer.PushGetHeadersMsg(locator, sm.nextCheckpoint.Hash)
	if err != nil {
		log.L.Error(err)
		log.L.Errorf("failed to send getheaders message to peer %s: %v", peer,
			err)
		return
	}
}
...
func (sm *SyncManager) handleInvMsg(imsg *invMsg) {
	...
	state, exists := sm.peerStates[peer]
	if !exists {
		log.L.Trace("received inv message from unknown peer", peer)
		return
	}
	...
	// current sync peer or we're current, then update the last announced block
	// for this peer. We'll use this information later to update the heights of
	// peers based on blocks we've accepted that they previously announced.
	if lastBlock != -1 && (peer != sm.syncPeer || sm.current()) {
		peer.UpdateLastAnnouncedBlock(&invVects[lastBlock].Hash)
	}
	// Ignore invs from peers that aren't the sync if we are not current. Helps
	// prevent fetching a mass of orphans.
	if peer != sm.syncPeer && !sm.current() {
		return
	}
	// If our chain is current and a peer announces a block we already know of,
	// then update their current block height.
	...
	// Finally, attempt to detect potential stalls due to long side chains we
	// already have and request more blocks to prevent them.
	for i, iv := range invVects {
		...
		if iv.Type == wire.InvTypeBlock {
			// The block is an orphan block that we already have. When the
			// existing orphan was processed, it requested the missing parent
			// blocks.  When this scenario happens, it means there were more
			// blocks missing than are allowed into a single inventory message.
			// As a result, once this peer requested the final advertised block,
			// the remote peer noticed and is now resending the orphan block as an
			// available block to signal there are more missing blocks that need
			// to be requested.
			if sm.chain.IsKnownOrphan(&iv.Hash) {
				// Request blocks starting at the latest known up to the root of
				// the orphan that just came in.
			...
			// We already have the final block advertised by this inventory
			// message, so force a request for more.  This should only happen if
			// we're on a really long side chain.
			if i == lastBlock {
				// Request blocks after this one up to the final one the remote
				// peer knows about (zero stop hash).
	...
	// request will be requested on the next inv message.
	numRequested := 0
...
func (sm *SyncManager) handleNewPeerMsg(peer *peerpkg.Peer) {
	...
	log.L.Tracef("new valid peer %s (%s)", peer, peer.UserAgent())
...
func (sm *SyncManager) handleTxMsg(tmsg *txMsg) {
	...
	// message.  Unfortunately, the reference implementation permits unrequested
	// data, so it has allowed wallets that don't follow the spec to
	// proliferate.  While this is not ideal, there is no check here to
	// disconnect peers for sending unsolicited transactions to provide
	// interoperability.
	txHash := tmsg.tx.Hash()
	...
	// Remove transaction from request maps. Either the mempool/chain already
	// knows about it and as such we shouldn't have any more instances of trying
	// to fetch it, or we failed to insert and thus we'll retry next time we get
	// an inv.
...
func (sm *SyncManager) haveInventory(invVect *wire.InvVect) (bool, error) {
	...
	case wire.InvTypeTx:
		...
		// Check if the transaction exists from the point of view of the end of
		// the main chain.  Note that this is only a best effort since it is
		// expensive to check existence of every output and the only purpose of
		// this check is to avoid downloading already known transactions.
		// Only the first two outputs are checked because the vast majority of
		// transactions consist of two outputs where one is some form of
		// "pay-to-somebody-else" and the other is a change output.
	...
	// The requested inventory is is an unsupported type, so just claim it is
	// known to avoid requesting it.
	return true, nil
}
...
func (sm *SyncManager) isSyncCandidate(peer *peerpkg.Peer) bool {
	...
	// however regression test is special in that the regression tool is not a
	// full node and still needs to be considered a sync candidate.
	if sm.chainParams == &netparams.RegressionTestParams {
		// The peer is not a candidate if it's not coming from localhost or the
		...
		// Additionally, if the segwit soft-fork package has activated, then the
...
func // limitMap is a helper function for maps that require a maximum limit by
// evicting a random transaction if adding a new value would cause it to
// overflow the maximum allowed.
(sm *SyncManager) limitMap(m map[chainhash.Hash]struct{}, limit int) {
	if len(m)+1 > limit {
		// Remove a random entry from the map.  For most compilers, Go's range
		// statement iterates starting at a random item although that is not 100%
		// guaranteed by the spec.  The iteration order is not important here
...
func (sm *SyncManager) resetHeaderState(newestHash *chainhash.Hash, newestHeight int32) {
	...
	sm.headerList.Init()
	sm.startHeader = nil
	// When there is a next checkpoint, add an entry for the latest known block
	// into the header pool.  This allows the next downloaded header to prove it
	// links to the chain properly.
	if sm.nextCheckpoint != nil {
...
func (sm *SyncManager) startSync() {
	...
	// Once the segwit soft-fork package has activated, we only want to sync
	// from peers which are witness enabled to ensure that we fully validate all
	// blockchain data.
	segwitActive, err := sm.chain.IsDeploymentActive(chaincfg.DeploymentSegwit)
	if err != nil {
		log.L.Error("unable to query for segwit soft-fork state:", err)
	...
	for peer, state := range sm.peerStates {
		...
		if segwitActive && !peer.IsWitnessEnabled() {
			log.L.Debug("peer", peer, "not witness enabled, skipping")
			continue
		}
		...
		// TODO(davec): Use a better algorithm to choose the best peer. For now,
		// just pick the first available candidate.
		bestPeer = peer
	}
	// Start syncing from the best peer if one was selected.
	if bestPeer != nil {
		// Clear the requestedBlocks if the sync peer changes, otherwise we may
		// ignore blocks we need that the last sync peer failed to send.
		...
		// block headers to learn about which blocks comprise the chain up to the
		// checkpoint and perform less validation for them.  This is possible
		// since each header contains the hash of the previous header and a
		// merkle root.
		// Therefore if we validate all of the received headers link together
		// properly and the checkpoint hashes match, we can be sure the hashes
		// for the blocks in between are accurate.  Further, once the full blocks
		// are downloaded, the merkle root is computed and compared against the
		// value in the header which proves the full block hasn't been tampered
		// with.
		// Once we have passed the final checkpoint, or checkpoints are disabled,
		// use standard inv messages learn about the blocks and fully validate
		// them.  Finally, regression test mode does not support the
		// headers-first approach so do normal block downloads when in regression
		// test mode.
		if sm.nextCheckpoint != nil &&
		...
		log.L.Trace("no sync peer candidates available")
	}
}
...
func New(config *Config) (*SyncManager, error) {
	...
	if !config.DisableCheckpoints {
		...
		log.L.Info("checkpoints are disabled")
	}
