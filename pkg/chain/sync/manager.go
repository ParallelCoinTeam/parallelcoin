	...
	for {
		...
		case m := <-sm.msgChan:
			...
			case processBlockMsg:
				...
				var isOrphan bool
				var err error
				if _, isOrphan, err = sm.chain.ProcessBlock(
					workerNumber,
					msg.block,
					msg.flags,
					heightUpdate,
...
func (sm *SyncManager) handleBlockchainNotification(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockDisconnected:
		...
		for _, tx := range block.Transactions()[1:] {
			_, _, err := sm.txMemPool.MaybeAcceptTransaction(
				sm.chain, tx,
				false, false,
			)
			if err != nil {
...
func (sm *SyncManager) handleTxMsg(tmsg *txMsg) {
	...
	// orphan handling, etc.
	acceptedTxs, err := sm.txMemPool.ProcessTransaction(
		sm.chain, tmsg.tx,
...
func (sm *SyncManager) haveInventory(invVect *wire.InvVect) (bool, error) {
	...
	case wire.InvTypeTx:
		...
		// Check if the transaction exists from the point of view of the end of the main
