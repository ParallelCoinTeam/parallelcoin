...
// mode, as this makes it harder to manipulate even with huge hash power
const (
	// maxAllowedOffsetSeconds is the maximum number of seconds in either
	// direction that local clock will be adjusted.  When the median time of the
...
(m *medianTime) AddTimeSample(sourceID string, timeVal time.Time) {
	...
	// Truncate the provided offset to seconds and append it to the slice of
	// offsets while respecting the maximum number of allowed entries by
	// replacing the oldest entry with the new entry once the maximum number of
	// entries is reached.
	now := time.Unix(time.Now().Unix(), 0)
	...
	// entries is odd, but the max number of entries is 200, an even number.
	// Thus, the offset will never be updated again once the max number of
	// entries is reached. The median offset is only updated when there are
	// enough offsets and the number of offsets is odd so the middle value is
	// the true median. Thus, there is nothing to do when those conditions are
	// not met.
	if numOffsets < 5 || numOffsets&0x01 != 1 {
		return
	}
	// At this point the number of offsets in the list is odd, so the middle
	// value of the sorted offsets is the median.
	median := sortedOffsets[numOffsets/2]
	// Set the new offset when the median offset is within the allowed offset
	// range.
	if math.Abs(float64(median)) < maxAllowedOffsetSecs {
		m.offsetSecs = median
	} else {
		// The median offset of all added time data is larger than the maximum
		// allowed offset, so don't use an offset.  This effectively limits how
		// far the local clock can be skewed.
		m.offsetSecs = 0
