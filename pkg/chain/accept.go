...
// the main chain. It performs several validation checks which depend on its position within the block chain before
// adding it.
//
...
func (b *BlockChain) maybeAcceptBlock(workerNumber uint32, block *util.Block,
	...
	if pn != nil {
		// The block must pass all of the validation rules which depend on the position of the block within the block
		// chain.
		if err = b.checkBlockContext(workerNumber, block, prevNode, flags, DoNotCheckPow); slog.Check(err) {
			return
		}
	}
	// Insert the block into the database if it's not already there. Even though it is possible the block will
	// ultimately fail to connect, it has already passed all proof-of-work and validity tests which means it would be
	// prohibitively expensive for an attacker to fill up the disk with a bunch of blocks that fail to connect. This is
	// necessary since it allows block download to be decoupled from the much more expensive connection logic. It also
	// has some other nice properties such as making blocks that never become part of the main chain or blocks that fail
	// to connect available for further analysis.
	if err = b.db.Update(func(dbTx database.Tx) (err error) {
		return dbStoreBlock(dbTx, block)
	}); slog.Check(err) {
		return
	}
	// Warn("creating new block node for new block") Create a new block node for the block and add it to the node index.
	// Even if the block ultimately gets connected to the main chain, it starts out on a side chain.
