...
func (b *BlockChain) maybeAcceptBlock(block *util.Block, flags BehaviorFlags) (bool, error) {
	log <- cl.Trace{"maybeAcceptBlock", cl.Ine()}
	...
	log <- cl.Trace{"check for blacklisted addresses",cl.Ine()}
	...
	log <- cl.Trace{"found no blacklisted addresses", cl.Ine()}
	var err error
	if pn != nil {
	...
	log <- cl.Trace{"creating new block node for new block", cl.Ine()}
