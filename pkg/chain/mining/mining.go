...
const (
	...
	// CoinbaseFlags is added to the coinbase script of a generated block and is
	// used to monitor BIP16 support as well as blocks that are generated via pod.
...
type (
	...
	// TxSource represents a source of transactions to consider for inclusion in new
	// blocks. The interface contract requires that all of these methods are safe
	// for concurrent access with respect to the source.
	TxSource interface {
		// LastUpdated returns the last time a transaction was added to or removed from
		// the source pool.
		LastUpdated() time.Time
		// MiningDescs returns a slice of mining descriptors for all the transactions in
		// the source pool.
		MiningDescs() []*TxDesc
	...
	txPrioItem struct {
		...
		//
		// It will only be set when the transaction references other transactions in the
		// source pool and hence must come after them in a block.
	...
	BlockTemplate struct {
		...
		// other transactions.
		Fees []int64
		// SigOpCosts contains the number of signature operations each transaction in
		// the generated template performs.
		SigOpCosts []int64
		// Height is the height at which the block template connects to the main chain.
		Height int32
		// ValidPayAddress indicates whether or not the template coinbase pays to an
		// address or is redeemable by anyone. See the documentation on NewBlockTemplate
	...
	// also houses additional state required in order to ensure the templates are
...
func txPQByPriority(pq *txPriorityQueue, i, j int) bool {
	// Using > here so that pop gives the highest priority item as opposed to the
	// lowest. Sort by priority first, then fee.
...
func txPQByFee(pq *txPriorityQueue, i, j int) bool {
	// Using > here so that pop gives the highest fee item as opposed to the lowest.
...
// based on the passed block height to the provided address. When the address is
// nil, the coinbase transaction will instead be redeemable by anyone. See the
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *util.Tx, height int32) (e error) {
...
func medianAdjustedTime(chainState *blockchain.BestState, timeSource blockchain.MedianTimeSource) time.Time {
	...
	// last several blocks. Thus, choose the maximum between the current time and
	// one second after the past median time. The current timestamp is truncated to
	// a second boundary before comparison since a block timestamp does not
	// supported a precision greater than one second.
...
// the transactions they depend on have been included. Once the high-priority
// area (if configured) has been filled with transactions, or the priority falls
// below what is considered high-priority, the priority queue is updated to
// prioritize by fees per kilobyte (then priority).
//
// When the fees per kilobyte drop below the TxMinFreeFee policy setting, the
// transaction will be skipped unless the BlockMinSize policy setting is
// nonzero, in which case the block will be filled with the low-fee/free
// transactions until the block size reaches that minimum size. Any transactions
// which would cause the block to exceed the BlockMaxSize policy setting, exceed
// the maximum allowed signature operations per block, or otherwise cause the
// block to be invalid are skipped.
//
...
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress util.Address, algo string,) (*BlockTemplate, error) {
	...
	if coinbaseScript, e = standardCoinbaseScript(nextBlockHeight, extraNonce); dbg.Chk(e) {
		return nil, e
	}
...
mempoolLoop:
	...
	totalFees := int64(0)
	// // Query the version bits state to see if segwit has been activated, if so then
	...
	// segwitState, e = g.Chain.ThresholdState(chaincfg.DeploymentSegwit)
	// if e != nil  {
	// 		// 	return nil, e
	// }
	...
	for priorityQueue.Len() > 0 {
		...
		tx := prioItem.tx
		// switch {
		// // If segregated witness has not been activated yet, then we shouldn't include
		// // any witness transactions in the block.
		...
		// 	// If we're about to include a transaction bearing witness data, then we'll also
		// 	// need to include a witness commitment in the coinbase transaction. Therefore,
		// 	// we account for the additional weight within the block with a model coinbase
		...
		if e = spendTransaction(blockUtxos, tx, nextBlockHeight); dbg.Chk(e) {
		}
	...
	// // we'll need to include a commitment to the witness data in an OP_RETURN output
	// // within the coinbase transaction.
	// var witnessCommitment []byte
	// if witnessIncluded {
	// 	// The witness of the coinbase transaction MUST be exactly 32-bytes of all
	// 	// zeroes.
	...
	// 	// The witness commitment itself is the double-sha256 of the witness preimage
	// 	// generated above. With the commitment generated, the witness script for the
	// 	// output is: OP_RETURN OP_DATA_36 {0xaa21a9ed || witnessCommitment}. The
	// 	// leading prefix is referred to as the "witness magic bytes".
	...
	if reqDifficulty, e = g.Chain.CalcNextRequiredDifficulty(algo); dbg.Chk(e) {
		return nil, e
	}
	...
	for _, tx := range blockTxns {
		if e := msgBlock.AddTransaction(tx.MsgTx()); dbg.Chk(e) {
			return nil, e
		}
	}
...
MsgBlock,
	...
	if g.ChainParams.ReduceMinDifficulty {
		...
		var e error
		if difficulty, e = g.Chain.CalcNextRequiredDifficulty(
			fork.GetAlgoName(
...
func (g *BlkTmplGenerator) UpdateExtraNonce(
	...
	var coinbaseScript []byte
	if coinbaseScript, e = standardCoinbaseScript(blockHeight, extraNonce); dbg.Chk(e) {
		return e
	}
...
// treated as immutable since it is shared by all callers. This function is safe
