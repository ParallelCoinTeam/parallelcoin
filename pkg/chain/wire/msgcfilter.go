...
const (
	// MaxCFilterDataSize is the maximum byte size of a committed filter. The maximum size is currently defined as
	// 256KiB.
	MaxCFilterDataSize = 256 * 1024
)

// MsgCFilter implements the Message interface and represents a bitcoin cfilter message. It is used to deliver a
// committed filter in response to a getcfilters (MsgGetCFilters) message.
...
// Deserialize decodes a filter from r into the receiver using a format that is suitable for long-term storage such as a
// database. This function differs from BtcDecode in that BtcDecode decodes from the bitcoin wire protocol as it was
// sent across the network. The wire encoding can technically differ depending on the protocol version and doesn't even
// really need to match the format of a stored filter at all. As of the time this comment was written, the encoded
// filter is the same in both instances, but there is a distinct difference and separating the two allows the API to be
// flexible enough to with changes.
