...
// defaultInvListAlloc is the default size used for the backing array for an inventory list. The array will dynamically
// grow as needed, but this figure is intended to provide enough space for the max number of inventory vectors in a
// *typical* inventory message without needing to grow the backing array multiple times. Technically, the list can grow
// to MaxInvPerMsg, but rather than using that large figure, this figure more accurately reflects the typical case.
...
func (msg *MsgInv) AddInvVect(iv *InvVect) (e error) {
...
func (msg *MsgInv) BtcDecode(r io.Reader, pver uint32, enc MessageEncoding) (e error) {
	...
	for i := uint64(0); i < count; i++ {
		iv := &invList[i]
		if e = readInvVect(r, pver, iv); dbg.Chk(e) {
			return
		}
		if e = msg.AddInvVect(iv); dbg.Chk(e) {
		}
	}
	return
}
...
func (msg *MsgInv) BtcEncode(w io.Writer, pver uint32, enc MessageEncoding) (e error) {
	...
	for _, iv := range msg.InvList {
		if e = writeInvVect(w, pver, iv); dbg.Chk(e) {
			return
		}
	}
	return
...
// NewMsgInvSizeHint returns a new bitcoin inv message that conforms to the Message interface. See MsgInv for details.
// This function differs from NewMsgInv in that it allows a default allocation size for the backing array which houses
// the inventory vector list. This allows callers who know in advance how large the inventory list will grow to avoid
// the overhead of growing the internal backing array several times when appending large amounts of inventory vectors
// with AddInvVect. Note that the specified hint is just that - a hint that is used for the default allocation size.
// Adding more (or less) inventory vectors will still work properly. The size hint is limited to MaxInvPerMsg.
