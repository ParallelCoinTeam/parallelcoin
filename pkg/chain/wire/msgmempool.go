...
func (msg *MsgMemPool) BtcDecode(r io.Reader, pver uint32, enc MessageEncoding) (err error) {
...
func (msg *MsgMemPool) BtcEncode(w io.Writer, pver uint32, enc MessageEncoding) (err error) {
	if pver < BIP0035Version {
		str := fmt.Sprintf("mempool message invalid for protocol "+
			"version %d", pver)
