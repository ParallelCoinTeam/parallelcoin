...
func (msg *MsgHeaders) BtcDecode(r io.Reader, pver uint32, enc MessageEncoding) (e error) {
	...
	for i := uint64(0); i < count; i++ {
		bh := &headers[i]
		if e = readBlockHeader(r, pver, bh); dbg.Chk(e) {
			return
		}
		...
		if e = msg.AddBlockHeader(bh); dbg.Chk(e) {
		}
	}
	return
}
...
func (msg *MsgHeaders) BtcEncode(w io.Writer, pver uint32, enc MessageEncoding) (e error) {
	...
	for _, bh := range msg.Headers {
		if e = writeBlockHeader(w, pver, bh); dbg.Chk(e) {
			return
		}
		...
		if e = WriteVarInt(w, pver, 0); dbg.Chk(e) {
			return
		}
	}
	return
