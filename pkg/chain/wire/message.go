...
const (
	...
	// WitnessEncoding encodes all messages other than transaction messages using the default Bitcoin wire protocol
	// specification. For transaction messages, the new encoding format detailed in BIP0144 will be used.
	WitnessEncoding
)
...
func ReadMessageWithEncodingN(r io.Reader, pver uint32, btcnet BitcoinNet, enc MessageEncoding) (int, Message, []byte, error) {
	...
	// Check for maximum length based on the message type as a malicious client could otherwise create a well-formed
