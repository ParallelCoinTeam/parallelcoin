...
func readMessageHeader(r io.Reader) (n int, mh *messageHeader, e error) {
	...
	if n, e = io.ReadFull(r, headerBytes[:]); dbg.Chk(e) {
		if e != io.EOF {
			trc.Ln(err)
		}
		return n, nil, e
	...
	if e = readElements(hr, &hdr.magic, &command, &hdr.length, &hdr.checksum); dbg.Chk(e) {
	}
...
func WriteMessageWithEncodingN(
	...
	var bw bytes.Buffer
	if e = msg.BtcEncode(&bw, pver, encoding); dbg.Chk(e) {
		return totalBytes, e
	}
	payload := bw.Bytes()
	lenp := len(payload)
	...
	if e = writeElements(hw, hdr.magic, command, hdr.length, hdr.checksum); dbg.Chk(e) {
	}
	// Write header.
	var n int
...
func ReadMessageWithEncodingN(r io.Reader, pver uint32, btcnet BitcoinNet, enc MessageEncoding) (
	...
	if msg, e = makeEmptyMessage(command); dbg.Chk(e) {
		discardInput(r, hdr.length)
	...
	if e = msg.BtcDecode(pr, pver, enc); dbg.Chk(e) {
		return totalBytes, nil, nil, e
	}
	return
}
