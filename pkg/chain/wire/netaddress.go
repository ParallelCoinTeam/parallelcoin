...
type NetAddress struct {
	// Last time the address was seen. This is, unfortunately, encoded as a uint32
	// on the wire and therefore is limited to 2106. This field is not present in
	// the bitcoin version message (MsgVersion) nor was it added until protocol
	...
	IP net.IP
	// Port the peer is using. This is encoded in big endian on the wire which
	// differs from most everything else.
	Port uint16
}
...
func readNetAddress(r io.Reader, pver uint32, na *NetAddress, ts bool) (e error) {
	...
	if ts && pver >= NetAddressTimeVersion {
		if e = readElement(r, (*uint32Time)(&na.Timestamp));dbg.Chk(e){
			return
		}
	}
	if e = readElements(r, &na.Services, &ip);dbg.Chk(e){
		return
	}
	// Sigh.  Bitcoin protocol mixes little and big endian.
	var port uint16
	if port, e = binarySerializer.Uint16(r, bigEndian);dbg.Chk(e){
		return
	}
	*na = NetAddress{
...
func writeNetAddress(w io.Writer, pver uint32, na *NetAddress, ts bool) (e error) {
	...
	if ts && pver >= NetAddressTimeVersion {
		if e = writeElement(w, uint32(na.Timestamp.Unix()));dbg.Chk(e){
			return
		}
	}
	...
	if e = writeElements(w, na.Services, ip);dbg.Chk(e){
		return
	}
	// Sigh.  Bitcoin protocol mixes little and big endian.
