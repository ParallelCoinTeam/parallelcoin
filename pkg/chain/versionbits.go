...
// RuleChangeActivationThreshold is the number of blocks for which the condition must be true in order to lock in a rule change. This implementation returns the value defined by the chain netparams the checker is associated with. This is part of the thresholdConditionChecker interface implementation.
func (c bitConditionChecker) RuleChangeActivationThreshold() uint32 {
	return c.chain.params.RuleChangeActivationThreshold
}

// MinerConfirmationWindow is the number of blocks in each threshold state retarget window. This implementation returns the value defined by the chain netparams the checker is associated with. This is part of the thresholdConditionChecker interface implementation.
...
// deploymentChecker provides a thresholdConditionChecker which can be used to test a specific deployment rule.  This is required for properly detecting and activating consensus rule changes.
