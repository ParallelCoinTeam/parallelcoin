...
func (b *BlockChain) findPreviousCheckpoint() (*blockNode, error) {
	...
	if b.checkpointNode == nil && b.nextCheckpoint == nil {
		...
		// No known latest checkpoint.  This will only happen on blocks before the first known checkpoint.  So, set the next expected checkpoint to the first checkpoint and return the fact there is no latest known checkpoint block.
