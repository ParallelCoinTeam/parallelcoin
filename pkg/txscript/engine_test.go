...
func TestBadPC(t *testing.T) {
	...
	for _, test := range tests {
		...
		if e == nil {
			t.Errorf(
				"DisasmPC with invalid pc (%v) succeeds!",
				test,
			)
		}
	}
...
func TestCheckErrorCondition(t *testing.T) {
	...
	for i := 0; i < len(pkScript)-1; i++ {
		...
		if !IsErrorCode(e, ErrScriptUnfinished) {
			t.Fatalf(
				"got unexepected error %v on %dth iteration",
				e, i,
			)
		}
	}
	...
	if e != nil {
		t.Fatalf("final step failed %v", e)
	}
	if !done {
	...
	e = vm.CheckErrorCondition(false)
	if e != nil {
		t.Errorf("unexpected error %v on final check", e)
	}
}
...
func TestInvalidFlagCombinations(t *testing.T) {
	...
	for i, test := range tests {
		...
		if !IsErrorCode(e, ErrInvalidFlags) {
			t.Fatalf(
				"TestInvalidFlagCombinations #%d unexpected "+
					"error: %v", i, e,
			)
		}
	}
...
func TestCheckPubKeyEncoding(t *testing.T) {
	...
	for _, test := range tests {
		...
		if e != nil && test.isValid {
			t.Errorf(
				"checkSignatureEncoding test '%s' failed "+
					"when it should have succeeded: %v", test.name,
				e,
			)
		} else if e == nil && !test.isValid {
			t.Errorf(
				"checkSignatureEncooding test '%s' succeeded "+
