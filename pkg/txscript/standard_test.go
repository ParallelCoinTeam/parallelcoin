...
func mustParseShortForm(script string) []byte {
	...
	if e != nil {
		panic(
			"invalid short form script in test source: err " +
				e.Error() + ", script: " + script,
		)
	}
	return s
}
...
func TestExtractPkScriptAddrs(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			script: hexToBytes(
				"410411db93e1dcdb8a016b49840f8c53b" +
		...
		},
		{
			...
			script: hexToBytes(
				"4106192d74d0cb94344c9569c2e779015" +
		...
		},
		{
			...
			script: hexToBytes(
				"4104b0bd634234abbb1ba1e986e884185" +
		...
		},
		{
			...
			script: hexToBytes(
				"4107b0bd634234abbb1ba1e986e884185" +
					"c61cf43e001f9137f23c2c409273eb16e6537a576782" +
					"eba668a7ef8bd3b3cfb1edb7117ab65129b8a2e681f3" +
					"c1e0908ef7bac",
			),
		...
		// from real tx 60a20bd93aa49ab4b28d514ec10b06e1829ce6818ec06cd3aabd013ebcdc4bb1, vout 0
		{
			...
			script: hexToBytes(
				"514104cc71eb30d653c0c3163990c47b9" +
		...
		// from real tx d646f82bd5fbdb94a36872ce460f97662b80c3050ad3209bef9d1e398ea277ab, vin 1
		{
			...
			script: hexToBytes(
				"524104cb9c3c222c5f7a7d3b9bd152f36" +
		...
		},
		{
			...
			script: hexToBytes(
				"47304402204e45e16932b8af514961a1d" +
		...
		// Note the technically the pubkey is the second item on the stack, but since the address extraction intentionally only works with standard PkScripts, this should not return any addresses.
		{
			name: "valid sigscript to reedeem p2pk - no addresses",
			script: hexToBytes(
				"493046022100ddc69738bf2336318e4e0" +
		...
		// from real tx 691dd277dc0e90a462a3d652a1171686de49cf19067cd33c7df0392833fb986a, vout 0 invalid public keys
		{
			name: "1 of 3 multisig with invalid pubkeys",
			script: hexToBytes(
				"51411c2200007353455857696b696c656" +
		...
		// from real tx: 691dd277dc0e90a462a3d652a1171686de49cf19067cd33c7df0392833fb986a, vout 44 invalid public keys
		{
			name: "1 of 3 multisig with invalid pubkeys 2",
			script: hexToBytes(
				"514134633365633235396337346461636" +
	...
	for i, test := range tests {
		...
		if !reflect.DeepEqual(addrs, test.addrs) {
			t.Errorf(
				"ExtractPkScriptAddrs #%d (%s) unexpected "+
					"addresses\ngot  %v\nwant %v", i, test.name,
				addrs, test.addrs,
			)
			continue
...
func TestCalcScriptInfo(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// Invented scripts, the hashes do not match Truncated version of test below:
		...
		{
			// from 567a53d1ce19ce3d07711885168484439965501536d0d0294c5d46d46c10e53b from the blockchain.
	...
	for _, test := range tests {
		...
		if *si != test.scriptInfo {
			t.Errorf(
				"%s: scriptinfo doesn't match expected. "+
...
// ScriptAddress simply returns an empty byte slice.  It exists to satisfy the util.Address interface.
...
// String simply returns an empty string.  It exists to satisfy the util.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	if e != nil {
		t.Fatalf(
			"Unable to create pubkey address (compressed): %v",
			e,
		)
	}
	...
	if e != nil {
		t.Fatalf(
			"Unable to create pubkey address (compressed 2): %v",
			e,
		)
	}
	...
	if e != nil {
		t.Fatalf(
			"Unable to create pubkey address (uncompressed): %v",
			e,
		)
	}
...
var scriptClassTests = []struct {
	...
	{
		// Nulldata with more than max allowed data to be considered standard (so therefore nonstandard)
	...
	{
		// Almost nulldata, but add an additional opcode after the data to make it nonstandard.
...
func TestScriptClass(t *testing.T) {
	...
	for _, test := range scriptClassTests {
		...
		if class != test.class {
			t.Errorf(
				"%s: expected %s got %s (script %x)", test.name,
...
func TestNullDataScript(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			data: hexToBytes(
				"0102030405060708090a0b0c0d0e0f10111" +
					"2131415161718",
			),
			expected: mustParseShortForm(
				"RETURN 0x18 0x01020304" +
	...
	for i, test := range tests {
		...
		if !bytes.Equal(script, test.expected) {
			t.Errorf(
				"NullDataScript: #%d (%s) wrong result\n"+
		...
		if scriptType != test.class {
			t.Errorf(
				"GetScriptClass: #%d (%s) wrong result -- "+
