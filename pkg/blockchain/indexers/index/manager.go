...
func dbIndexConnectBlock(
	...
	if curTipHash, _, e = dbFetchIndexerTip(dbTx, idxKey); err.Chk(e) {
		return e
	}
	...
	if e := indexer.ConnectBlock(dbTx, block, stxo); err.Chk(e) {
		return e
	}
...
func dbIndexDisconnectBlock(
	...
	if e := indexer.DisconnectBlock(dbTx, block, stxo); err.Chk(e) {
		return e
	}
	// Update the current index tip.
...
func (m *Manager) maybeFinishDrops(interrupt <-chan struct{}) (e error) {
	...
	for i, indexer := range m.enabledIndexes {
		...
		if e = dropIndex(m.db, indexer.Key(), indexer.Name(), interrupt); err.Chk(e) {
			return e
		}
	}
	return nil
...
func (m *Manager) maybeCreateIndexes(dbTx database.Tx) (e error) {
	...
	for _, indexer := range m.enabledIndexes {
		...
		if e := indexer.Create(dbTx); err.Chk(e) {
			return e
		}
...
func (m *Manager) Init(chain *blockchain.BlockChain, interrupt <-chan struct{}) (e error) {
	...
	if e := m.maybeFinishDrops(interrupt); err.Chk(e) {
		return e
	}
...
func dropIndex(db database.DB, idxKey []byte, idxName string, interrupt <-chan struct{}) (e error) {
	...
	if idxName == txIndexName {
		if e := dropBlockIDIndex(db); err.Chk(e) {
			return e
		}
	}
	...
	e = db.Update(
		func(dbTx database.Tx) (e error) {
			...
			if e := indexesBucket.Delete(idxKey); err.Chk(e) {
				return e
			}
