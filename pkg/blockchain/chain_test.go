...
// nodeHeaders is a convenience function that returns the headers for all of the passed indexes of the provided nodes.
// It is used to construct expected located headers in the tests.
...
// TestLocateInventory ensures that locating inventory via the LocateHeaders and LocateBlocks functions behaves as
// expected.
func TestLocateInventory(t *testing.T) {
	...
	// Create chain views for different branches of the overall chain to simulate a local and remote node on different
	// parts of the chain.
	localView := newChainView(tip(branch0Nodes))
	remoteView := newChainView(tip(branch1Nodes))
	// Create a chain view for a completely unrelated block chain to simulate a remote node on a totally different
	// chain.
	...
	tests := []struct {
		...
		// stop hash for locator
		hashStop chainhash.Hash
		// max to locate, 0 = wire const
		maxAllowed uint32
		// expected located headers
		headers []wire.BlockHeader
		// expected located hashes
		...
		{
			// Locators based on remote being on side chain and a stop hash local node doesn't know about. The expected
			// result is the blocks after the fork point in the main chain and the stop hash has no effect.
		...
		{
			// Locators based on remote being on side chain and a stop hash in side chain. The expected result is the
			// blocks after the fork point in the main chain and the stop hash has no effect.
		...
		{
			// Locators based on remote being on side chain and a stop hash in main chain, but before fork point. The
			// expected result is the blocks after the fork point in the main chain and the stop hash has no effect.
		...
		{
			// Locators based on remote being on side chain and a stop hash in main chain, but exactly at the fork
			// point. The expected result is the blocks after the fork point in the main chain and the stop hash has no
			// effect.
		...
		{
			// Locators based on remote being on side chain and a stop hash in main chain just after the fork point. The
			// expected result is the blocks after the fork point in the main chain up to and including the stop hash.
		...
		{
			// Locators based on remote being on side chain and a stop hash in main chain some time after the fork
			// point. The expected result is the blocks after the fork point in the main chain up to and including the
			// stop hash.
		...
		{
			// Locators based on remote being on main chain in the past and a stop hash local node doesn't know about.
			// The expected result is the blocks after the known point in the main chain and the stop hash has no
			// effect.
		...
		{
			// Locators based on remote being on main chain in the past and a stop hash in a side chain. The expected
			// result is the blocks after the known point in the main chain and the stop hash has no effect.
		...
		{
			// Locators based on remote being on main chain in the past and a stop hash in the main chain before that
			// point. The expected result is the blocks after the known point in the main chain and the stop hash has no
			// effect.
		...
		{
			// Locators based on remote being on main chain in the past and a stop hash in the main chain exactly at
			// that point. The expected result is the blocks after the known point in the main chain and the stop hash
			// has no effect.
		...
		{
			// Locators based on remote being on main chain in the past and a stop hash in the main chain just after
			// that point. The expected result is the blocks after the known point in the main chain and the stop hash
			// has no effect.
		...
		{
			// Locators based on remote being on main chain in the past and a stop hash in the main chain some time
			// after that point. The expected result is the blocks after the known point in the main chain and the stop
			// hash has no effect.
		...
		{
			// Locators based on remote being at exactly the same point in the main chain and a stop hash local node
			// doesn't know about. The expected result is no located inventory.
		...
		{
			// Locators based on remote being at exactly the same point in the main chain and a stop hash at exactly the
			// same point. The expected result is no located inventory.
		...
		{
			// Locators from remote that don't include any blocks the local node knows. This would happen if the remote
			// node is on a completely separate chain that isn't rooted with the same genesis block. The expected result
			// is the blocks after the genesis block.
		...
		},
		{
			...
			// The expected result is the blocks after the second block limited by the max.
			name:       "remote genesis",
		...
		{
			// Poorly formed locator. Locator from remote that only includes a single block on a side chain the local
			// node knows. The expected result is the blocks after the genesis block since even though the block is
			// known, it is on a side chain and there are no more locators to find the fork point.
		...
		},
		{
			...
			// node knows however none in the main chain. The expected result is the blocks after the genesis block
			// since even though the blocks are known, they are all on a side chain and there are no more locators to
			// find the fork point.
		...
		{
			// Poorly formed locator. Locator from remote that only includes multiple blocks on a side chain the local
			// node knows however none in the main chain but includes a stop hash in the main chain. The expected result
			// is the blocks after the genesis block up to the stop hash since even though the blocks are known, they
			// are all on a side chain and there are no more locators to find the fork point.
