...
const (
	...
	tfSpent
	// tfModified indicates that a txout has been modified since it was loaded.
	tfModified
)

// UtxoEntry houses details about an individual transaction output in a utxo view such as whether or not it was
// contained in a coinbase tx, the height of the block that contains the tx, whether or not it is spent, its public key
...
type UtxoEntry struct {
	...
	// result in minimal padding. There will be a lot of these in memory, so a few extra bytes of padding adds up.
	amount      int64
	pkScript    []byte // The public key script for the output.
	blockHeight int32  // Height of block containing tx.
	// packedFlags contains additional info about output such as whether it is a coinbase, whether it is spent, and
	// whether it has been modified since it was loaded. This approach is used in order to reduce memory usage since
...
// IsSpent returns whether or not the output has been spent based upon the current state of the unspent transaction
// output view it was obtained from.
...
// UtxoViewpoint represents a view into the set of unspent transaction outputs from a specific point of view in the
// chain. For example, it could be for the end of the main chain, some point in the history of the main chain, or down a
// side chain.
//
// The unspent outputs are needed by other transactions for things such as script validation and double spend
// prevention.
...
// LookupEntry returns information about a given transaction output according to the current state of the view. It will
// return nil if the passed output does not exist in the view or is otherwise not available such as when it has been
// disconnected during a reorg.
func (view *UtxoViewpoint) LookupEntry(outpoint wire.OutPoint) *UtxoEntry {
	return view.entries[outpoint]
}

// addTxOut adds the specified output to the view if it is not provably unspendable. When the view already has an entry
// for the output, it will be marked unspent. All fields will be updated for existing entries since it's possible it has
// changed during a reorg.
func (view *UtxoViewpoint) addTxOut(outpoint wire.OutPoint, txOut *wire.TxOut, isCoinBase bool, blockHeight int32) {
	...
	// Update existing entries. All fields are updated because it's possible (although extremely unlikely) that the
	// existing entry is being replaced by a different transaction with the same hash. This is allowed so long as the
...
// AddTxOut adds the specified output of the passed transaction to the view if it exists and is not provably
// unspendable. When the view already has an entry for the output, it will be marked unspent. All fields will be updated
...
// AddTxOuts adds all outputs in the passed transaction which are not provably unspendable to the view. When the view
// already has entries for any of the outputs, they are simply marked unspent. All fields will be updated for existing
...
func (view *UtxoViewpoint) AddTxOuts(tx *util.Tx, blockHeight int32) {
	...
	for txOutIdx, txOut := range tx.MsgTx().TxOut {
		// Update existing entries. All fields are updated because it's possible (although extremely unlikely) that the
		// existing entry is being replaced by a different transaction with the same hash. This is allowed so long as
		// the previous transaction is fully spent.
		prevOut.Index = uint32(txOutIdx)
		view.addTxOut(prevOut, txOut, isCoinBase, blockHeight)
	}
}

// connectTransaction updates the view by adding all new utxos created by the passed transaction and marking all utxos
// that the transactions spend as spent.
//
...
func (view *UtxoViewpoint) connectTransaction(tx *util.Tx, blockHeight int32, stxos *[]SpentTxOut) (e error) {
	...
	// Spend the referenced utxos by marking them spent in the view and, if a slice was provided for the spent txout
	// details, append an entry to it.
	for _, txIn := range tx.MsgTx().TxIn {
		// Ensure the referenced utxo exists in the view. This should never happen unless there is a bug is introduced
		// in the code.
		...
		// Mark the entry as spent. This is not done until after the relevant details have been accessed since spending
		// it might clear the fields from memory in the future.
		entry.Spend()
	}
	// Add the transaction's outputs as available utxos.
	view.AddTxOuts(tx, blockHeight)
...
func (view *UtxoViewpoint) disconnectTransactions(db database.DB, block *block.Block, stxos []SpentTxOut) (e error) {
	...
	for txIdx := len(transactions) - 1; txIdx > -1; txIdx-- {
		...
		// Mark all of the spendable outputs originally created by the transaction as spent. It is instructive to note
		// that while the outputs aren't actually being spent here, rather they no longer exist, since a pruned utxo set
		// is used, there is no practical difference between a utxo that does not exist and one that has been spent.
		//
		// When the utxo does not already exist in the view, add an entry for it and then mark it spent. This is done
		// because the code relies on its existence in the view in order to signal modifications have happened.
		txHash := tx.Hash()
		...
		// Loop backwards through all of the transaction inputs (except for the coinbase which has no inputs) and
		// unspend the referenced txos. This is necessary to match the order of the spent txout entries.
		if isCoinBase {
			continue
		}
		for txInIdx := len(tx.MsgTx().TxIn) - 1; txInIdx > -1; txInIdx-- {
			// Ensure the spent txout index is decremented to stay in sync with the transaction input.
			stxo := &stxos[stxoIdx]
			stxoIdx--
			// When there is not already an entry for the referenced output in the view, it means it was previously
			// spent, so create a new utxo entry in order to resurrect it.
			...
			// The legacy v1 spend journal format only stored the coinbase flag and height when the output was the last
			// unspent output of the transaction. As a result, when the information is missing, search for it by
			// scanning all possible outputs of the transaction since it must be in one of them.
			//
			// It should be noted that this is quite inefficient, but it realistically will almost never run since all
			// new entries include the information for all outputs and thus the only way this will be hit is if a long
			// enough reorg happens such that a block with the old spend data is being disconnected. The probability of
			// that in practice is extremely low to begin with and becomes vanishingly small the more new blocks are
			...
			// Restore the utxo using the stxo data from the spend journal and mark it as modified.
...
// RemoveEntry removes the given transaction output from the current state of the view. It will have no effect if the
// passed output does not exist in the view.
...
// fetchUtxosMain fetches unspent transaction output data about the provided set of outpoints from the point of view of
// the end of the main chain at the time of the call.
//
...
func (view *UtxoViewpoint) fetchUtxosMain(db database.DB, outpoints map[wire.OutPoint]struct{}) (e error) {
	...
	// Load the requested set of unspent transaction outputs from the point of view of the end of the main chain.
	//
...
func (view *UtxoViewpoint) fetchInputUtxos(db database.DB, block *block.Block) (e error) {
	...
	// Loop through all of the transaction inputs (except for the coinbase which has no inputs) collecting them into
	// sets of what is needed and what is already known (in-flight).
	neededSet := make(map[wire.OutPoint]struct{})
	for i, tx := range transactions[1:] {
		for _, txIn := range tx.MsgTx().TxIn {
			// It is acceptable for a transaction input to reference the output of another transaction in this block
			// only if the referenced transaction comes before the current one in this block. Add the outputs of the
			// referenced transaction as available utxos when this is the case. Otherwise, the utxo details are still
			// needed.
			//
			// NOTE: The >= is correct here because i is one less than the actual position of the transaction within the
			// block due to skipping the coinbase.
			...
			// Don't request entries that are already in the view from the database.
...
// FetchUtxoView loads unspent transaction outputs for the inputs referenced by the passed transaction from the point of
// view of the end of the main chain.
//
// It also attempts to fetch the utxos for the outputs of the transaction itself so the returned view can be examined
...
// main chain.
//
// NOTE: Requesting an output for which there is no data will NOT return an error. Instead both the entry and the error
// will be nil. This is done to allow pruning of spent transaction outputs. In practice this means the caller must check
// if the returned entry is nil before invoking methods on it.
//
