...
// MsgReject implements the Message interface and represents a bitcoin reject message. This message was not added until
// protocol version RejectVersion.
type MsgReject struct {
	// Cmd is the command for the message which was rejected such as as CmdBlock or CmdTx. This can be obtained from the
	// Command function of a Message.
	Cmd string
...
func (msg *MsgReject) MaxPayloadLength(pver uint32) uint32 {
	...
	if pver >= RejectVersion {
		// Unfortunately the bitcoin protocol does not enforce a sane limit on the length of the reason, so the max
		// payload is the overall maximum message payload.
