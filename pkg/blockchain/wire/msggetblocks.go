...
// double the step each loop iteration to decrease the number of hashes the
// further away from head and closer to the genesis block you get.
...
func (msg *MsgGetBlocks) AddBlockLocatorHash(hash *chainhash.Hash) (e error) {
...
func (msg *MsgGetBlocks) BtcDecode(r io.Reader, pver uint32, enc MessageEncoding) (e error) {
	if e = readElement(r, &msg.ProtocolVersion); err.Chk(e) {
		return
	}
	...
	for i := uint64(0); i < count; i++ {
		...
		if e = readElement(r, hash); err.Chk(e) {
			return
		}
		if e = msg.AddBlockLocatorHash(hash); err.Chk(e) {
		}
	}
...
func (msg *MsgGetBlocks) BtcEncode(w io.Writer, pver uint32, enc MessageEncoding) (e error) {
	...
	if e = writeElement(w, msg.ProtocolVersion); err.Chk(e) {
	...
	for _, hash := range msg.BlockLocatorHashes {
		if e = writeElement(w, hash); err.Chk(e) {
			return
		}
	}
