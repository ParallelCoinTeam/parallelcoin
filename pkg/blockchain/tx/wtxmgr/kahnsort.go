...
import chainhash "github.com/p9c/pod/pkg/blockchain/chainhash"
...
func makeGraph(set map[chainhash.Hash]*TxRecord) hashGraph {
	...
	for _, rec := range set {
		// Add a node for every transaction record. The output edges and input degree are set by iterating over each
		// record's inputs below.
		if _, ok := graph[rec.Hash]; !ok {
			graph[rec.Hash] = graphNode{value: rec}
		}
	inputLoop:
		for _, input := range rec.MsgTx.TxIn {
			// Transaction inputs that reference transactions not included in the set do not create any (local) graph
			// edges.
			...
			// Mark a directed edge from the previous transaction hash to this transaction record and increase the input
			// degree for this record's node.
...
// graphRoots returns the roots of the graph. That is, it returns the node's values for all nodes which contain an input
// degree of 0.
...
// dependencySort topologically sorts a set of transaction records by their dependency order. It is implemented using
// Kahn's algorithm.
