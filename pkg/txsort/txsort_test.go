...
import (
	...
	txsort "github.com/p9c/pod/pkg/txsort"
...
func TestSort(t *testing.T) {
	...
	for _, test := range tests {
		...
		if got := txsort.IsSorted(&tx); got != test.isSorted {
			t.Errorf("IsSorted (%s): txsort does not match "+
		...
		// Now txsort the transaction using the mutable version and ensure the resulting hash is the expected value.
