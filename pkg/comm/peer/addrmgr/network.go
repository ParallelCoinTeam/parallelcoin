...
var (
	...
	// rfc4862Net specifies the IPv6 stateless address autoconfiguration address block as defined by RFC4862 (FE80::/64).
	rfc4862Net = ipNet("FE80::", 64, 128)
	// rfc5737Net specifies the IPv4 documentation address blocks as defined by RFC5737 (192.0.2.0/24, 198.51.100.0/24, 203.0.113.0/24)
	...
	// onionCatNet defines the IPv6 address block used to support Tor. bitcoind encodes a .onion address as a 16 byte number by decoding the address prior to the .onion (i.e. the key hash) base32 into a ten/ byte number. It then stores the first 6 bytes of the address as 0xfd, 0x87, 0xd8, 0x7e, 0xeb, 0x43.
	// This is the same range used by OnionCat, which is part part of the RFC4193 unique local IPv6 range.
...
// IsValid returns whether or not the passed address is valid.  The address is considered invalid under the following circumstances:
