...
type (
	...
	SeriesData struct {
		version uint32
		// Whether or not a series is active. This is serialized/deserialized but for now there's no way to deactivate a
		// series.
		active bool
	...
	// Pool represents an arrangement of notary servers to securely store and account for customer cryptocurrency
	// deposits and to redeem valid withdrawals. For details about how the arrangement works, see
	...
	// PoolAddress represents a voting pool P2SH address, generated by deriving public HD keys from the series' master
	// keys using the given branch/index and constructing a M-of-N multi-sig script.
	...
	// ChangeAddress is a votingpool address meant to be used on transaction change outputs. All change addresses have
	// branch==0.
...
// saveSeriesToDisk stores the given series ID and data in the database, first encrypting the public/private extended
// keys.
//
...
// Convert the given slice of strings into a slice of ExtendedKeys, checking that all of them are valid public (and not
// private) keys, and that there are no duplicates.
...
// putSeries creates a new seriesData with the given arguments, ordering the given public keys (using
// CanonicalKeyOrder), validating and converting them to hdkeychain.ExtendedKeys, saves that to disk and adds it to this
// voting pool's seriesLookup map. It also ensures inRawPubKeys has at least minSeriesPubKeys items and reqSigs is not
...
func (p *Pool) ActivateSeries(ns walletdb.ReadWriteBucket, seriesID uint32) (e error) {
...
// decryptExtendedKey uses Manager.Decrypt() to decrypt the encrypted byte slice and return an extended (public or
// private) key representing it.
//
// This method must be called with the Pool's manager unlocked.
...
// validateAndDecryptSeriesKeys checks that the length of the public and private key slices is the same, decrypts them,
...
// LoadAllSeries fetches all series (decrypting their public and private extended keys) for this Pool from the database
// and populates the seriesLookup map with them. If there are any private extended keys for a series, it will also
// ensure they have a matching extended public key in that series.
//
// This method must be called with the Pool's manager unlocked.
...
func branchOrder(pks []*hdkeychain.ExtendedKey, branch Branch) ([]*hdkeychain.ExtendedKey, error) {
	if pks == nil {
		// This really shouldn't happen, but we want to be good citizens, so we return an error instead of crashing.
...
// DepositScriptAddress calls DepositScript to get a multi-signature redemption script and returns the
// pay-to-script-hash-address for that script.
func (p *Pool) DepositScriptAddress(seriesID uint32, branch Branch, index Index) (btcaddr.Address, error) {
	script, e := p.DepositScript(seriesID, branch, index)
	if e != nil {
		return nil, e
	}
...
func (p *Pool) addressFor(script []byte) (btcaddr.Address, error) {
	scriptHash := btcaddr.Hash160(script)
	return btcaddr.NewScriptHashFromHash(scriptHash, p.manager.ChainParams())
}

// DepositScript constructs and returns a multi-signature redemption script where a certain number (Series.reqSigs) of
// the public keys belonging to the series with the given ID are required to sign the transaction for it to be
// successful.
...
// ChangeAddress returns a new votingpool address for the given seriesID and index, on the 0th branch (which is reserved
// for change addresses). The series with the given ID must be active.
...
// WithdrawalAddress queries the address manager for the P2SH address of the redeem script generated with the given
// series/branch/index and uses that to populate the returned WithdrawalAddress. This is done because we should only
// withdraw from previously used addresses but also because when processing withdrawals we may iterate over a huge
...
// EmpowerSeries adds the given extended private key (in raw format) to the series with the given ID, thus allowing it
// to sign deposit/withdrawal scripts. The series with the given ID must exist, the key must be a valid private extended
// key and must match one of the series' extended public keys.
//
// This method must be called with the Pool's manager unlocked.
func (p *Pool) EmpowerSeries(ns walletdb.ReadWriteBucket, seriesID uint32, rawPrivKey string) (e error) {
	...
	found := false
	// Make sure the private key has the corresponding public key in the series, to be able to empower it.
...
func (p *Pool) addUsedAddr(
	...
	encryptedHash, e := p.manager.Encrypt(waddrmgr.CKTPublic, btcaddr.Hash160(script))
	if e != nil {
...
func (p *Pool) getUsedAddr(ns, addrmgrNs walletdb.ReadBucket, seriesID uint32, branch Branch, index Index) (
	...
	addr, e := btcaddr.NewScriptHashFromHash(hash, mgr.ChainParams())
	if e != nil {
...
// highestUsedIndexFor returns the highest index from this Pool's used addresses with the given seriesID and branch. It
// returns 0 if there are no used addresses with the given seriesID and branch.
