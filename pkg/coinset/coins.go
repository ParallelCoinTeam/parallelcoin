...
func (cs *CoinSet) TotalValue() (value amt.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue amt.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue amt.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount amt.Amount
}

// CoinSelect will attempt to select coins using the algorithm described in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue amt.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue amt.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue amt.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue amt.Amount, coins []Coin) (Coins, error) {
...
func (c *SimpleCoin) Value() amt.Amount {
	return amt.Amount(c.txOut().Value)
}
