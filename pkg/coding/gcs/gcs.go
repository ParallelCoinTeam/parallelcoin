...
func BuildGCSFilter(P uint8, M uint64, key [KeySize]byte, data [][]byte) (f *Filter, err error) {
	...
	// First we'll compute the value of m, which is the modulus we use within our finite field. We want to compute: mScalar * 2^P. We use math.Round in order to round the value up, rather than down.
...
func (f *Filter) MatchAny(key [KeySize]byte, data [][]byte) (tf bool, err error) {
	...
	sort.Sort(values)
	// Zip down the filters, comparing values until we either run out of values to compare in one of the filters or we reach a matching value.
