...
const (
	// ErrDbTypeRegistered indicates two different database drivers attempt to
	// register with the name database type.
	ErrDbTypeRegistered ErrorCode = iota
	// ErrDbUnknownType indicates there is no driver registered for the specified
	// database type.
	ErrDbUnknownType
	// ErrDbDoesNotExist indicates open is called for a database that does not
	// exist.
	ErrDbDoesNotExist
	// ErrDbExists indicates create is called for a database that already exists.
	ErrDbExists
	// ErrDbNotOpen indicates a database instance is accessed before it is opened
	// or after it is closed.
	ErrDbNotOpen
	// ErrDbAlreadyOpen indicates open was called on a database that is already
	// open.
	ErrDbAlreadyOpen
	// ErrInvalid indicates the specified database is not valid.
	ErrInvalid
	// ErrCorruption indicates a checksum failure occurred which invariably means
	...
	ErrTxClosed
	// ErrTxNotWritable indicates an operation that requires write access to the
	// database was attempted against a read-only transaction.
	ErrTxNotWritable
	// ErrBucketNotFound indicates an attempt to access a bucket that has not
	// been created yet.
	ErrBucketNotFound
	// ErrBucketExists indicates an attempt to create a bucket that already
	// exists.
	ErrBucketExists
	// ErrBucketNameRequired indicates an attempt to create a bucket with a blank
	// name.
	...
	// max allowed value size. The max key size depends on the specific backend
	// driver being used.
	ErrValueTooLarge
	// ErrIncompatibleValue indicates the value in question is invalid for the
	// specific requested operation. For example, trying create or delete a
	// bucket with an existing non-bucket key, attempting to create or delete a
	// non-bucket key with an existing bucket key, or trying to delete a value
	// via a cursor when it points to a nested bucket.
	ErrIncompatibleValue
	// ErrBlockNotFound indicates a block with the provided hash does not exist
	// in the database.
	ErrBlockNotFound
	// ErrBlockExists indicates a block with the provided hash already exists in
	// the database.
	ErrBlockExists
	// ErrBlockRegionInvalid indicates a region that exceeds the bounds of the
	// specified block was requested. When the hash provided by the region does
	// not correspond to an existing block, the error will be ErrBlockNotFound
	// instead.
	ErrBlockRegionInvalid
	// ErrDriverSpecific indicates the Err field is a driver-specific error. This
	// provides a mechanism for drivers to plug-in their own custom errors for
	// any situations which aren't already covered by the error codes provided by
	// this package.
	ErrDriverSpecific
...
//
// The caller can use type assertions to determine if an error is an DBError and
...
// underlying error. Depending on the backend driver, the Err field might be set
// to the underlying error for other error codes as well.
type DBError struct {
