...
func TestAddDuplicateDriver(t *testing.T) {
	...
	if e != walletdb.ErrDbTypeRegistered {
		t.Errorf(
			"unexpected duplicate driver registration error - "+
				"got %v, want %v", e, walletdb.ErrDbTypeRegistered,
		)
	}
	dbPath := "dupdrivertest.db"
	db, e := walletdb.Create(dbType, dbPath)
	if E.Chk(e) {
		t.Errorf("failed to create database: %v", e)
		return
	}
...
func TestCreateOpenUnsupported(t *testing.T) {
	...
	dbType := "unsupported"
	_, e := walletdb.Create(dbType)
	if e != walletdb.ErrDbUnknownType {
		t.Errorf(
			"expected error not received - got: %v, want %v", e,
