...
func (d *DuOS) GetBalance() mod.DuOSbalance {
	...
	getBalance, err := legacy.GetBalance(&btcjson.GetBalanceCmd{Account: &acct,
		MinConf: &minconf}, d.CtX.WalletServer)
	if err != nil {
	...
	if ok {
		...
		d.SrV.Status.Balance.Balance = bb
	}

	getUnconfirmedBalance, err := legacy.GetUnconfirmedBalance(&btcjson.
		GetUnconfirmedBalanceCmd{Account: &acct}, d.CtX.WalletServer)
	if err != nil {
		d.PushDuOSalert("Error", err.Error(), "error")
	}
	...
	if ok {
		...
		d.SrV.Status.Balance.Unconfirmed = ubb
	}
	return d.SrV.Status.Balance
}
func (d *DuOS) GetTransactions(from, count int, cat string) (txs mod.DuOStransactions) {
	...
	lt, err := d.CtX.WalletServer.ListTransactions(0, 10)
	if err != nil {
		d.PushDuOSalert("Error", err.Error(), "error")
	}
...
func (d *DuOS) GetTransactionsExcertps() (txse mod.DuOStransactionsExcerpts) {
	lt, err := d.CtX.WalletServer.ListTransactions(0, 99999)
	if err != nil {
		d.PushDuOSalert("Error", err.Error(), "error")
	}
...
func (d *DuOS) GetAddressBook() mod.DuOSaddressBook {
	...
	syncBlock := d.CtX.WalletServer.Manager.SyncedTo()
	// Intermediate data for all addresses.
	allAddrData := make(map[string]AddrData)

	// Create an AddrData entry for each active address in the account.
	// Otherwise we'll just get addresses from transactions later.
	sortedAddrs, err := d.CtX.WalletServer.SortedActivePaymentAddresses()
	if err != nil {
	}
	idx := 0
	...
	err = wallet.ExposeUnstableAPI(d.CtX.WalletServer).RangeTransactions(0, endHeight, func(details []wtxmgr.TxDetails) (bool, error) {
		for _, tx := range details {
			for _, cred := range tx.Credits {
				pkScript := tx.MsgTx.TxOut[cred.Index].PkScript
				_, addrs, _, err := txscript.ExtractPkScriptAddrs(
					pkScript, d.CtX.WalletServer.ChainParams())
				if err != nil {
...
func (d *DuOS) DuoSend(wp string, ad string, am float64) string {
	if am > 0 {
		getBlockChain, err := rpc.HandleGetBlockChainInfo(d.CtX.RPCServer, nil, nil)
		if err != nil {
			d.PushDuOSalert("Error", err.Error(), "error")
		...
		if err == nil {
			...
			va, err = legacy.ValidateAddress(&btcjson.
				ValidateAddressCmd{Address: addr.String()}, d.CtX.WalletServer)
			if err != nil {
				d.PushDuOSalert("Error", err.Error(), "error")
...
func (d *DuOS) CreateNewAddress(acctName string) string {
	account, err := d.CtX.WalletServer.AccountNumber(waddrmgr.KeyScopeBIP0044, acctName)
	if err != nil {
	}
	addr, err := d.CtX.WalletServer.NewAddress(account,
...
func (d *DuOS) SaveAddressLabel(address, label string) {
	...
	d.DbS.DbWrite("addressbook", addressHash, mod.AddBook{
		Address: addressHash,
		Label:   label,
	})
