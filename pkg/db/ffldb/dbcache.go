...
const (
	...
	ldbRecordIKeySize = 8
	// These are used to help preallocate space needed for a batch in one allocation instead of letting leveldb itself
	// constantly grow it.
	//
	// This results in far less pressure on the GC and consequently helps prevent the GC from allocating a lot of extra
	// unneeded space.
)

// ldbCacheIter wraps a treap iterator to provide the additional functionality needed to satisfy the
// leveldb iterator.Iterator interface.
...
func (iter *ldbCacheIter) Error() (e error) {
	return nil
}

// SetReleaser is only provided to satisfy the iterator interface as there is no need to override it. This is part of
...
// newLdbCacheIter creates a new treap iterator for the given slice against the pending keys for the passed cache
// snapshot and returns it wrapped in an ldbCacheIter so it can be used as a leveldb iterator.
...
// dbCacheIterator defines an iterator over the key/value pairs in the database cache and underlying database.
...
// skipPendingUpdates skips any keys at the current database iterator position that are being updated by the cache.
//
// The forwards flag indicates the direction the iterator is moving.
...
// current iterator to the appropriate iterator depending on their validity and the order they compare in while taking
// into account the direction flag.
//
// When the iterator is being moved forwards and both iterators are valid, the iterator with the smaller key is chosen
...
// First positions the iterator at the first key/value pair and returns whether or not the pair exists.
//
// This is part of the leveldb iterator. Iterator interface implementation.
func (iter *dbCacheIterator) First() bool {
	// Seek to the first key in both the database and cache iterators and choose the iterator that is both valid and has
	// the smaller key.
	iter.dbIter.First()
...
// Last positions the iterator at the last key/value pair and returns whether or not the pair exists.
//
// This is part of the leveldb iterator. Iterator interface implementation.
func (iter *dbCacheIterator) Last() bool {
	// Seek to the last key in both the database and cache iterators and choose the iterator that is both valid and has
	// the larger key.
	iter.dbIter.Last()
	iter.cacheIter.Last()
...
// Next moves the iterator one key/value pair forward and returns whether or not the pair exists.
//
// This is part of the leveldb iterator. Iterator interface implementation.
...
// Prev moves the iterator one key/value pair backward and returns whether or not the pair exists.
//
// This is part of the leveldb iterator. Iterator interface implementation.
...
// Valid indicates whether the iterator is positioned at a valid key/value pair. It will be considered invalid when the
// iterator is newly created or exhausted.
//
// This is part of the leveldb iterator. Iterator interface implementation.
...
// SetReleaser is only provided to satisfy the iterator interface as there is
...
// Release releases the iterator by removing the underlying treap iterator from the list of active iterators against the
// pending keys treap.
//
// This is part of the leveldb iterator.Iterator interface implementation.
...
//
// This is part of the leveldb iterator.Iterator interface implementation.
func (iter *dbCacheIterator) Error() (e error) {
	return nil
}

// dbCacheSnapshot defines a snapshot of the database cache and underlying database at a particular point in time.
...
// NewIterator returns a new iterator for the snapshot. The newly returned iterator is not pointing to a valid item
// until a call to one of the methods to position it is made.
//
// The slice parameter allows the iterator to be limited to a range of keys.
//
// The start key is inclusive and the limit key is exclusive. Either or both can be nil if the functionality is not
// desired.
...
// dbCache provides a database cache layer backed by an underlying database.
//
// It allows a maximum cache size and flush interval to be specified such that the cache is flushed to the database when
// the cache size exceeds the maximum configured value or it has been longer than the configured interval since the last
// flush.
//
// This effectively provides transaction batching so that callers can commit transactions at will without incurring
// large performance hits due to frequent disk syncs.
type dbCache struct {
	// ldb is the underlying leveldb DB for metadata.
	ldb *leveldb.DB
	// store is used to sync blocks to flat files.
	store *blockStore
	// The following fields are related to flushing the cache to persistent storage.
	//
	// Note that all flushing is performed in an opportunistic fashion.
	//
	// This means that it is only flushed during a transaction or when the database cache is closed.
	//
	// maxSize is the maximum size threshold the cache can grow to before it is flushed.
	//
	// flushInterval is the threshold interval of time that is allowed to pass before the cache is flushed.
	//
	// lastFlush is the time the cache was last flushed. It is used in conjunction with the current time and the flush
	// interval.
	//
	// NOTE: These flush related fields are protected by the database write lock.
	maxSize       uint64
	flushInterval time.Duration
	lastFlush     time.Time
	// The following fields hold the keys that need to be stored or deleted from the underlying database once the cache
	// is full, enough time has passed, or when the database is shutting down.
	//
	// Note that these are stored using immutable treaps to support O(1) MVCC snapshots against the cached data.
	//
	// The cacheLock is used to protect concurrent access for cache updates and snapshots.
	cacheLock    sync.RWMutex
	cachedKeys   *treap.Immutable
	cachedRemove *treap.Immutable
}

// Snapshot returns a snapshot of the database cache and underlying database at a particular point in time.
//
...
func (c *dbCache) Snapshot() (*dbCacheSnapshot, error) {
	...
	// the tree under the lock which is used to atomically swap the root.
	c.cacheLock.RLock()
	cacheSnapshot := &dbCacheSnapshot{
...
// updateDB invokes the passed function in the context of a managed leveldb transaction. Any errors returned from the
...
func (c *dbCache) updateDB(fn func(ldbTx *leveldb.Transaction) error) (e error) {
	...
	if e := fn(ldbTx); dbg.Chk(e) {
		ldbTx.Discard()
		return e
	}
	// Commit the leveldb transaction and convert any errors as needed.
	if e := ldbTx.Commit(); dbg.Chk(e) {
		return convertErr("failed to commit leveldb transaction", e)
	}
	return nil
}

// TreapForEacher is an interface which allows iteration of a treap in ascending order using a user-supplied callback
// for each key/value pair.
//
// It mainly exists so both mutable and immutable treaps can be atomically committed to the database with the same
// function.
...
//
// This involes syncing the block store and replaying all transactions that have been applied to the cache to the
// underlying database.
//
// This function MUST be called with the database write lock held.
func (c *dbCache) flush() (e error) {
	...
	if e := c.store.syncBlocks(); dbg.Chk(e) {
		return e
	}
	// Since the cached keys to be added and removed use an immutable treap, a snapshot is simply obtaining the root of
	// the tree under the lock which is used to atomically swap the root.
	...
	// Perform all leveldb updates using an atomic transaction.
	if e := c.commitTreaps(cachedKeys, cachedRemove); dbg.Chk(e) {
		return e
	}
...
// needsFlush returns whether or not the database cache needs to be flushed to persistent storage based on its current
// size, whether or not adding all of the entries in the passed database transaction would cause it to exceed the
// configured limit, and how much time has elapsed since the last time the cache was flushed.
//
// This function MUST be called with the database write lock held.
func (c *dbCache) needsFlush(tx *transaction) bool {
	...
	// A flush is needed when the size of the database cache exceeds the specified max cache size.
	//
	// The total calculated size is multiplied by 1.
	//
	// 5 here to account for additional memory consumption that will be needed during the flush as well as old nodes in
	// the cache that are referenced by the snapshot used by the transaction.
	snap := tx.snapshot
...
// commitTx atomically adds all of the pending keys to add and remove into the database cache.
//
// When adding the pending keys would cause the size of the cache to exceed the max cache size, or the time since the
// last flush exceeds the configured flush interval, the cache will be flushed to the underlying persistent database.
//
// This is an atomic operation with respect to the cache in that either all of the pending keys to add and remove in the
// transaction will be applied or none of them will. The database cache itself might be flushed to the underlying
// persistent database even if the transaction fails to apply, but it will only be the state of the cache without the
// transaction applied.
//
// This function MUST be called during a database write transaction which in turn implies the database write lock will
// be held.
func (c *dbCache) commitTx(tx *transaction) (e error) {
	...
	// At this point a database flush is not needed, so atomically commit the transaction to the cache.
	//
	// Since the cached keys to be added and removed use an immutable treap, a snapshot is simply obtaining the root of
	// the tree under the lock which is used to atomically swap the root.
	c.cacheLock.RLock()
...
// Close cleanly shuts down the database cache by syncing all data and closing the underlying leveldb database.
//
// This function MUST be called with the database write lock held.
...
// newDbCache returns a new database cache instance backed by the provided leveldb instance.
//
