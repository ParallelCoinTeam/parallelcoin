...
const (
	...
	//
	// This is simply the constant from wire and is only provided here for convenience since wire.MaxBlockHeaderPayload
	// is quite long.
...
var (
	// byteOrder is the preferred byte order used through the database and block files. Sometimes big endian will be
	// used to allow ordered byte sortable integer values.
	...
	// metadataBucketID is the ID of the top-level metadata bucket. It is the value 0 encoded as an unsigned big-endian
	// uint32.
	metadataBucketID = [4]byte{}
	// blockIdxBucketID is the ID of the internal block metadata bucket. It is the value 1 encoded as an unsigned
	// big-endian uint32.
...
func makeDbErr(c database.ErrorCode, desc string, e error) database.DBError {
	return database.DBError{ErrorCode: c, Description: desc, Err: e}
}

// convertErr converts the passed leveldb error into a database error with an equivalent error code and the passed
// description.
//
// It also sets the passed error as the underlying error.
func convertErr(desc string, ldbErr error) database.DBError {
	// Use the driver-specific error code by default.
	//
	// The code below will update this with the converted error if it's recognized.
...
// copySlice returns a copy of the passed slice.
//
// This is mostly used to copy leveldb iterator keys and values since they are only valid until the iterator is moved
// instead of during the entirety of the transaction.
...
// cursor is an internal type used to represent a cursor over key/value pairs and nested buckets of a bucket and
// implements the database.Cursor interface.
type cursor struct {
	bucket      *bucket
...
func (c *cursor) Bucket() database.Bucket {
	...
	if e := c.bucket.tx.checkClosed(); dbg.Chk(e) {
		return nil
	}
	return c.bucket
}
...
//
//   - ErrIncompatibleValue if attempted when the cursor points to a nested bucket
//
...
// skipPendingUpdates skips any keys at the current database iterator position that are being updated by the
// transaction.
//
...
// chooseIterator first skips any entries in the database iterator that are being updated by the transaction and sets
// the current iterator to the appropriate iterator depending on their validity and the order they compare in while
// taking into account the direction flag.
//
// When the cursor is being moved forwards and both iterators are valid, the iterator with the smaller key is chosen and
// vice versa when the cursor is being moved backwards.
func (c *cursor) chooseIterator(forwards bool) bool {
	...
	// Both iterators are valid, so choose the iterator with either the smaller or larger key depending on the forwards
	// flag.
...
// First positions the cursor at the first key/value pair and returns whether or not the pair exists. This function is
...
// Last positions the cursor at the last key/value pair and returns whether or not the pair exists. This function is
// part of the database.
//
// Cursor interface implementation.
...
// Next moves the cursor one key/value pair forward and returns whether or not the pair exists. This function is part of
// the database.
//
// Cursor interface implementation.
...
// Prev moves the cursor one key/value pair backward and returns whether or not the pair exists. This function is part
// of the database.
//
// Cursor interface implementation.
func (c *cursor) Prev() bool {
	...
	// Move the current iterator to the previous entry and choose the iterator that is both valid and has the larger
	// key.
	c.currentIter.Prev()
	return c.chooseIterator(false)
}
...
// rawKey returns the current key the cursor is pointing to without stripping the current bucket prefix or bucket index
// prefix.
...
// cursorFinalizer is either invoked when a cursor is being garbage collected or called manually to ensure the
// underlying cursor iterators are released.
...
func newCursor(b *bucket, bucketID []byte, cursorTyp cursorType) *cursor {
	...
	case ctBuckets:
		...
		//
		// Create an iterator for the both the database and the pending keys which are prefixed by the bucket index
		// identifier and the provided bucket ID.
	...
	default:
		...
		bucketRange := util.BytesPrefix(prefix)
		keyRange := util.BytesPrefix(bucketID)
		// Since both keys and buckets are needed from the database, create an individual iterator for each prefix and
		// then create a merged iterator from them.
		...
		dbIter = iterator.NewMergedIterator(
			iters,
			comparer.DefaultComparer, true,
		)
		// Since both keys and buckets are needed from the pending keys, create an individual iterator for each prefix
		// and then create a merged iterator from them.
		pendingKeyIter := newLdbTreapIter(b.tx, keyRange)
		pendingBucketIter := newLdbTreapIter(b.tx, bucketRange)
		iters = []iterator.Iterator{pendingKeyIter, pendingBucketIter}
		pendingIter = iterator.NewMergedIterator(
			iters,
			comparer.DefaultComparer, true,
		)
	}
...
// bucket is an internal type used to represent a collection of key/value pairs and implements the database.Bucket
// interface.
type bucket struct {
	tx *transaction
	id [4]byte
...
// bucketIndexKey returns the actual key to use for storing and retrieving a child bucket in the bucket index.
//
// This is required because additional information is needed to distinguish nested buckets with the same name.
...
// bucketizedKey returns the actual key to use for storing and retrieving a key for the provided bucket ID.
//
// This is required because bucketizing is handled through the use of a unique prefix per bucket.
...
func (b *bucket) Bucket(key []byte) database.Bucket {
	// Ensure transaction state is valid.
	if e := b.tx.checkClosed(); dbg.Chk(e) {
		return nil
	}
...
func (b *bucket) CreateBucket(key []byte) (database.Bucket, error) {
	...
	// Find the appropriate next bucket ID to use for the new bucket. In the case of the special internal block index,
	// keep the fixed ID.
	var childID [4]byte
	if b.id == metadataBucketID && bytes.Equal(key, blockIdxBucketName) {
		...
		var e error
		childID, e = b.tx.nextBucketID()
		if e != nil {
			return nil, e
		}
	}
	// Add the new bucket to the bucket index.
	if e := b.tx.putKey(bidxKey, childID[:]); dbg.Chk(e) {
...
func (b *bucket) DeleteBucket(key []byte) (e error) {
	...
	// Remove the nested bucket from the bucket index. Any buckets nested under it were already removed above.
...
// transaction has ended will likely result in an access violation. This function is part of the database.Bucket
...
// WARNING: It is not safe to mutate data while iterating with this method.
//
// Doing so may cause the underlying cursor to be invalidated and return unexpected keys.
//
// Returns the following errors as required by the interface contract:
//
//   - ErrTxClosed if the transaction has already been closed
...
// transaction has ended results in undefined behavior. Additionally, the value must NOT be modified by the caller.
//
// This function is part of the database.Bucket interface implementation.
...
// pendingBlock houses a block that will be written to disk when the database transaction is committed.
...
//
// Bucket interface. The transaction provides a root bucket against which all read and writes occur.
type transaction struct {
	...
	//
	// The pendingBlocks map is kept to allow quick lookups of pending data by block hash.
	...
	// Active iterators that need to be notified when the pending keys have been updated so the cursors can properly
	// handle updates to the transaction state.
...
func (tx *transaction) removeActiveIter(iter *treap.Iterator) {
	// An indexing for loop is intentionally used over a range here as range does not reevaluate the slice on each
	// iteration nor does it adjust the index for the modified slice.
...
// addActiveIter adds the passed iterator to the list of active iterators for the pending keys treap.
...
// notifyActiveIters notifies all of the active iterators for the pending keys treap that it has been updated.
...
func (tx *transaction) checkClosed() (e error) {
...
// hasKey returns whether or not the provided key exists in the database while taking into account the current
// transaction state.
...
// putKey adds the provided key to the list of keys to be updated in the database when the transaction is committed.
//
...
func (tx *transaction) putKey(key, value []byte) (e error) {
...
// fetchKey attempts to fetch the provided key from the database cache ( and hence underlying database) while taking
// into account the current transaction state. Returns nil if the key does not exist.
...
// deleteKey adds the provided key to the list of keys to be deleted from the database when the transaction is
// committed.
//
// The notify iterators flag is useful to delay notifying iterators about the changes during bulk deletes.
//
// NOTE: This function must only be called on a writable transaction.
//
// Since it is an internal helper function, it does not check.
...
func (tx *transaction) nextBucketID() ([4]byte, error) {
	...
	if e := tx.putKey(curBucketIDKeyName, nextBucketID[:]); dbg.Chk(e) {
		return [4]byte{}, e
	}
...
func (tx *transaction) hasBlock(hash *chainhash.Hash) bool {
	// Return true if the block is pending to be written on commit since it exists from the viewpoint of this
	// transaction.
...
//
// There are no checks to ensure the block connects to a previous block, contains double spends, or any additional
// functionality such as transaction indexing.
//
...
func (tx *transaction) StoreBlock(block *u.Block) (e error) {
	...
	if e != nil {
		str := fmt.Sprintf(
			"failed to get serialized bytes for block %s",
			blockHash,
		)
		return makeDbErr(database.ErrDriverSpecific, str, e)
	}
	// Add the block to be stored to the list of pending blocks to store when the transaction is committed.
	//
	// Also add it to pending blocks map so it is easy to determine the block is pending based on the block hash.
	if tx.pendingBlocks == nil {
		tx.pendingBlocks = make(map[chainhash.Hash]int)
	}
	tx.pendingBlocks[*blockHash] = len(tx.pendingBlockData)
	tx.pendingBlockData = append(
		tx.pendingBlockData, pendingBlock{
			hash:  blockHash,
			bytes: blockBytes,
		},
	)
...
// fetchBlockRow fetches the metadata stored in the block index for the provided hash. It will return ErrBlockNotFound
...
//
// NOTE: The data returned by this function is only valid during a database transaction. Attempting to access it after a
// transaction has ended results in undefined behavior.
//
...
func (tx *transaction) FetchBlockHeader(hash *chainhash.Hash) ([]byte, error) {
	return tx.FetchBlockRegion(
		&database.BlockRegion{
			Hash:   hash,
			Offset: 0,
			Len:    blockHdrSize,
		},
...
func (tx *transaction) FetchBlock(hash *chainhash.Hash) ([]byte, error) {
	...
	blockRow, e := tx.fetchBlockRow(hash)
	if e != nil {
		return nil, e
	}
	location := deserializeBlockLoc(blockRow)
	// Read the block from the appropriate location. The function also performs a checksum over the data to detect data
	// corruption.
	blockBytes, e := tx.db.store.readBlock(hash, location)
	if e != nil {
		return nil, e
	}
...
func (tx *transaction) FetchBlocks(hashes []chainhash.Hash) ([][]byte, error) {
	...
	// failure case, however callers will not typically be calling this function with invalid values, so optimize for
	// the common case. Load the blocks.
	blocks := make([][]byte, len(hashes))
	for i := range hashes {
		var e error
		blocks[i], e = tx.FetchBlock(&hashes[i])
		if e != nil {
			return nil, e
		}
	}
...
// fetchPendingRegion attempts to fetch the provided region from any block which are pending to be written on commit.
//
...
func (tx *transaction) fetchPendingRegion(region *database.BlockRegion) ([]byte, error) {
	...
	if endOffset < region.Offset || endOffset > blockLen {
		str := fmt.Sprintf(
			"block %s region offset %d, length %d "+
...
//
// For example, it is possible to directly extract Bitcoin transactions and/or scripts from a block with this function.
...
func (tx *transaction) FetchBlockRegion(region *database.BlockRegion) ([]byte, error) {
	...
	// When the block is pending to be written on commit return the bytes from there.
	if tx.pendingBlocks != nil {
		regionBytes, e := tx.fetchPendingRegion(region)
		if e != nil {
			return nil, e
		}
		if regionBytes != nil {
	...
	// Lookup the location of the block in the files from the block index.
	blockRow, e := tx.fetchBlockRow(region.Hash)
	if e != nil {
		return nil, e
	}
	location := deserializeBlockLoc(blockRow)
	...
	regionBytes, e := tx.db.store.readBlockRegion(
...
//
// For example, it is possible to directly extract Bitcoin transactions and/or scripts from various blocks with this
...
//
//   - ErrBlockRegionInvalid if one or more region exceed the bounds of the associated block
//
//   - ErrTxClosed if the transaction has already been closed
//
//   - ErrCorruption if the database has somehow become corrupted
...
func (tx *transaction) FetchBlockRegions(regions []database.BlockRegion) ([][]byte, error) {
	...
	// fetch list which would be faster in the failure case, however callers will not typically be calling this function
	// with invalid values, so optimize for the common case.
	//
	...
	// by reducing the number of file open/closes and random accesses needed.
	//
	// The fetchList is intentionally allocated with a cap because some of the regions might be fetched from the pending
	// blocks and hence there is no need to fetch those from disk.
	...
	for i := range regions {
		...
		if endOffset < region.Offset || endOffset > location.blockLen {
			str := fmt.Sprintf(
				"block %s region offset %d, length "+
...
// close marks the transaction closed then releases any pending data, the underlying snapshot, the transaction read
// lock, and the write lock when the transaction is writable.
func (tx *transaction) close() {
	...
	// Release the writer lock for writable transactions to unblock any other write transaction which are possibly
	// waiting.
	if tx.writable {
...
// writePendingAndCommit writes pending block data to the flat block files, updates the metadata with their locations as
// well as the new current write location, and commits the metadata to the memory database cache.
//
...
func (tx *transaction) writePendingAndCommit() (e error) {
	// Save the current block store write position for potential rollback.
	//
	// These variables are only updated here in this function and there can only be one write transaction active at a
	// time, so it's safe to store them for potential rollback.
	...
	for _, blockData := range tx.pendingBlockData {
		...
		location, e := tx.db.store.writeBlock(blockData.bytes)
		if e != nil {
			rollback()
			return e
		}
		// Add a record in the block index for the block. The record includes the location information needed to locate
		// the block on the filesystem as well as the block header since they are so commonly needed.
		blockRow := serializeBlockLoc(location)
		e = tx.blockIdxBucket.Put(blockData.hash[:], blockRow)
		if e != nil {
			rollback()
			return e
		}
	...
	if e := tx.metaBucket.Put(writeLocKeyName, writeRow); dbg.Chk(e) {
		rollback()
		return convertErr("failed to store write cursor", e)
	}
	// Atomically update the database cache.
	//
	// The cache automatically handles flushing to the underlying persistent storage database.
	return tx.db.cache.commitTx(tx)
}

// Commit commits all changes that have been made to the root metadata bucket and all of its sub-buckets to the database
// cache which is periodically synced to persistent storage. In addition, it commits all new blocks directly to
// persistent storage bypassing the db cache.
//
// Blocks can be rather large so this help increase the amount of cache available for the metadata updates and is safe
// since blocks are immutable.
//
// This function is part of the database.Tx interface implementation.
...
// Type returns the database driver type the current database instance was created with. This function is part of the
...
//
// This function is only separate because it returns the internal transaction which is used by the managed transaction
// code while the database method returns the interface.
func (db *db) begin(writable bool) (*transaction, error) {
	// Whenever a new writable transaction is started, grab the write lock to ensure only a single write transaction can
	// be active at the same time.
	//
	// This lock will not be released until the transaction is closed ( via Rollback or Commit).
	if writable {
		db.writeLock.Lock()
	}
	// Whenever a new transaction is started, grab a read lock against the database to ensure Close will wait for the
...
// control the life-cycle of the transaction.
//
// As the documentation on Begin calls out, callers opting to use manual
// transactions will have to ensure the transaction is rolled back on panic if
// it desires that functionality as well or the database will fail to close
// since the read-lock will never be released.
...
// transaction with the root bucket for the namespace.
//
...
func (db *db) View(fn func(database.Tx) error) (e error) {
	// Start a read-only transaction.
	tx, e := db.begin(false)
	if e != nil {
		return e
	}
	// Since the user-provided function might panic, ensure the transaction releases
	// all mutexes and resources.
	//
	// There is no guarantee the caller won't use recover and keep going.
	//
	// Thus, the database must still be in a usable state on panics due to caller
	// issues.
	...
	tx.managed = false
	if e != nil {
		// The error is ignored here because nothing was written yet and regardless of a rollback failure, the tx is
		// closed now anyways.
		_ = tx.Rollback()
		return e
	}
...
// Update invokes the passed function in the context of a managed read -write transaction with the root bucket for the
// namespace.
//
// Any errors returned from the user-supplied function will cause the transaction to be rolled back and are returned
// from this function.
//
// Otherwise, the transaction is committed when the user-supplied function returns a nil error. This function is part of
// the database. DB interface implementation.
func (db *db) Update(fn func(database.Tx) error) (e error) {
	// Start a read-write transaction.
	tx, e := db.begin(true)
	if e != nil {
		return e
	}
	// Since the user-provided function might panic, ensure the transaction releases all mutexes and resources.
	//
	// There is no guarantee the caller won't use recover and keep going.
	//
	// Thus, the database must still be in a usable state on panics due to caller issues.
	defer rollbackOnPanic(tx)
	tx.managed = true
	e = fn(tx)
	tx.managed = false
...
//
// It will block until all database transactions have been finalized ( rolled back or committed). This function is part
// of the database.
//
...
func (db *db) Close() (e error) {
	...
	db.closed = true
	// NOTE: Since the above lock waits for all transactions to finish and prevents any new ones from being started, it
	// is safe to flush the cache and clear all state without the individual locks. Close the database cache which will
	// flush any existing entries to disk and close the underlying leveldb database.
	//
	// Any error is saved and returned at the end after the remaining cleanup since the database will be marked closed
...
func initDB(ldb *leveldb.DB) (e error) {
	...
	//
	// NOTE: Since buckets are virtualized through the use of prefixes, there is no need to store the bucket index data
	// for the metadata bucket in the database. However, the first bucket ID to use does need to account for it to
	...
	if e := ldb.Write(batch, nil); dbg.Chk(e) {
		str := fmt.Sprintf(
...
func openDB(dbPath string, network wire.BitcoinNet, create bool) (database.DB, error) {
	...
	if !dbExists {
		// The error can be ignored here since the call to leveldb. OpenFile will fail if the directory couldn't be
		// created.
	...
	ldb, e := leveldb.OpenFile(metadataDbPath, &opts)
	if e != nil {
		return nil, convertErr(e.Error(), e)
	}
	// Create the block store which includes scanning the existing flat block files to find what the current write
	// cursor position is according to the data that is actually on disk.
	//
