...
// writeData is a helper function for writeBlock which writes the provided data at the current write offset and updates the write cursor accordingly.  The field name parameter is only used when there is an error to provide a nicer error message.
// The write cursor will be advanced the number of bytes actually written in the event of failure.
// NOTE: This function MUST be called with the write cursor current file lock held and must only be called during a write transaction so it is effectively locked for writes.  Also, the write cursor current file must NOT be nilog.
...
func (s *blockStore) writeBlock(rawBlock []byte) (blockLocation, error) {
	...
	fullLen := blockLen + 12
	// Move to the next block file if adding the new block would exceed the max allowed size for the current block file.  Also detect overflow to be paranoid, even though it isn't possible currently, numbers might change in the future to make it possible.
	// NOTE: The writeCursor.offset field isn't protected by the mutex since it's only read/changed during this function which can only be called during a write transaction, of which there can be only one at a time.
	wc := s.writeCursor
	...
	if finalOffset < wc.curOffset || finalOffset > s.maxBlockFileSize {
		...
		// Close the current write file to force a read-only reopen with LRU tracking.  The close is done under the write lock for the file to prevent it from being closed out from under any readers currently reading from it.
		wc.Lock()
		wc.curFile.Lock()
	...
	// Open the current file if needed.  This will typically only be the case when moving to the next file to write to or on initial database load.  However, it might also be the case if rollbacks happened after file writes started during a transaction commit.
	if wc.curFile.file == nil {
...
func (s *blockStore) readBlock(hash *chainhash.Hash, loc blockLocation) ([]byte, error) {
	...
	// Calculate the checksum of the read data and ensure it matches the serialized checksum.  This will detect any data corruption in the flat file without having to do much more expensive merkle root calculations on the loaded block.
...
// syncBlocks performs a file system sync on the flat file associated with the store's current write cursor.  It is safe to call even when there is not a current write file in which case it will have no effect.
// This is used when flushing cached metadata updates to disk to ensure all the block data is fully written before updating the metadata.  This ensures the metadata and block data can be properly reconciled in failure scenarios.
...
// handleRollback rolls the block files on disk back to the provided file number and offset.  This involves potentially deleting and truncating the files that were partially written.
...
// In either case, the write cursor will be repositioned to the old block file offset regardless of any other errors that occur while attempting to undo
// writes.
