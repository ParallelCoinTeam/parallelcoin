...
reconcileDB(pdb *db, create bool) (database.DB, error) {
	...
	if wc.curFileNum > curFileNum || (wc.curFileNum == curFileNum &&
		...
		log.L.Warn("detected unclean shutdown - repairing")
		log.L.Debugf("metadata claims file %d, "+
		...
		log.L.Debug("database sync complete")
	}
	...
	if wc.curFileNum < curFileNum || (wc.curFileNum == curFileNum &&
		...
		log.L.Warn("***Database corruption detected***:", str)
