...
const (
	...
	ErrCorruption
	// Errors related to database transactions. ErrTxClosed indicates an attempt was made to commit or rollback a
	// transaction that has already had one of those operations performed.
	ErrTxClosed
	// ErrTxNotWritable indicates an operation that requires write access to the database was attempted against a
	...
	ErrKeyRequired
	// ErrKeyTooLarge indicates an attmempt to insert a key that is larger than the max allowed key size. The max key
	// size depends on the specific backend driver being used. As a general rule, key sizes should be relatively, so
	// this should rarely be an issue.
	ErrKeyTooLarge
	// ErrValueTooLarge indicates an attmpt to insert a value that is larger than max allowed value size. The max key
	// size depends on the specific backend driver being used.
	ErrValueTooLarge
	// ErrIncompatibleValue indicates the value in question is invalid for the specific requested operation. For
	// example, trying create or delete a bucket with an existing non-bucket key, attempting to create or delete a
	// non-bucket key with an existing bucket key, or trying to delete a value via a cursor when it points to a nested
	// bucket.
	...
	ErrBlockExists
	// ErrBlockRegionInvalid indicates a region that exceeds the bounds of the specified block was requested. When the
	// hash provided by the region does not correspond to an existing block, the error will be ErrBlockNotFound instead.
	ErrBlockRegionInvalid
	// Support for driver-specific errors. ErrDriverSpecific indicates the Err field is a driver-specific error. This
	// provides a mechanism for drivers to plug-in their own custom errors for any situations which aren't already
	// covered by the error codes provided by this package.
	ErrDriverSpecific
...
// DBError provides a single type for errors that can happen during database operation. It is used to indicate several
// types of failures including errors with caller requests such as specifying invalid block regions or attempting to
// access data against closed database transactions, driver errors, errors retrieving data, and errors communicating
// with database servers.
//
// The caller can use type assertions to determine if an error is an DBError and access the ErrorCode field to ascertain
// the specific reason for the failure.
//
// The ErrDriverSpecific error code will also have the Err field set with the underlying error. Depending on the backend
// driver, the Err field might be set to the underlying error for other error codes as well.
type DBError struct {
...
// makeError creates an DBError given a set of arguments. The error code must be one of the error codes provided by this
// package.
