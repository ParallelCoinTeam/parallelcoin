...
// Add adds the passed inventory to the map and handles eviction of the oldest item if adding the new item would exceed
// the max limit. Adding an existing item makes it the most recently used item.
//
// This function is safe for concurrent access.
func (m *mruInventoryMap) Add(iv *wire.InvVect) {
	...
	// Evict the least recently used entry (back of the list) if the the new entry would exceed the size limit for the
	// map. Also reuse the list node so a new one doesn't have to be allocated.
...
// newMruInventoryMap returns a new inventory map that is limited to the number of entries specified by limit. When the
// number of entries exceeds the limit, the oldest (least recently used) entry will be removed to make room for the new
// entry.
