...
func mockRemotePeer() error {
	...
	go func() {
		...
		if err != nil {
			log.L.Errorf("Accept: error %v", err)
			return
		}
...
func Example_newOutboundPeer() {
	...
	if err := mockRemotePeer(); err != nil {
		log.L.Errorf("mockRemotePeer: unexpected error %v", err)
		return
	}
	...
	if err != nil {
		log.L.Errorf("NewOutboundPeer: error %v", err)
		return
	}
	// Establish the connection to the peer address and mark it connected.
	conn, err := net.Dial("tcp", p.Addr())
	if err != nil {
		log.L.Errorf("net.Dial: error %v", err)
		return
	}
	p.AssociateConnection(conn)
	...
	case <-time.After(time.Second * 1):
		log.L.Error("Example_peerConnection: verack timeout")
	}
	// Disconnect the peer.
	p.Disconnect()
	p.WaitForDisconnect()
