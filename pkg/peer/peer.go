...
func (p *Peer) UpdateLastAnnouncedBlock(blkHash *chainhash.Hash) {
	L.Trace("updating last blk for peer", p.addr, ",", blkHash)
	p.statsMtx.Lock()
...
func (p *Peer) stallHandler() {
	L.Trace("starting stallHandler for", p.addr)
...
cleanup:
	...
	L.Trace("peer stall handler done for", p)
}

// inHandler handles all incoming messages for the peer.
// It must be run as a goroutine.
func (p *Peer) inHandler() {
	L.Trace("starting inHandler for", p.addr)
	// The timer is stopped when a new message is received and reset after it
	// is processed.
	idleTimer := time.AfterFunc(idleTimeout, func() {
		L.Warnf("peer %s no answer for %s -- disconnecting", p, idleTimeout)
		p.Disconnect()
	})
out:
	for atomic.LoadInt32(&p.disconnect) == 0 {
		...
		if err != nil {
			...
			if p.isAllowedReadError(err) {
				L.Errorf("allowed test error from %s: %v", p, err)
		...
		case *wire.MsgVerAck:
			...
			// if p.verAckReceived {
			// 	L.Infof("already received 'verack' from peer %v"+
			// 		" -- disconnecting", p)
			// 	break out
			// }
	...
	p.Disconnect()
	close(p.inQuit)
	L.Trace("peer input handler done for", p)
}
...
func (p *Peer) queueHandler() {
	L.Trace("starting queueHandler for", p.addr)
...
cleanup:
	...
	close(p.queueQuit)
	L.Trace("peer queue handler done for", p)
}
...
func (p *Peer) outHandler() {
	L.Trace("starting outHandler for", p.addr)
out:
	for {
		select {
		case msg := <-p.sendQueue:
			...
			if err != nil {
				...
				if p.shouldLogWriteError(err) {
					L.Errorf("failed to send message to %s: %v", p, err)
				}
				if msg.doneChan != nil {
					msg.doneChan <- struct{}{}
				}
...
cleanup:
	...
	close(p.outQuit)
	L.Trace("peer output handler done for", p)
}

// pingHandler periodically pings the peer.  It must be run as a goroutine.
func (p *Peer) pingHandler() {
	L.Trace("starting pingHandler for", p.addr)
...
out:
	for {
		...
		case <-pingTicker.C:
			...
			if err != nil {
				L.Error(err)
				L.Errorf("not sending ping to %s: %v", p, err)
				continue
			}
...
func (p *Peer) readRemoteVersionMsg() error {
	if p.versionKnown {
		L.Debug("received version previously, dropping")
		return nil
	}
	...
	p.flagsMtx.Unlock()
	L.Tracef("negotiated protocol version %d for peer %s",
		p.protocolVersion, p)
...
func (p *Peer) AssociateConnection(conn net.Conn) {
	...
	if p.inbound {
		...
		if err != nil {
			L.Error(err)
			L.Error("cannot create remote net address:", err)
			p.Disconnect()
			return
		}
		p.na = na
	...
	go func() {
		if err := p.start(); err != nil {
			L.Debugf("cannot start peer %v: %v", p, err)
			p.Disconnect()
		}
	}()
}
