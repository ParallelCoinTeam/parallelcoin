...
func main() {
	...
	out.WriteString("\n\n")
	if e := genDir(iconsDir); dbg.Chk(e) {
		ftl.Ln(err)
	}
	_, _ = fmt.Fprintf(out,
	...
	if *outDir != "" {
		if e := os.MkdirAll(*outDir, 0775); err != nil && !os.IsExist(err) {
			ftl.Ln(err)
		}
	}
	raw := out.Bytes()
	...
	// formatted := raw
	if e := ioutil.WriteFile(filepath.Join(*outDir, "data.go"), formatted, 0644); dbg.Chk(e) {
	...
	}
	{
		...
		if e := ioutil.WriteFile(filepath.Join(*outDir, "data_test.go"), formatted, 0644); dbg.Chk(e) {
			Fatalf("WriteFile failed: %s\n", err)
		}
	}
}
...
func genDir(dirName string) (e error) {
	...
	for _, info := range infos {
		...
		var size int
		if n, e := fmt.Sscanf(nameParts[2], "%dpx.svg", &size); err != nil || n != 1 {
			continue
		}
	...
	for _, baseName := range baseNames {
		...
		if e = genFile(f, baseName, float32(sizes[baseName])); dbg.Chk(e) {
			failures = append(failures, fmt.Sprintf("%s: %v", path, err))
			continue
		}
	}
...
func genFile(svgData []byte, baseName string, outSize float32) (e error) {
	...
	g := &SVG{}
	if e := xml.Unmarshal(svgData, g); dbg.Chk(e) {
		return err
	}
	...
	for _, p := range g.Paths {
		if e := genPath(&enc, p, adjs, outSize, size, offset, g.Circles); dbg.Chk(e) {
			return err
		}
		g.Circles = nil
	}
	...
	if len(g.Circles) != 0 {
		if e := genPath(&enc, &Path{}, adjs, outSize, size, offset, g.Circles); dbg.Chk(e) {
			return err
		}
		g.Circles = nil
	}
...
func parseColor(col string) (color.RGBA, error) {
	...
	for i := range elems {
		e, e := strconv.ParseUint(col[i*2:i*2+2], 16, 8)
		if e != nil  {
...
func genPath(enc *iconvg.Encoder, p *Path, adjs map[float32]uint8, outSize, size float32, offset f32.Vec2, circles []Circle) (e error) {
	adj := uint8(0)
	opacity := float32(1)
	if p.Opacity != nil {
		opacity = *p.Opacity
	...
	if p.D != "" {
		needStartPath = false
		if e := genPathData(enc, adj, p.D, outSize, size, offset); dbg.Chk(e) {
			return err
		}
	}
