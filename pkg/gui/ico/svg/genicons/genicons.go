...
func main() {
	...
	out.WriteString("\n\n")
	if e := genDir(iconsDir); err.Chk(e) {
		ftl.Ln(e)
	}
	_, _ = fmt.Fprintf(out,
	...
	if *outDir != "" {
		if e := os.MkdirAll(*outDir, 0775); err != nil && !os.IsExist(e) {
			ftl.Ln(e)
		}
	}
	raw := out.Bytes()
	...
	// formatted := raw
	if e := ioutil.WriteFile(filepath.Join(*outDir, "data.go"), formatted, 0644); err.Chk(e) {
	...
	}
	{
		...
		if e := ioutil.WriteFile(filepath.Join(*outDir, "data_test.go"), formatted, 0644); err.Chk(e) {
			Fatalf("WriteFile failed: %s\n", err)
		}
	}
}
...
func genDir(dirName string) (e error) {
	...
	for _, baseName := range baseNames {
		...
		if e = genFile(f, baseName, float32(sizes[baseName])); err.Chk(e) {
			failures = append(failures, fmt.Sprintf("%s: %v", path, err))
			continue
		}
	}
