// generated by go run github.com/p9c/pod/pkg/gui/p9/poolgen/poolgen.go; DO NOT EDIT
//
...
func (p *Pool) GetBool() (out *Bool) {
	if len(p.bools) >= p.boolsInUse {
		for i := 0; i < 10; i++ {
			p.bools = append(p.bools, p.th.Bool(false))
		}
	}
...
func (p *Pool) GetList() (out *List) {
	if len(p.lists) >= p.listsInUse {
		for i := 0; i < 10; i++ {
			p.lists = append(p.lists, p.th.List())
		}
	}
...
func (p *Pool) GetCheckable() (out *Checkable) {
	if len(p.checkables) >= p.checkablesInUse {
		for i := 0; i < 10; i++ {
			p.checkables = append(p.checkables, p.th.Checkable())
		}
	}
...
func (p *Pool) GetClickable() (out *Clickable) {
	if len(p.clickables) >= p.clickablesInUse {
		for i := 0; i < 10; i++ {
			p.clickables = append(p.clickables, p.th.Clickable())
		}
	}
...
func (p *Pool) GetEditor() (out *Editor) {
	if len(p.editors) >= p.editorsInUse {
		for i := 0; i < 10; i++ {
			p.editors = append(p.editors, p.th.Editor())
		}
	}
...
func (p *Pool) GetIncDec() (out *IncDec) {
	if len(p.incDecs) >= p.incDecsInUse {
		for i := 0; i < 10; i++ {
			p.incDecs = append(p.incDecs, p.th.IncDec())
		}
	}
