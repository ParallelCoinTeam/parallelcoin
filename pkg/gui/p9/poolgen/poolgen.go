...
func main() {
	var out string
	out += `// generated by go run github.com/p9c/pod/pkg/gui/p9/poolgen/poolgen.go; DO NOT EDIT
//
...
func (p *Pool) Get` + types[i].name + `() (out *` + types[i].name + `) {
	if len(p.` + types[i].sliceName + `) >= p.` + types[i].sliceName + `InUse {
		for i := 0; i < 10; i++ {
			p.` + types[i].sliceName + ` = append(p.` + types[i].sliceName + `, p.th.` + types[i].constructor + `)
		}
	}
