...
func Start(w *worker.Worker) {
	...
	var n int
	var e error
	if n, e = w.StdConn.Write([]byte("run ")); n < 1 || dbg.Chk(e) {
...
func Stop(w *worker.Worker) {
	...
	var n int
	var e error
	if n, e = w.StdConn.Write([]byte("stop")); n < 1 || dbg.Chk(e) {
		dbg.Ln("failed to write", w.Args)
	}
}
...
func Kill(w *worker.Worker) {
	...
	if n, e = w.StdConn.Write([]byte("kill")); n < 1 || dbg.Chk(e) {
		dbg.Ln("failed to write")
		return
	}
	// close(w.Quit)
	// w.StdConn.Quit.Q()
	if e = w.Cmd.Wait(); dbg.Chk(e) {
	}
