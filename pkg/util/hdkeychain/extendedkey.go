...
var (
	...
	ErrNotPrivExtKey = errors.New("unable to create private keys from a " +
		"public extended key")
	// ErrInvalidChild describes an error in which the child at a specific index is invalid due to the derived key falling outside of the valid range for secp256k1 private keys.  This error indicates the caller should simply ignore the invalid child extended key at this index and increment to the next index.
	ErrInvalidChild = errors.New("the extended key at this index is invalid")
	// ErrUnusableSeed describes an error in which the provided seed is not usable due to the derived key falling outside of the valid range for secp256k1 private keys.  This error indicates the caller must choose another seed.
	...
	ErrInvalidKeyLen = errors.New("the provided serialized extended key " +
		"length is invalid")
	// masterKey is the master key used along with a random seed used to generate the master node in the hierarchical tree.
...
// next index.
func (k *ExtendedKey) Child(i uint32) (ek *ExtendedKey, err error) {
	...
	if _, err = hmac512.Write(data); slog.Check(err) {
	}
	ilr := hmac512.Sum(nil)
...
//public keys.
func (k *ExtendedKey) Neuter() (ek *ExtendedKey, err error) {
	...
	var version []byte
	if version, err = config.HDPrivateKeyToPublicKeyID(k.version); slog.Check(err) {
		return
	}
...
func (k *ExtendedKey) ECPubKey() (pk *ec.PublicKey, err error) {
...
func (k *ExtendedKey) ECPrivKey() (privKey *ec.PrivateKey, err error) {
	if !k.isPrivate {
...
func (k *ExtendedKey) Address(net *netparams.Params) (pkh *util.AddressPubKeyHash, err error) {
...
func NewMaster(seed []byte, net *netparams.Params) (ek *ExtendedKey, err error) {
	...
	if _, err = hmac512.Write(seed); slog.Check(err) {
	}
	lr := hmac512.Sum(nil)
	// Split "I" into two 32-byte sequences Il and Ir where:
...
func NewKeyFromString(key string) (ek *ExtendedKey, err error) {
...
func GenerateSeed(length uint8) (buf []byte, err error) {
	// Per [BIP32], the seed must be in range [MinSeedBytes, MaxSeedBytes].
	if length < MinSeedBytes || length > MaxSeedBytes {
		return nil, ErrInvalidSeedLen
	}
	buf = make([]byte, length)
	if _, err = rand.Read(buf); slog.Check(err) {
		return
	}
	return
}
