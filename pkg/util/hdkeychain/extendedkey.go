...
// References:
//   [BIP32]: BIP0032 - Hierarchical Deterministic Wallets   https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki
import (
	"bytes"
	"crypto/hmac"
	"crypto/rand"
...
var (
	...
	// ErrUnusableSeed describes an error in which the provided seed is not usable due to the derived key falling
...
// ExtendedKey houses all the information needed to support a hierarchical deterministic extended key. See the package
// overview documentation for more details on how to use extended keys.
...
// NewExtendedKey returns a new instance of an extended key with the given fields. No error checking is performed here
// as it's only intended to be a convenience method used to create a populated struct.
//
...
// pubKeyBytes returns bytes for the serialized compressed public key associated with this extended key in an efficient
// manner including memoization as necessary.
//
// When the extended key is already a public key, the key is simply returned as is since it's already in the correct
// form. However, when the extended key is a private key, the public key will be calculated and memoized so future
...
// IsPrivate returns whether or not the extended key is a private extended key. A private extended key can be used to
// derive both hardened and non-hardened child private and public extended keys. A public extended key can only be used
// to derive non-hardened child public extended keys.
func (k *ExtendedKey) IsPrivate() bool {
	return k.isPrivate
}

// Depth returns the current derivation level with respect to the root. The root key has depth zero, and the field has a
// maximum of 255 due to how depth is serialized.
...
// extended key. It is only possible to derive a hardended extended key from a private extended key. Consequently, this
// function will return ErrDeriveHardFromPublic if a hardened child extended key is requested from a public extended
// key.
//
...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	// The data used to derive the child key depends on whether or not the child is hardened per [BIP32].
	//
	...
	// The algorithm used to derive the child key depends on whether or not a private or public child is being derived.
	//
...
// ECPrivKey converts the extended key to a ec private key and returns it. As you might imagine this is only possible if
// the extended key is a private extended key (as determined by the IsPrivate function). The ErrNotPrivExtKey error will
// be returned if this function is called on a public extended key.
...
func (k *ExtendedKey) String() string {
	...
	//
	//   version (4) || depth (1) || parent fingerprint (4)) || child num (4) || chain code (32) || key data (33) ||
	//     checksum (4)
...
// Zero manually clears all fields and bytes in the extended key. This can be used to explicitly clear key material from
// memory for enhanced security against memory scraping. This function only clears this particular key and not any
