...
const (
	// staticDepth is the size of the static array to use for keeping track of the parent stack during treap iteration.  Since a treap has a very high probability that the tree height is logarithmic, it is exceedingly unlikely that the parent stack will ever exceed this size even for extremely large numbers of items.
	staticDepth = 128
	// nodeFieldsSize is the size the fields of each node takes excluding the contents of the key and value.  It assumes 64-bit pointers so technically it is smaller on 32-bit platforms, but overestimating the size in that case is acceptable since it avoids the need to import unsafe.  It consists of 24-bytes for each key and value + 8 bytes for each of the priority, left, and right fields (24*2 + 8*3).
	nodeFieldsSize = 72
)

var (
	// emptySlice is used for keys that have no value associated with them so callers can distinguish between a key that does not exist and one that has no value associated with it.
