...
func TestMutableEmptyIterator(t *testing.T) {
	...
	if iter.Valid() {
		t.ftl.Ln("Valid: iterator should be exhausted")
	}
	// Ensure First and Last on empty iterator report it as exhausted.
	if iter.First() {
		t.ftl.Ln("First: iterator should be exhausted")
	}
	if iter.Last() {
		t.ftl.Ln("Last: iterator should be exhausted")
	}
	// Ensure Next and Prev on empty iterator report it as exhausted.
	if iter.Next() {
		t.ftl.Ln("Next: iterator should be exhausted")
	}
	if iter.Prev() {
		t.ftl.Ln("Prev: iterator should be exhausted")
	}
...
func TestIteratorUpdates(t *testing.T) {
	...
	if !iter.Next() {
		t.ftl.Ln("ForceReseek.Next: unexpected exhausted iterator")
	}
	...
	if !iter.Prev() {
		t.ftl.Ln("ForceReseek.Prev: unexpected exhausted iterator")
	}
	wantKey = serializeUint32(4)
	gotKey = iter.Key()
	if !bytes.Equal(gotKey, wantKey) {
