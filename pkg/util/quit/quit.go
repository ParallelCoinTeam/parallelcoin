...
func (c C) Q() {
	...
	if !testChanIsClosed(c) {
		trc.Ln("closing chan from "+loc, logi.Caller("from", 1))
		close(c)
	} else {
		trc.Ln(logi.Caller("from", 1), "#### channel", loc, "was already closed")
	}
	mx.Unlock()
	// PrintChanState()
}
...
func (c C) Wait() <-chan struct{} {
	// trc.Ln(logi.Caller(">>> waiting on quit channel at", 1))
	return c
}
