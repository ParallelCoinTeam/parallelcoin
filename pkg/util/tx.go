...
// TxIndexUnknown is the value returned for a transaction index that is unknown. This is typically because the
// transaction has not been inserted into a block yet.
...
// Hash returns the hash of the transaction. This is equivalent to calling TxHash on the underlying wire.MsgTx, however
// it caches the result so subsequent calls are more efficient.
...
// Index returns the saved index of the transaction within a block. This value will be TxIndexUnknown if it hasn't
...
// NewTxFromReader returns a new instance of a bitcoin transaction given a Reader to deserialize the transaction. See
// Tx.
