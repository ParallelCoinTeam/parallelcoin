...
func ProvideSeed() ([]byte, error) {
	...
	for {
		log.Print("Enter existing wallet seed: ")
		...
		if err != nil || len(seed) < hdkeychain.MinSeedBytes ||
			...
			log.L.Errorf("Invalid seed specified.  "+
...
func promptPass(reader *bufio.Reader, prefix string, confirm bool) ([]byte, error) {
	...
	for {
		...
		if !bytes.Equal(pass, confirm) {
			log.L.Error("The entered passphrases do not match")
			continue
		}
		return pass, nil
	}
...
func PrivatePass(reader *bufio.Reader, legacyKeyStore *keystore.Store) ([]byte, error) {
	...
	log.L.Warn("You have an existing legacy wallet.  " +
...
func PublicPass(reader *bufio.Reader, privPass []byte,
	...
	log.Println("NOTE: Use the --walletpass option to configure your " +
		"public passphrase.")
	return pubPass, nil
}
...
func Seed(reader *bufio.Reader) ([]byte, error) {
	...
	if !useUserSeed {
		...
		log.Printf("\n%x\n\n", seed)
		log.Print("IMPORTANT: Keep the seed in a safe place as you will NOT be" +
			" able to restore your wallet without it.\n\n")
		log.Print("Please keep in mind that anyone who has access to the seed" +
		...
		for {
			log.Print(`Once you have stored the seed in a safe ` +
	...
	for {
		...
		if err != nil || len(seed) < hdkeychain.MinSeedBytes ||
			len(seed) > hdkeychain.MaxSeedBytes {
			log.L.Errorf("Invalid seed specified.  Must be a "+
