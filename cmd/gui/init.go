...
func (wg *WalletGUI) loaderCreateWallet() l.Widget {
	createWalletLayoutList := []l.Widget{
		...
		wg.th.CheckBox(wg.bools["encryption"].SetOnChange(func(b bool) {
			...
			Fn,
		wg.th.CheckBox(wg.bools["seed"].SetOnChange(func(b bool) {
			Debug("change state to", b)
		})).
			IconColor("Primary").
			TextColor("DocText").
		...
		wg.th.CheckBox(wg.bools["testnet"].SetOnChange(func(b bool) {
			Debug("change state to", b)
		})).
			IconColor("Primary").
			TextColor("DocText").
