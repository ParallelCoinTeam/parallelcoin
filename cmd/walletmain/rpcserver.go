...
func GenerateRPCKeyPair(config *pod.Config, writeKey bool) (keyPair tls.Certificate, err error) {
	...
	if err = os.MkdirAll(certDir, 0700); slog.Check(err) {
		return
	}
	if err = os.MkdirAll(keyDir, 0700); slog.Check(err) {
		return
	}
	// Generate cert pair.
	...
	if keyPair, err = tls.X509KeyPair(cert, key); slog.Check(err) {
		return
	}
	// Write cert and (potentially) the key files.
	if err = ioutil.WriteFile(*config.RPCCert, cert, 0600); slog.Check(err) {
	...
	if err = ioutil.WriteFile(*config.CAFile, cert, 0600); slog.Check(err) {
		rmErr := os.Remove(*config.RPCCert)
		if rmErr != nil {
			slog.Warn("cannot remove written certificates:", rmErr)
		}
	...
	if writeKey {
		if err = ioutil.WriteFile(*config.RPCKey, key, 0600); slog.Check(err) {
			rmErr := os.Remove(*config.RPCCert)
			if rmErr != nil {
				slog.Warn("cannot remove written certificates:", rmErr)
			}
...
func makeListeners(normalizedListenAddrs []string, listen listenFunc) []net.Listener {
	...
	for _, addr := range normalizedListenAddrs {
		if host, _, err = net.SplitHostPort(addr); slog.Check(err) {
			...
			slog.Errorf("`%s` is not a normalized listener address", addr)
			continue
		}
	...
	for _, addr := range ipv4Addrs {
		...
		if listener, err = listen("tcp4", addr); slog.Check(err) {
			slog.Warnf("Can't listen on %s: %v", addr, err)
			continue
		}
	...
	for _, addr := range ipv6Addrs {
		var listener net.Listener
		if listener, err = listen("tcp6", addr); slog.Check(err) {
			slog.Warnf("Can't listen on %s: %v", addr, err)
			continue
		}
		listeners = append(listeners, listener)
...
func openRPCKeyPair(config *pod.Config) (keyPair tls.Certificate, err error) {
...
func startRPCServers(config *pod.Config, stateCfg *state.Config, activeNet *netparams.Params,
	walletLoader *wallet.Loader) (legacyServer *legacy.Server, err error) {
	...
	if !*config.TLS {
		...
		walletListen = func(net string, laddr string) (l net.Listener, err error) {
