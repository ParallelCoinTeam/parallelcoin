...
var (
	// hdSeed is the BIP 32 seed used by the memWallet to initialize it's HD
	// root key. This value is hard coded in order to ensure deterministic
...
// utxo represents an unspent output spendable by the memWallet.
// The maturity height of the transaction is recorded in order to properly
// observe the maturity period of direct coinbase outputs.
type utxo struct {
	pkScript       []byte
	value          util.Amount
	keyIndex       uint32
...
// chainUpdate encapsulates an update to the current main chain.
// This struct is used to sync up the memWallet each time a new block is
...
// undoEntry is functionally the opposite of a chainUpdate.
// An undoEntry is created for each new block received,
// then stored in a log in order to properly handle block re-orgs.
...
// wallet functionality to the harness.
// The wallet uses a hard-coded HD key hierarchy which promotes
// reproducibility between harness test runs.
type memWallet struct {
	...
	// received. Once a block is disconnected,
	// the undo entry for the particular height is evaluated,
	// thereby rewinding the effect of the disconnected block on the wallet's
	// set of spendable utxos.
...
func newMemWallet(net *netparams.Params, harnessID uint32) (*memWallet, err error) {
	...
	// newly generated DUO we can spend.
...
func (m *memWallet) SetRPCClient(rpcClient *client.Client) {
	m.rpc = rpcClient
}

// IngestBlock is a call-back which is to be triggered each time a new block
// is connected to the main chain. It queues the update for the chain syncer,
...
func (m *memWallet) ingestBlock(update *chainUpdate) {
	// Update the latest synced height,
	// then process each filtered transaction in the block creating and destroying utxos within
	...
	// Finally, record the undo entry for this block so we can properly
	// update our internal state in response to the block being re-org'd from
	// the main chain.
...
func (m *memWallet) chainSyncer() {
	...
	for range m.chainUpdateSignal {
		...
		// so pop the new chain update from the front of the update queue.
		m.chainMtx.Lock()
...
// evalOutputs evaluates each of the passed outputs,
// creating a new matching utxo within the wallet if we're able to spend the output.
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	isCoinbase bool, undo *undoEntry) {
	for i, output := range outputs {
		pkScript := output.PkScript
		// Scan all the addresses we currently control to see if the output
		// is paying to us.
		for keyIndex, addr := range m.addrs {
			...
			// If this is a coinbase output,
			// then we mark the maturity height at the proper block height in
			// the future.
...
// evalInputs scans all the passed inputs,
// destroying any utxos within the wallet which are spent by an input.
...
// It queues the update for the chain syncer,
// calling the private version in sequential order.
...
// fundTx attempts to fund a transaction sending amt bitcoin.
// The coins are selected such that the final amount spent pays enough fees
// as dictated by the passed fee rate.
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt util.Amount,
	...
	for outPoint, utxo := range m.utxos {
		...
		// updating the current tx size while accounting for the size of the
		// future sigScript.
		op := outPoint
		tx.AddTxIn(wire.NewTxIn(&op, nil, nil))
		txSize = tx.SerializeSize() + spendSize*len(tx.TxIn)
		// Calculate the fee required for the txn at this point observing the
		// specified fee rate.
		// If we don't have enough coins from he current amount selected to
		// pay the fee, then continue to grab more coins.
		...
		// output, then add an additional output to the transaction reserved
		// for it.
...
// SendOutputs creates then sends a transaction paying to the specified
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	...
	// transaction, mark the outputs are "locked".
	// This action ensures these outputs won't be double spent by any
