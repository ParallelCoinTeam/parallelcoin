...
func (n *nodeConfig) setDefaults() (err error) {
	var datadir string
	if datadir, err = ioutil.TempDir("", n.prefix+"-data"); slog.Check(err) {
		return
	}
	n.dataDir = datadir
	var logDir string
	if logDir, err = ioutil.TempDir("", n.prefix+"-logs"); slog.Check(err) {
		return
	}
	n.logDir = logDir
	var cert []byte
	if cert, err = ioutil.ReadFile(n.certFile); slog.Check(err) {
		return
	}
	n.certificates = cert
	return
...
func (n *nodeConfig) cleanup() (err error) {
	dirs := []string{
		n.logDir,
		n.dataDir,
	}
	for _, dir := range dirs {
		if err = os.RemoveAll(dir); slog.Check(err) {
		}
	}
	return
}
...
func (n *node) start() (err error) {
	if err = n.cmd.Start(); slog.Check(err) {
		return
	}
	var pid *os.File
	if pid, err = os.Create(filepath.Join(n.dataDir, fmt.Sprintf("%s.pid", n.config))); slog.Check(err) {
		return
	}
	n.pidFile = pid.Name()
	if _, err = fmt.Fprintf(pid, "%d\n", n.cmd.Process.Pid); slog.Check(err) {
		return
	}
	if err = pid.Close(); slog.Check(err) {
	}
	return
}
...
func (n *node) stop() (err error) {
	...
	defer func() {
		if err := n.cmd.Wait(); slog.Check(err) {
		}
	}()
...
func (n *node) cleanup() (err error) {
	if n.pidFile != "" {
		if err = os.Remove(n.pidFile); slog.Check(err) {
			slog.Errorf("unable to remove file %s: %v", n.pidFile, err)
		}
	}
...
func (n *node) shutdown() (err error) {
	if err = n.stop(); slog.Check(err) {
		return
	}
	if err = n.cleanup(); slog.Check(err) {
		return
	}
	return
}
...
func genCertPair(certFile, keyFile string) (err error) {
	...
	var cert, key []byte
	if cert, key, err = util.NewTLSCertPair(org, validUntil, nil); slog.Check(err) {
		return
	}
	// Write cert and key files.
	if err = ioutil.WriteFile(certFile, cert, 0666); slog.Check(err) {
		return
	}
	if err = ioutil.WriteFile(keyFile, key, 0600); err != nil {
		if err = os.Remove(certFile); slog.Check(err) {
			return
		}
	}
	return
