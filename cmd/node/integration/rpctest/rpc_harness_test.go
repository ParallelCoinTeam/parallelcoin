...
func assertConnectedTo(t *testing.T, nodeA *Harness, nodeB *Harness) {
	nodeAPeers, e := nodeA.Node.GetPeerinf.Ln()
	if e != nil  {
	...
	if !addrFound {
		t.ftl.Ln("nodeA not connected to nodeB")
	}
}
...
func testJoinMempools(r *Harness, t *testing.T) {
	...
	if len(pooledHashes) != 0 {
		t.ftl.Ln("main test harness mempool not empty")
	}
...
func testGenerateAndSubmitBlockWithCustomCoinbaseOutputs(r *Harness,
	...
	block, e := r.GenerateAndSubmitBlockWithCustomCoinbaseOutputs(txns,
	...
	timestamp := block.MsgBlock().Header.Timestamp.Add(time.Minute)
	targetBlockVersion := uint32(1337)
	block, e = r.GenerateAndSubmitBlockWithCustomCoinbaseOutputs(nil,
...
func TestHarness(t *testing.T) {
	...
	nodeInfo, e := mainHarness.Node.Getinf.Ln()
	if e != nil  {
