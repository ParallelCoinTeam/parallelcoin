...
const (
	...
	// numbers used by a test harness.
	// The min port is inclusive while the max port is exclusive.
	minPeerPort = 10000
	maxPeerPort = 35000
...
var (
	...
	numTestInstances = 0
	// processID is the process ID of the current running process,
	// it is used to calculate ports based upon it when launching an rpc
	// harnesses.
	// The intent is to allow multiple process to run in parallel without
	// port collisions.
	// It should be noted however that there is still some small probability
	// that there will be port collisions either due to other processes running
	// or simply due to the stars aligning on the process IDs.
	processID = os.Getpid()
	// testInstances is a private package-level slice used to keep track of
	// all active test harnesses.
	// This global can be used to perform various "joins" shutdown several
	// active harnesses after a test, etc.
...
// platform for creating rpc driven integration tests involving pod.
// The active pod node will typically be run in simnet mode in order to allow
// for easy generation of test blockchains.
// The active pod process is fully managed by Harness,
// which handles the necessary initialization,
// and teardown of the process along with any temporary directories created
// as a result. Multiple Harness instances may be run concurrently,
// in order to allow for testing complex scenarios involving multiple nodes.
// The harness also includes an in-memory wallet to streamline various
// classes of tests.
type Harness struct {
...
// Optionally, websocket handlers and a specified configuration may be
...
func New(activeNet *netparams.Params, handlers *rpcclient.NotificationHandlers,
	...
	// then create a wrapper callback which executes both the currently
	// registered callback and the mem wallet's callback.
	...
	// Track this newly created test instance within the package level global
	// map of all active test instances.
	testInstances[h.testNodeDir] = h
	return h, nil
}

// SetUp initializes the rpc test state.
// Initialization includes: starting up a simnet node,
// creating a websockets client and connecting to the started node,
// and finally: optionally generating and submitting a testchain with a
...
// pod process belonging to this Harness instance.
// If the initial connection attempt fails, this function will retry h.
...
// specified outputs while observing the passed fee rate and ignoring a
// change output.
...
// CreateTransaction returns a fully signed transaction paying to the
// specified outputs while observing the desired fee rate.
// The passed fee rate should be expressed in satoshis-per-byte.
// The transaction being created can optionally include a change output
// indicated by the change boolean.
// Any unspent outputs selected as inputs for the crafted transaction are
// marked as unspendable in order to avoid potential double-spends by future
// calls to this method.
// If the created transaction is cancelled for any reason then the selected
// inputs MUST be freed via a call to UnlockOutputs.
// Otherwise, the locked inputs won't be returned to the pool of spendable
// outputs.
// This function is safe for concurrent access.
...
// P2PAddress returns the harness' P2P listening address.
// This allows potential peers (
// such as SPV peers) created within tests to connect to a given test harness
// instance.
...
// transactions and submits it to the running simnet node.
// For generating blocks with only a coinbase tx,
// callers can simply pass nil instead of transactions to be mined.
// Additionally, a custom block version can be set by the caller.
// A blockVersion of -1 indicates that the current default block version
// should be used. An uninitialized time.Time should be used for the blockTime
...
// should be used. An uninitialized time.Time should be used for the blockTime
// parameter if one doesn't wish to set a custom time.
// The mineTo list of outputs will be added to the coinbase; this is not
// checked for correctness until the block is submitted; thus,
// it is the caller's responsibility to ensure that the outputs are correct.
// If the list is empty,
// the coinbase reward goes to the wallet managed by the Harness.
// This function is safe for concurrent access.
...
// addresses designated for the current rpc test.
// If there haven't been any test instances created,
// the default ports are used.
// Otherwise in order to support multiple test nodes running at once the p2p
// and rpc port are incremented after each initialization.
