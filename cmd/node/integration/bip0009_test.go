...
// assertVersionBit gets the passed block hash from the given test harness
// and ensures its version either has the provided bit set or unset per the
// set flag.
...
// test harness and ensures the provided soft fork key is both available and
// its status is the equivalent of the passed state.
...
// transition rules set forth by the BIP for the provided soft fork key.
// It uses the regression test network to signal support and advance through
// the various threshold states including failure to achieve locked in
// status. See TestBIP0009 for an overview of what is tested.
...
func testBIP0009(t *testing.T, forkKey string, deploymentID uint32) {
	...
	defer r.TearDown()
	// *** ThresholdDefined ***
	// Assert the chain height is the expected value and the soft fork status starts out as defined.
	...
	// current one.
	// All of the heights below are thus offset by one to compensate.
	...
	// Generate enough blocks to reach the next window in such a way that the
	// number blocks with the version bit set to signal support is 1 less
	// than required to achieve locked in status.
	// Assert the chain height is the expected value and the soft fork status
	// is still started and did NOT move to locked in.
	...
	// *** ThresholdLockedIn ***
	// Generate enough blocks to reach the next window in such a way that the
	// number blocks with the version bit set to signal support is exactly
	// the number required to achieve locked in status.
	// Assert the chain height is the expected value and the soft fork status
	// moved to locked in.
	...
	// Generate enough blocks to reach the height just before the next window
	// without continuing to signal support since it is already locked in.
	// Assert the chain height is the expected value and the soft fork status
	// is still locked in and did NOT move to active.
...
// TestBIP0009 ensures the BIP0009 soft fork mechanism follows the state
// transition rules set forth by the BIP for all soft forks.
// It uses the regression test network to signal support and advance through
// the various threshold states including failure to achieve locked in status.
//
// Overview:
//
...
// rules set forth by BIP0009 by using the test dummy deployment.
//
// Overview:
//
// - Generate block 1
...
func TestBIP0009Mining(t *testing.T) {
	...
	// The second to last generated block should not have the test bit set in
	// the version.
	// The last generated block should now have the test bit set in the
	// version since the pod mining code will have recognized the test dummy
	// deployment as started.
	confirmationWindow := r.ActiveNet.MinerConfirmationWindow
	...
	// version since the pod mining code will have recognized the test dummy
	// deployment as activated and thus there is no longer any need to set
	// the bit.
	hashes, err = r.Node.Generate(confirmationWindow)
	if err != nil {
		t.Fatalf("failed to generated %d blocks: %v", confirmationWindow,
