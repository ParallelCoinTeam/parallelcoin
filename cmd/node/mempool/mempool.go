...
// in the pool. This is part of the mining.
...
func (mp *TxPool) RawMempoolVerbose() map[string]*json.GetRawMempoolVerboseResult {
	...
	result := make(map[string]*json.GetRawMempoolVerboseResult, len(mp.pool))
...
func (mp *TxPool) addOrphan(tx *util.Tx, tag Tag) {
	...
	log <- cl.Debug{
		"stored orphan transaction", tx.Hash(), "(total:", len(mp.orphans), ")", cl.Ine()}
}
...
func (mp *TxPool) limitNumOrphans() error {
	...
	// The iteration order is not important here because an adversary would have to be able to pull off preimage attacks on the hashing function in order to target eviction of specific entries anyways.
	for _, otx := range mp.orphans {
...
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAcceptTransaction(b *blockchain.BlockChain, tx *util.Tx, isNew, rateLimit, rejectDupOrphans bool) ([]*chainhash.Hash, *TxDesc, error) {
	txHash := tx.Hash()
	// If a transaction has witness data, and segwit isn't active yet, If segwit isn't active yet, then we won't accept it into the mempool as it can't be mined yet.
	...
	// Therefore as long as the size of the transaction does not exceeed 1000
...
func (mp *TxPool) maybeAddOrphan(tx *util.Tx, tag Tag) error {
	...
	// which is ~5MB using the default values at the time this comment was written).
...
// the passed transaction from the orphan pool.
