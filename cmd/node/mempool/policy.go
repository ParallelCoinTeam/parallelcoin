...
const (
	// maxStandardP2SHSigOps is the maximum number of signature operations that are considered standard in a
	...
	// maxStandardSigScriptSize is the maximum size allowed for a transaction input signature script to be considered
	// standard. This value allows for a 15-of-15 CHECKMULTISIG pay-to-script-hash with compressed keys. The form of the
	// overall script is: OP_0 <15 signatures> OP_PUSHDATA2 <2 bytes len> [OP_15 <15 pubkeys> OP_15 OP_CHECKMULTISIG]
	// For the p2sh script portion, each of the 15 compressed pubkeys are 33 bytes ( plus one for the OP_DATA_33
	// opcode), and the thus it totals to ( 15*34)+3 = 513 bytes. Next each of the 15 signatures is a max of 73 bytes
	// (plus one for the OP_DATA_73 opcode). Also there is one extra byte for the initial extra OP_0 push and 3 bytes
	// for the OP_PUSHDATA2 needed to specify the 513 bytes for the script push. That brings the total to
	// 1+(15*74)+3+513 = 1627. This value also adds a few extra bytes to provide a little buffer. ( 1 + 15*74 + 3) +
	// (15*34 + 3) + 23 = 1650
	maxStandardSigScriptSize = 1650
	// DefaultMinRelayTxFee is the minimum fee in satoshi that is required for a transaction to be treated as free for
	// relay and mining purposes. It is also used to help determine if a transaction is considered dust and as a base
	// for calculating minimum required fees for larger transactions. This value is in Satoshi/1000 bytes.
...
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee util.Amount) int64 {
	// Calculate the minimum fee for a transaction to be allowed into the mempool and relayed by scaling the base fee (
	...
	// Set the minimum fee to the maximum possible value if the calculated fee is not in the valid range for monetary
	// amounts.
...
// checkInputsStandard performs a series of checks on a transaction's inputs to ensure they are "standard". A standard
// transaction input within the context of this function is one whose referenced public key script is of a standard form
// and for pay-to -script-hash, does not have more than maxStandardP2SHSigOps signature operations. However it should
// also be noted that standard inputs also are those which have a clean stack after execution and only contain pushed
// data in their signature scripts. This function does not perform those checks because the script engine already does
// this more accurately and concisely via the txscript. ScriptVerifyCleanStack and txscript.ScriptVerifySigPushOnly
// flags.
func checkInputsStandard(tx *util.Tx, utxoView *blockchain.UtxoViewpoint) error {
	// NOTE: The reference implementation also does a coinbase check here, but coinbases have already been rejected
	// prior to calling this function so no need to recheck.
	for i, txIn := range tx.MsgTx().TxIn {
		// It is safe to elide existence and index checks here since they have already been checked prior to calling
		// this function.
...
// checkPkScriptStandard performs a series of checks on a transaction output script (public key script) to ensure it is
// a "standard" public key script. A standard public key script is one that is a recognized form, and for
// multi-signature scripts only contains from 1 to maxStandardMultiSigKeys public keys.
...
// isDust returns whether or not the passed transaction output amount is considered dust or not based on the passed
// minimum transaction relay fee. Dust is defined in terms of the minimum transaction relay fee. In particular, if the
// cost to the network to spend coins is more than 1/3 of the minimum transaction relay fee, it is considered dust.
func isDust(txOut *wire.TxOut, minRelayTxFee util.Amount) bool {
	// Unspendable outputs are considered dust.
	if txscript.IsUnspendable(txOut.PkScript) {
		return true
	}
	// The total serialized size consists of the output and the associated input script to redeem it. Since there is no
	// input script to redeem it yet, use the minimum size of a typical input script. Pay-to-pubkey-hash bytes
	// breakdown:
	//
	...
	//
	// Theoretically this could examine the script type of the output script and use a different size for the typical
	// input script size for pay-to-pubkey vs pay-to-pubkey-hash inputs per the above breakdowns, but the only
	// combination which is less than the value chosen is a pay-to-pubkey script with a compressed pubkey, which is not
	// very common.
	//
	// The most common scripts are pay-to-pubkey-hash, and as per the above breakdown, the minimum size of a p2pkh input
	// script is 148 bytes. So that figure is used. If the output being spent is a witness program, then we apply the
	// witness discount to the size of the signature.
	//
	// The segwit analogue to p2pkh is a p2wkh output. This is the smallest output possible using the new segwit
	// features. The 107 bytes of witness data is discounted by a factor of 4, leading to a computed value of 67 bytes
	// of witness data.
	//
	...
	//
	// Using the typical values for a pay-to-pubkey-hash transaction from the breakdown above and the default minimum
	// free transaction relay fee of 1000, this equates to values less than 546 satoshi being considered dust.
	//
	// The following is equivalent to (value/totalSize) * (1/3) * 1000 without needing to do floating point math.
...
func checkTransactionStandard(
	...
	for i, txIn := range msgTx.TxIn {
		// Each transaction input signature script must not exceed the maximum size allowed for a standard transaction.
	...
	// None of the output public key scripts can be a non-standard script or be "dust" (except when the script is a null
	// data script).
...
// GetTxVirtualSize computes the virtual size of a given transaction. A transaction's virtual size is based off its
// weight, creating a discount for any witness data it contains, proportional to the current
