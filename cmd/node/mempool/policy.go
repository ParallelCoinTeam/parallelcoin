...
const (
	...
	// maxStandardSigScriptSize is the maximum size allowed for a transaction
	// input signature script to be considered standard.
	// This value allows for a 15-of-15 CHECKMULTISIG pay-to-script-hash with
	// compressed keys.
	// The form of the overall script is: OP_0 <15 signatures> OP_PUSHDATA2
	// <2 bytes len> [OP_15 <15 pubkeys> OP_15 OP_CHECKMULTISIG]
	// For the p2sh script portion,
	// each of the 15 compressed pubkeys are 33 bytes (
	// plus one for the OP_DATA_33 opcode), and the thus it totals to (
	// 15*34)+3 = 513 bytes.  Next each of the 15 signatures is a max of 73
	// bytes (plus one for the OP_DATA_73 opcode).
	// Also there is one extra byte for the initial extra OP_0 push and 3
	// bytes for the OP_PUSHDATA2 needed to specify the 513 bytes for the
	// script push.
	// That brings the total to 1+(15*74)+3+513 = 1627.
	// This value also adds a few extra bytes to provide a little buffer. (
	...
	// in a multi-signature transaction output script for it to be considered
	// standard.
	maxStandardMultiSigKeys = 3
)

func // calcMinRequiredTxRelayFee returns the minimum transaction fee
// required for a transaction with the passed serialized size to be accepted
// into the memory pool and relayed.
calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee util.Amount) int64 {
	// Calculate the minimum fee for a transaction to be allowed into the
	// mempool and relayed by scaling the base fee (
	// which is the minimum free transaction relay fee).
	// minTxRelayFee is in Satoshi/kB so multiply by serializedSize (
...
func // checkInputsStandard performs a series of checks on a transaction's
// inputs to ensure they are "standard".
// A standard transaction input within the context of this function is one
// whose referenced public key script is of a standard form and for pay-to
// -script-hash, does not have more than maxStandardP2SHSigOps signature
// operations.
// However it should also be noted that standard inputs also are those which
...
checkInputsStandard(tx *util.Tx, utxoView *blockchain.UtxoViewpoint) (err error) {
	...
	for i, txIn := range tx.MsgTx().TxIn {
		// It is safe to elide existence and index checks here since they
...
func isDust(txOut *wire.TxOut, minRelayTxFee util.Amount) bool {
	...
	// input script to redeem it.
	// Since there is no input script to redeem it yet,
	// use the minimum size of a typical input script.
	...
	//  and use a different size for the typical input script size for pay-to
	//  -pubkey vs pay-to-pubkey-hash inputs per the above breakdowns,
	//  but the only combination which is less than the value chosen is a pay
	//  -to-pubkey script with a compressed pubkey, which is not very common.
	// The most common scripts are pay-to-pubkey-hash, and as per the above
	// breakdown, the minimum size of a p2pkh input script is 148 bytes.  So
	// that figure is used. If the output being spent is a witness program, then
	// we apply the witness discount to the size of the signature.
	// The segwit analogue to p2pkh is a p2wkh output. This is the smallest
	// output possible using the new segwit features. The 107 bytes of witness
	// data is discounted by a factor of 4, leading to a computed value of 67
	...
	// breakdown above and the default minimum free transaction relay fee of
	// 1000, this equates to values less than 546 satoshi being considered dust.
...
// version in the supported range, being finalized,
...
func checkTransactionStandard(tx *util.Tx, height int32, medianTimePast time.Time, minRelayTxFee util.Amount,
	...
	// limit the maximum size of a transaction.
