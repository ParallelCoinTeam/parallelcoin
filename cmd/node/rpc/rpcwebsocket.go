...
func (s *Server) WebsocketHandler(conn *websocket.Conn, remoteAddr string,
	...
	log.L.Trace("new websocket client", remoteAddr)
	if s.NtfnMgr.GetNumClients()+1 > *s.Config.RPCMaxWebsockets {
		log.L.Infof("max websocket clients exceeded [%d] - disconnecting client"+
			" %s",
	...
	if err != nil {
		log.L.Errorf("failed to serve client %s: %v %s", remoteAddr, err)
		conn.Close()
		return
	}
	...
	log.L.Trace("disconnected websocket client", remoteAddr)
}
...
func (c *WSClient) Disconnect() {
	...
	log.L.Trace("disconnecting websocket client", c.Addr)
	close(c.Quit)
	c.Conn.Close()
	c.Disconnected = true
}
...
func (c *WSClient) Start() {
	log.L.Trace("starting websocket client", c.Addr)
...
out:
	for {
		...
		if err != nil {
			...
			if err != io.EOF && err != io.ErrUnexpectedEOF {
				log.L.Tracef("websocket receive error from %s: %v",
					c.Addr, err)
			}
			break out
		}
		...
		if cmd.Err != nil {
			...
			if err != nil {
				log.L.Error(err)
				log.L.Errorf("failed to marshal parse failure reply:", err)
				continue
			}
			c.SendMessage(reply, nil)
			continue
		}
		log.L.Tracef("received command <%s> from %s", cmd.Method, c.Addr)
		...
		case c.Authenticated && ok:
			...
			break out
		case !c.Authenticated && !ok:
			log.L.Warnf("unauthenticated websocket message received")
			break out
		case !c.Authenticated:
			...
			if cmp != 1 && limitcmp != 1 {
				log.L.Warn("authentication failure from", c.Addr)
				break out
			}
			...
			if err != nil {
				log.L.Error(err)
				log.L.Error("failed to marshal authenticate reply:", err)
				continue
			}
			c.SendMessage(reply, nil)
			continue
		...
		if !c.IsAdmin {
			if _, ok := RPCLimited[request.Method]; !ok {
				...
				if err != nil {
					log.L.Error(err)
					log.L.Error("failed to marshal parse failure reply:", err)
					continue
				}
				c.SendMessage(reply, nil)
				continue
	...
	c.Disconnect()
	c.WG.Done()
	log.L.Trace("websocket client input handler done for", c.Addr)
}
...
cleanup:
	...
	c.WG.Done()
	log.L.Trace("websocket client notification queue handler done for", c.Addr)
}
...
cleanup:
	...
	c.WG.Done()
	log.L.Trace("websocket client output handler done for", c.Addr)
}
...
OutPoint]map[chan struct{}]*WSClient, wsc *WSClient, ops []*wire.OutPoint) {
	...
	for _, op := range ops {
		...
		if spend != nil {
			log.L.Debugf("found existing mempool spend for outpoint<%v>: %v",
				op, spend.Hash())
	...
	for {
		...
		case n, ok := <-m.NotificationMsgs:
			...
			case *NotificationUnregisterNewMempoolTxs:
				...
				delete(txNotifications, wsc.Quit)
			default:
				log.L.Warn("unhandled notification type")
			}
...
func (*WSNtfnMgr) NotifyBlockConnected(clients map[chan struct{}]*WSClient, block *util.Block) {
	...
	if err != nil {
		log.L.Error(err)
		log.L.Error("failed to marshal block connected notification:", err)
		return
	}
...
func (*WSNtfnMgr) NotifyBlockDisconnected(
	...
	if err != nil {
		log.L.Error(err)
		log.L.Error("failed to marshal block disconnected notification:",
			err)
		return
	}
	for _, wsc := range clients {
...
func (m *WSNtfnMgr) NotifyFilteredBlockConnected(
	...
	if err != nil {
		log.L.Error("failed to serialize header for filtered block connected"+
			" notification:", err)
		return
	}
...
func (*WSNtfnMgr) NotifyFilteredBlockDisconnected(
	...
	if err != nil {
		log.L.Error(err)
		log.L.Error("failed to serialize header for filtered block disconnected"+
			" notification:", err)
		return
	}
...
func (m *WSNtfnMgr) NotifyForNewTx(clients map[chan struct{}]*WSClient,
	...
	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
	if err != nil {
		log.L.Error("failed to marshal tx notification:", err)
		return
	}
	...
	for _, wsc := range clients {
		if wsc.VerboseTxUpdates {
			...
			marshalledJSONVerbose, err = btcjson.MarshalCmd(nil,
				verboseNtfn)
			if err != nil {
				log.L.Error("failed to marshal verbose tx notification:", err)
			}
			return
		}
		err = wsc.QueueNotification(marshalledJSONVerbose)
...
func (m *WSNtfnMgr) NotifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*WSClient,
	...
	for i, txOut := range tx.MsgTx().TxOut {
		...
		for _, txAddr := range txAddrs {
			...
			marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
			if err != nil {
				log.L.Error("Failed to marshal processedtx notification:", err)
				continue
			}
...
func (m *WSNtfnMgr) NotifyRelevantTxAccepted(tx *util.Tx, clients map[chan struct{}]*WSClient) {
	...
	if len(clientsToNotify) != 0 {
		...
		if err != nil {
			log.L.Error("failed to marshal notification:", err)
			return
		}
...
func (*WSNtfnMgr) RemoveAddrRequest(
	...
	cmap, ok := addrs[addr]
	if !ok {
		log.L.Warnf("attempt to remove nonexistent addr request <%s> for"+
			" websocket"+
			" client %s",
			addr, wsc.Addr)
		return
...
OutPoint]map[chan struct{}]*WSClient, wsc *WSClient, op *wire.OutPoint) {
	...
	notifyMap, ok := ops[*op]
	if !ok {
		log.L.Warn("attempt to remove nonexistent spent request for"+
...
func HandleRescan(wsc *WSClient, icmd interface{}) (interface{}, error) {
	...
	if numAddrs == 1 {
		log.L.Debug("beginning rescan for 1 address")
	} else {
		log.L.Debugf("beginning rescan for %d addresses", numAddrs)
	}
	// Build lookup maps.
	lookups := RescanKeys{
		Fallbacks:           map[string]struct{}{},
...
fetchRange:
	for minBlock < maxBlock {
		...
		if err != nil {
			log.L.Error("error looking up block range:", err)
			return nil, &btcjson.RPCError{
				Code:    btcjson.ErrRPCDatabase,
		...
		for i := range hashList {
			...
			if err != nil {
				...
				if dbErr, ok := err.(database.Error); !ok ||
					...
					log.L.Error("error looking up block:", err)
					return nil, &btcjson.RPCError{
						Code: btcjson.ErrRPCDatabase,
				...
				// the reorg.
				if maxBlock != math.MaxInt32 {
					log.L.Error("stopping rescan for reorged block", cmd.EndBlock)
			...
			select {
			case <-wsc.Quit:
				log.L.Debugf("stopped rescan at height %v for disconnected"+
					" client",
					blk.Height())
				return nil, nil
			default:
			...
			if err != nil {
				log.L.Errorf("failed to marshal rescan progress notification: %v",
					err)
				continue
			}
...
func RecoverFromReorg(chain *blockchain.BlockChain, minBlock, maxBlock int32,
	...
	if err != nil {
		log.L.Error("error looking up possibly reorged block:", err)
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCDatabase,
			Message: "Database error: " + err.Error(),
		}
	...
	for _, tx := range blk.Transactions() {
		...
		for _, txin := range tx.MsgTx().TxIn {
			if _, ok := lookups.Unspent[txin.PreviousOutPoint]; ok {
				...
				marshalledJSON, err := NewRedeemingTxNotification(txHex,
					tx.Index(), blk)
				if err != nil {
					log.L.Error("failed to marshal redeemingtx notification:",
						err)
					continue
				}
				err = wsc.QueueNotification(marshalledJSON)
		...
		for txOutIdx, txout := range tx.MsgTx().TxOut {
			...
			for _, addr := range addrs {
				...
				marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
				if err != nil {
					log.L.Error("failed to marshal recvtx notification:", err)
					return
				}
				err = wsc.QueueNotification(marshalledJSON)
				// Stop the rescan early if the websocket client disconnected.
