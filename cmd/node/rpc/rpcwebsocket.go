...
func (*WSNtfnMgr) NotifyFilteredBlockDisconnected(
	...
	if err != nil {
		Error(err)
		Error("failed to serialize header for filtered block disconnected"+
			" notification:", err)
		return
	}
...
func (*WSNtfnMgr) RemoveAddrRequest(
	...
	cmap, ok := addrs[addr]
	if !ok {
		Warnf("attempt to remove nonexistent addr request <%s> for"+
			" websocket"+
			" client %s",
			addr, wsc.Addr)
		return
...
fetchRange:
	...
	loopHashList:
		for i := range hashList {
			...
			select {
			case <-wsc.Quit:
				Debugf("stopped rescan at height %v for disconnected"+
					" client",
					blk.Height())
				return nil, nil
			default:
