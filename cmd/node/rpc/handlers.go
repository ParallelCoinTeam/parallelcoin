...
func HandleGenerate(
	...
	log.L.Debug("cpu miner stuff is missing here")
...
func HandleGetAddedNodeInfo(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("getaddednodeinfo")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
...
func HandleGetBlock(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("getblock")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
...
func HandleGetBlockHash(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("getblockhash")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
...
func HandleGetBlockHeader(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("getblockheader")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
...
func HandleGetBlockTemplate(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("getblocktemplate")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
...
func HandleGetCFilter(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("getfilter")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
...
func HandleGetCFilterHeader(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("getcfilterheader")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
...
func HandleGetDifficulty(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("getdifficulty")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
...
func HandleGetGenerate(
	...
	if generating {
		log.L.Debug("miner is running internally")
	} else {
		log.L.Debug("miner is not running")
	}
	// return nil, nil
...
func HandleGetHashesPerSec(
	...
	log.L.Debug("miner hashes per second - multicast thing TODO")
...
func HandleGetHeaders(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("getheaders")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
...
func HandleGetMiningInfo(s *Server, cmd interface{},
	...
	case 0:
		...
		log.L.Debug("missing generate stats in here")
	...
	case 1:
		...
		log.L.Debug("missing cpu miner stuff in here") // cpuminer
...
func HandleGetRawTransaction(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("getrawtransaction")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
...
func HandleGetTxOut(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("gettxout")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
...
func HandleNode(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("help")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
...
func HandleSendRawTransaction(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("sendrawtransaction")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
...
func HandleSetGenerate(
	...
	log.L.Debug("generating", generate, "threads", genProcLimit)
	...
	*s.Config.GenThreads = genProcLimit
	if s.Cfg.CPUMiner != nil {
		log.L.Debug("stopping existing process")
		err := s.Cfg.CPUMiner.Process.Kill()
		if err != nil {
			log.L.Error(err)
		}
	}
	log.L.Debug("saving configuration")
	save.Pod(s.Config)
	if *s.Config.Generate && *s.Config.GenThreads != 0 {
		log.L.Debug("starting miner")
...
func HandleSubmitBlock(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("submitblock")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
...
func HandleValidateAddress(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("validateaddress")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
...
func HandleVerifyChain(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("verifychain")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
...
func HandleVerifyMessage(
	...
	if !ok {
		var h string
		h, err = s.HelpCacher.RPCMethodHelp("verifymessage")
		if err != nil {
			msg = err.Error() + "\n\n"
		}
		msg += h
