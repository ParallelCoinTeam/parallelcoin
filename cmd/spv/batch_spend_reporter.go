...
// reporter expects a sequence of blocks consisting of those containing a UTXO to watch, or any whose filter generates a
// match using current filterEntries. This instance supports multiple requests for the same outpoint.
type batchSpendReporter struct {
	// requests maps an outpoint to list of GetUtxoRequests waiting for that UTXO's spend report.
	requests map[wire.OutPoint][]*GetUtxoRequest
	// initialTxns contains a map from an outpoint to the "unspent" version of it's spend report. This value is
	// populated by fetching the output from the block in the request's start height. This spend report will be returned
	// in the case that the output remained unspent for the duration of the scan.
	initialTxns map[wire.OutPoint]*SpendReport
	// outpoints caches the filter entry for each outpoint, conserving allocations when reconstructing the current
	// filterEntries.
	outpoints map[wire.OutPoint][]byte
	// filterEntries holds the current set of watched outpoint, and is applied to cfilters to gauge whether we should
	// download the block.
	//
...
// ProcessBlock accepts a block, block height, and any new requests whose start height matches the provided height. If a
// non-zero number of new requests are presented, the block will first be checked for the initial outputs from which
...
func (b *batchSpendReporter) ProcessBlock(blk *wire.MsgBlock,
	...
	// Next, filter the block for any spends using the current set of watched outpoints. This will include any new
	// requests added above.
	spends := b.notifySpends(blk, height)
	// Finally, rebuild filter entries from cached entries remaining in outpoints map. This will provide an updated
	// watchlist used to scan the subsequent filters.
...
// addNewRequests adds a set of new GetUtxoRequests to the spend reporter's state. This method immediately adds the
// request's outpoints to the reporter's watchlist.
func (b *batchSpendReporter) addNewRequests(reqs []*GetUtxoRequest) {
	for _, req := range reqs {
		...
		// Build the filter entry only if it is the first time seeing the outpoint.
...
// findInitialTransactions searches the given block for the creation of the UTXOs that are supposed to be birthed in
// this block. If any are found, a spend report containing the initial outpoint will be saved in case the outpoint is
...
func (b *batchSpendReporter) findInitialTransactions(block *wire.MsgBlock,
	...
	// Iterate over the transactions in this block, hashing each and querying our reverse index to see if any requests
	// depend on the txn.
	...
	for _, tx := range block.Transactions {
		...
		// For all requests that are watching this txid, use the output index of each to grab the initial output.
		txOuts := tx.TxOut
	...
	// Finally, we must reconcile any requests for which the txid did not exist in this block. A nil spend report is
	// saved for every initial txn that could not be found, otherwise the result is copied from scan above. The copied
	// values can include valid initial txns, as well as nil spend report if the output index was invalid.
	for _, req := range newReqs {
...
// notifyRequests delivers the same final response to the given requests, and cleans up any remaining state for the
// outpoint.
//
...
func (b *batchSpendReporter) notifySpends(block *wire.MsgBlock,
	...
	for _, tx := range block.Transactions {
		...
		for i, ti := range tx.TxIn {
			...
			// With the requests located, we remove this outpoint from both the requests, outpoints, and initial txns
			// map. This will ensures we don't continue watching this outpoint.
