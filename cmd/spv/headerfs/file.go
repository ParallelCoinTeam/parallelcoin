...
// header type.
func (h *headerStore) readRaw(seekDist uint64) (rawHeader []byte, err error) {
	...
	case RegularFilter:
		headerSize = 32
	default:
		err = fmt.Errorf("unknown index type: %v", h.indexType)
		slog.Debug(err)
		return
	}
	...
	if _, err = h.file.ReadAt(rawHeader[:], int64(seekDist)); slog.Check(err) {
		rawHeader = nil
		return
	}
	return
...
func (h *blockHeaderStore) readHeaderRange(startHeight uint32,
	endHeight uint32) (headers []wire.BlockHeader, err error) {
	// Based on the defined header type, we'll determine the number of
	// bytes that we need to read past the sync point.
	var headerSize uint32
	switch h.indexType {
	...
	if _, err = h.file.ReadAt(rawHeaderBytes, int64(seekDistance)); slog.Check(err) {
		return
	}
	...
	for headerReader.Len() != 0 {
		...
		if err = nextHeader.Deserialize(headerReader); slog.Check(err) {
			headers = nil
			return
		}
...
func (h *blockHeaderStore) readHeader(height uint32) (header wire.BlockHeader, err error) {
	// Each header is 80 bytes, so using this information, we'll seek a
	// distance to cover that height based on the size of block headers.
	...
	// offset.
	var rawHeader []byte
	if rawHeader, err = h.readRaw(seekDistance); slog.Check(err) {
		return
	}
	headerReader := bytes.NewReader(rawHeader)
	// Finally, decode the raw bytes into a proper bitcoin header.
	if err = header.Deserialize(headerReader); slog.Check(err) {
		return
	}
	return
}
...
// flat files on disk.
func (f *FilterHeaderStore) readHeader(height uint32) (h *chainhash.Hash, err error) {
	seekDistance := uint64(height) * 32
	var rawHeader []byte
	if rawHeader, err = f.readRaw(seekDistance); slog.Check(err) {
		return
	}
