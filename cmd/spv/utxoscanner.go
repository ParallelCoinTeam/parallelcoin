...
type (
	...
	GetUtxoRequest struct {
		...
		// BirthHeight is the height at which we expect to find the original unspent outpoint. This is also the height
		// used when starting the search for spends.
		BirthHeight uint32
		// resultChan either the spend report or error for this request.
		resultChan chan *getUtxoResult
		// result caches the first spend report or error returned for this request.
	...
	// to an already running batch.
	GetUtxoRequestPQ []*GetUtxoRequest
	// UtxoScanner batches calls to GetUtxo so that a single scan can search for multiple outpoints. If a scan is in
	// progress when a new element is added, we check whether it can safely be added to the current batch, if not it
	...
	UtxoScannerConfig struct {
		...
		// BlockFilterMatches checks the cfilter for the block hash for matches against the rescan options.
...
// deliver tries to deliver the report or error to any subscribers. If resultChan cannot accept a new update, this
...
batchShutdown:
	...
	// Cancel all pending get utxo requests that were not pulled into the batchManager's main goroutine.
	for !s.pq.IsEmpty() {
...
// batchManager is responsible for scheduling batches of UTXOs to scan. Any incoming requests whose start height has
// already been passed will be added to the next batch, which gets scheduled after the current batch finishes.
//
...
func (s *UtxoScanner) batchManager() {
	...
	for {
		...
		// Initiate a scan, starting from the birth height of the least-height request currently in the queue.
...
func (s *UtxoScanner) dequeueAtHeight(height uint32) []*GetUtxoRequest {
	s.cv.L.Lock()
	defer s.cv.L.Unlock()
	// Take any requests that are too old to go in this batch and keep them for the next batch.
...
// scanFromHeight runs a single batch, pulling in any requests that get added above the batch's last processed height.
// If there was an error, then return the outstanding requests.
func (s *UtxoScanner) scanFromHeight(initHeight uint32) error {
	// Before beginning the scan, grab the best block stamp we know of, which will serve as an initial estimate for the
	// end height of the scan.
	...
	var (
		// startHeight and endHeight bound the range of the current scan. If more blocks are found while a scan is
		// running, these values will be updated afterwards to scan for the new blocks.
		startHeight = initHeight
		endHeight   = uint32(bestStamp.Height)
	)
	reporter := newBatchSpendReporter()
scanToEnd:
	// Scan forward through the blockchain and look for any transactions that might spend the given UTXOs.
	for height := startHeight; height <= endHeight; height++ {
		// Before beginning to scan this height, check to see if the utxoscanner has been signaled to exit.
		select {
		case <-s.quit:
		...
		// If there are any new requests that can safely be added to this batch, then try and fetch them.
		newReqs := s.dequeueAtHeight(height)
		// If an outpoint is created in this block, then fetch it regardless. Otherwise check to see if the filter
		// matches any of our watched outpoints.
		fetch := len(newReqs) > 0
		if !fetch {
			...
			// If still no match is found, we have no reason to fetch this block, and can continue to next height.
			if !match {
				continue
			}
		}
		// At this point, we've determined that we either (1) have new requests which we need the block to scan for
		// originating UTXOs, or (2) the watchlist triggered a match against the neutrino filter. Before fetching the
		// block, check to see if the utxoscanner has been signaled to exit so that we can exit the rescan before
	...
	// We've scanned up to the end height, now perform a check to see if we still have any new blocks to process. If
	// this is the first time through, we might have a few blocks that were added since the scan started.
	currStamp, err := s.cfg.BestSnapshot()
	if err != nil {
		Error(err)
		return reporter.FailRemaining(err)
	}
	// If the returned height is higher, we still have more blocks to go. Shift the start and end heights and continue
	// scanning.
