...
type (
	...
	// A GetUtxoRequestPQ implements heap.
	// Interface and holds GetUtxoRequests. The queue maintains that heap.
	// Pop() will always return the GetUtxo request with the least starting
	// height. This allows us to add new GetUtxo requests to an already
	// running batch.
	GetUtxoRequestPQ []*GetUtxoRequest
	// UtxoScanner batches calls to GetUtxo so that a single scan can search
	// for multiple outpoints. If a scan is in progress when a new element is
	// added, we check whether it can safely be added to the current batch,
	// if not it will be included in the next batch.
	UtxoScanner struct {
		started   uint32
		stopped   uint32
...
func // batchManager is responsible for scheduling batches of UTXOs to scan. Any
...
scanToEnd:
	...
	for height := startHeight; height <= endHeight; height++ {
		...
		log.L.Debugf("processing block height=%d hash=%s %s", height, hash)
...
func // NewUtxoScanner creates a new instance of UtxoScanner using the given
// chain interface.
NewUtxoScanner(cfg *UtxoScannerConfig) *UtxoScanner {
	scanner := &UtxoScanner{
		cfg:      cfg,
