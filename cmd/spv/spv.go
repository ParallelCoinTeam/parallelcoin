...
func (s *ChainService) handleAddPeerMsg(state *peerState, sp *ServerPeer) bool {
	...
	if atomic.LoadInt32(&s.shutdown) != 0 {
		log <- cl.Infof{"new peer %s ignored - server is shutting down", sp, cl.Ine()}
		sp.Disconnect()
		return false
	}
	...
	if err != nil {
		log <- cl.Debug{"can't split host/port:", err, cl.Ine()}
		sp.Disconnect()
		return false
	}
	...
	log <- cl.Debug{"new peer", sp, cl.Ine()}
...
func (s *ChainService) handleDonePeerMsg(state *peerState, sp *ServerPeer) {
	...
	if _, ok := list[sp.ID()]; ok {
		...
		delete(list, sp.ID())
		log <- cl.Debug{"removed peer", sp, cl.Ine()}
		return
	}
	if sp.connReq != nil {
		s.connManager.Disconnect(sp.connReq.ID())
...
func (s *ChainService) outboundPeerConnected(c *connmgr.ConnReq, conn net.Conn) {
	...
	if err != nil {
		log <- cl.Debugf{
			"cannot create outbound peer %s: %s %s", c.Addr, err, cl.Ine()}
