...
var ErrRescanExit = errors.New("rescan exited")
...
// NotificationHandlers specifies notification handlers for the rescan. These will always run in the same goroutine as
// the caller.
...
// StartBlock specifies the start block. The hash is checked first; if there's no such hash (zero hash avoids lookup),
// the height is checked next. If the height is 0 or the start block isn't specified, starts from the genesis block.
// This block is assumed to already be known, and no notifications will be sent for this block. The rescan uses the
// latter of StartBlock and StartTime.
...
// StartTime specifies the start time. The time is compared to the timestamp of each block, and the rescan only begins
// once the first block crosses that timestamp. When using this, it is advisable to use a margin of error and start
// rescans slightly earlier than required. The rescan uses the latter of StartBlock and StartTime.
...
// EndBlock specifies the end block. The hash is checked first; if there's no such hash (zero hash avoids lookup), the
// height is checked next. If the height is 0 or in the future or the end block isn't specified, the quit channel MUST
// be specified as Rescan will sync to the tip of the blockchain and continue to stay in sync and pass notifications.
...
// WatchAddrs specifies the addresses to watch/filter for. Each call to this function adds to the list of addresses
// being watched rather than replacing the list. Each time a transaction spends to the specified address, the outpoint
// is added to the WatchOutPoints list.
...
// InputWithScript couples an previous outpoint along with its input script. We'll use the prev script to match the
// filter itself, but then scan for the particular outpoint when we need to make a notification decision.
...
// WatchInputs specifies the outpoints to watch for on-chain spends. We also require the script as we'll match on the
// script, but then notify based on the outpoint. Each call to this function adds to the list of outpoints being watched
...
// TxIdx specifies a hint transaction index into the block in which the UTXO is created (eg, coinbase is 0, next
// transaction is 1, etc.)
...
// updateChan specifies an update channel. This is for internal use by the Rescan.Update functionality.
...
func (s *ChainService) rescan(options ...RescanOption) error {
	// First, we'll apply the set of default options, then serially apply all the options that've been passed in.
	...
	if ro.endBlock != nil {
		...
		// If the ending hash it nil, then check to see if the target height is non-nil. If not, then we'll use this to
	...
	// Now wait for either filter headers to be fully synced, or we are quitting. We also queue any incoming rescan
	// updates, such that we can apply them when the filters are synced.
...
filterHeaderWaitLoop:
	...
	// Compare the start time to the start block. If the start time is later, cycle through blocks until we find a block
	// timestamp later than the start time, and begin filter download at that block. Since time is non-monotonic between
	// blocks, we look for the first block to trip the switch, and download filters from there, rather than checking
	// timestamps at each block.
	...
	// blockRetryInterval is the interval in which we'll continually re-try to fetch the latest filter from our peers.
	//
	// TODO(roasbeef): add exponential back-off
	blockRetryInterval := time.Millisecond * 100
	// blockReFetchTimer is a stoppable timer that we'll use to reminder ourselves to refetch a block in the case that
	// we're unable to fetch the filter for a block the first time around.
	var blockReFetchTimer *time.Timer
	resetBlockReFetchTimer := func(headerTip wire.BlockHeader, height int32) {
		// If so, then we'll avoid notifying the block, and will instead add this to our retry queue, as we should be
		// getting block disconnected notifications in short order.
		...
		// We'll start a timer to re-send this header so we re-process if in the case that we don't get a re-org soon
		// afterwards.
	...
	// Loop through blocks, one at a time. This relies on the underlying ChainService API to send blockConnected and
	// blockDisconnected notifications in the correct order.
	current := false
rescanLoop:
	for {
		// If we've reached the ending height or hash for this rescan, then we'll exit.
		...
		case true:
			...
			case update := <-ro.update:
				rewound, err := ro.updateFilter(update, &curStamp, &curHeader)
				if err != nil {
					Error(err)
					return err
				}
				...
				if rewound {
					Tracef("rewound to block %d (%s), no longer current", curStamp.Height, curStamp.Hash)
					current = false
					s.unsubscribeBlockMsgs(subscription)
					subscription = nil
				}
			case header := <-blockConnected:
				// If we've somehow missed a header in the range, then we'll mark ourselves as not current so we can
				// walk down the chain and notify the callers of blocks we may have missed.
				//
				// It's possible due to the nature of the current subscription system that we get a duplicate block.
				// We'll catch this and continue forwards to avoid an unnecessary state transition back to the !current
				// state.
				if header.PrevBlock != curStamp.Hash &&
					header.BlockHash() != curStamp.Hash {
					Debugf("rescan got out of order block %s with previous block"+
						" %s, curHeader: %s %s", header.BlockHash(), header.PrevBlock, curStamp.Hash)
					current = false
					continue rescanLoop
				}
				// Do not process block until we have all filter headers. Don't worry, the block will get re-queued
				// every time there is a new filter available. However, if it's a duplicate block notification, then we
				// can re-process it without any issues.
				if header.BlockHash() != curStamp.Hash &&
					!s.hasFilterHeadersByHeight(uint32(curStamp.Height+1)) {
					Warnf("missing filter header for height=%v, skipping", curStamp.Height+1)
					continue rescanLoop
				}
				...
				Tracef("rescan got block %d (%s)", curStamp.Height, curStamp.Hash)
				// We're only scanning if the header is beyond the horizon of our start time.
				if !scanning {
					scanning = ro.startTime.Before(curHeader.Timestamp)
				}
				// If we're actually scanning and we have a non-empty watch list, then we'll attempt to fetch the filter
				// from the network.
				var blockFilter *gcs.Filter
				queryOptions := NumRetries(0)
				blockFilter, err = s.GetCFilter(curStamp.Hash, wire.GCSFilterRegular, queryOptions)
				switch {
				// If the block index doesn't know about this block, then it's likely we're mid re-org so we'll accept
				...
				if blockFilter == nil {
					// TODO(halseth): this is racy, as blocks can come in before we refetch.
					resetBlockReFetchTimer(header, curStamp.Height)
					continue
				}
				err := s.notifyBlockWithFilter(ro, &curHeader, &curStamp, blockFilter)
				if err != nil {
					Error(err)
					return err
				}
			...
			case header := <-blockDisconnected:
				Debugf("rescan disconnect block %d (%s) %s", curStamp.Height, curStamp.Hash)
				...
				if header.BlockHash() == curStamp.Hash {
					...
					if ro.ntfn.OnFilteredBlockDisconnected != nil {
						ro.ntfn.OnFilteredBlockDisconnected(curStamp.Height, &curHeader)
					}
					if ro.ntfn.OnBlockDisconnected != nil {
						ro.ntfn.OnBlockDisconnected(&curStamp.Hash, curStamp.Height, curHeader.Timestamp)
					}
					header := s.getReorgTip(header.PrevBlock)
					curHeader = *header
					curStamp.Hash = header.BlockHash()
					curStamp.Height--
					// Now that we got a re-org, if we had a re-fetch timer going, we'll re-set is at the new header
					// tip.
					if blockReFetchTimer != nil {
						resetBlockReFetchTimer(*header, curStamp.Height)
					}
		...
		// If we're not yet current, then we'll walk down the chain until we reach the tip of the chain as we know it.
		// At this point, we'll be "current" again.
		case false:
		...
		updateFilterLoop:
			...
			if nextHeight > bestBlock.Height {
				Debugf("rescan became current at %d (%s), "+
					"subscribing to block notifications %s", curStamp.Height, curStamp.Hash)
				current = true
				// Ensure we cancel the old subscription if we're going back to scan for missed blocks.
				if subscription != nil {
					s.unsubscribeBlockMsgs(subscription)
				}
			...
			// notification, maybe also scanning the filters for the block.
			header, err := s.BlockHeaders.FetchHeaderByHeight(
				uint32(nextHeight),
			)
			if err != nil {
...
func (s *ChainService) notifyBlock(ro *rescanOptions,
	...
	scanning bool) error {
	// Find relevant transactions based on watch list. If scanning is false, we can safely assume this block has no
	// relevant transactions.
	var relevantTxs []*util.Tx
	if len(ro.watchList) != 0 && scanning {
		// If we have a non-empty watch list, then we need to see if it matches the rescan's filters, so we get the
		// basic filter from the DB or network.
...
// extractBlockMatches fetches the target block from the network, and filters out any relevant transactions found within
// the block.
func (s *ChainService) extractBlockMatches(ro *rescanOptions,
	curStamp *waddrmgr.BlockStamp) ([]*util.Tx, error) {
	// We've matched. Now we actually get the block and cycle through the transactions to see which ones are relevant.
...
// notifyBlockWithFilter calls appropriate listeners based on the block filter. This differs from notifyBlock in that is
// expects the caller to already have obtained the target filter.
func (s *ChainService) notifyBlockWithFilter(
	ro *rescanOptions,
	curHeader *wire.BlockHeader,
	curStamp *waddrmgr.BlockStamp,
	filter *gcs.Filter,
) error {
	// Based on what we find within the block or the filter, we'll be sending out a set of notifications with
	// transactions that are relevant to the rescan.
	var relevantTxs []*util.Tx
	// If we actually have a filter, then we'll go ahead an attempt to match the items within the filter to ensure we
	// create any relevant notifications.
	if filter != nil {
...
// block is certainly not interesting to us. This method differs from blockFilterMatches in that it expects the filter
// to already be obtained, rather than fetching the filter from the network.
...
func (s *ChainService) blockFilterMatches(ro *rescanOptions,
	...
	// If we found the basic filter, and the filter isn't "nil", then we'll check the items in the watch list against
	// it.
	...
	// We don't need the extended filter, since all of the things a rescan can watch for are currently added to the same
	// watch list and available in the basic filter. In the future, we can watch for data pushes in input scripts (incl.
	// P2SH and witness). In the meantime, we return false if the basic filter didn't match our watch list.
	return false, nil
}

// hasFilterHeadersByHeight checks whether both the basic and extended filter headers for a particular height are known.
...
func (ro *rescanOptions) updateFilter(update *updateOptions,
	...
	// If we need to rewind, then we'll walk backwards in the chain until we arrive at the block _just_ before the
	// rewind.
	for curStamp.Height > int32(update.rewind) {
		...
		// We just disconnected a block above, so we're now in rewind mode. We set this to true here so we properly send
...
// Rescan is an object that represents a long-running rescan/notification client with updateable filters. It's meant to
// be close to a drop-in replacement for the btcd rescan and notification functionality used in wallets. It only
...
// NewRescan returns a rescan object that runs in another goroutine and has an updatable filter. It returns the
...
// WaitForShutdown waits until all goroutines associated with the rescan have exited. This method is to be called once
// the passed quitchan (if any) has been closed.
...
// Rewind rewinds the rescan to the specified height (meaning, disconnects down to the block immediately after the
// specified height) and restarts it from that point with the (possibly) newly expanded filter. Especially useful when
...
// DisableDisconnectedNtfns tells the rescan not to send `OnBlockDisconnected` and `OnFilteredBlockDisconnected`
// notifications when rewinding.
...
// SpendReport is a struct which describes the current spentness state of a particular output. In the case that an
// output is spent, then the spending transaction and related details will be populated. Otherwise, only the target
// unspent output in the chain will be returned.
type SpendReport struct {
	// SpendingTx is the transaction that spent the output that a spend report was requested for.
	//
	// NOTE: This field will only be populated if the target output has been spent.
	SpendingTx *wire.MsgTx
	// SpendingTxIndex is the input index of the transaction above which spends the target output.
	//
	// NOTE: This field will only be populated if the target output has been spent.
	SpendingInputIndex uint32
	// SpendingTxHeight is the hight of the block that included the transaction above which spent the target output.
	//
	// NOTE: This field will only be populated if the target output has been spent.
	SpendingTxHeight uint32
	// Output is the raw output of the target outpoint.
	//
	// NOTE: This field will only be populated if the target is still unspent.
	Output *wire.TxOut
}
...
func (s *ChainService) GetUtxo(options ...RescanOption) (*SpendReport, error) {
	// Before we start we'll fetch the set of default options, and apply any user specified options in a functional
	// manner.
	ro := defaultRescanOptions()
	...
	// As this is meant to fetch UTXO's, the options MUST specify exactly one outpoint.
...
// getReorgTip gets a block header from the chain service's cache. This is only required until the block subscription
// API is factored out into its own package.
//
