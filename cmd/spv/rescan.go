...
rescanOptions struct {
	...
	ntfn         client.NotificationHandlers
	startTime    time.Time
	...
	for {
		...
		case true:
			...
			case header := <-blockConnected:
				...
				switch {
				// If the block index doesn't know about this block,
				// then it's likely we're mid re-org so we'll accept this as
				...
				// have any peers to fetch this filter from or the peer(
				// s) that we're trying to fetch from are in the progress of
				// a re-org.
				if blockFilter == nil {
