...
var (
	...
	// QueryNumRetries specifies how many times to retry sending a query to each peer before we've concluded we aren't
	// going to get a valid response. This allows to make up for missed messages in some instances.
	QueryNumRetries = 2
	// QueryPeerConnectTimeout specifies how long to wait for the underlying chain service to connect to a peer before
...
// queryOptions are a set of options that can be modified per-query, unlike global options.
//
// TODO: Make more query options that override global options.
type queryOptions struct {
	// timeout lets the query know how long to wait for a peer to answer the query before moving onto the next peer.
	timeout time.Duration
	// numRetries tells the query how many times to retry asking each peer the query.
	numRetries uint8
	// peerConnectTimeout lets the query know how long to wait for the underlying chain service to connect to a peer
	// before giving up on a query in case we don't have any peers.
	peerConnectTimeout time.Duration
	// encoding lets the query know which encoding to use when queueing messages to a peer.
	encoding wire.MessageEncoding
	// doneChan lets the query signal the caller when it's done, in case it's run in a goroutine.
	doneChan chan<- struct{}
	// persistToDisk indicates whether the filter should also be written to disk in addition to the memory cache. For
	// "normal" wallets, they'll almost never need to re-match a filter once it's been fetched unless they're doing
	// something like a key import.
	persistToDisk bool
}
...
// queryState is an atomically updated per-query state for each query in a batch.
//
// State transitions are:
//
// * queryWaitSubmit->queryWaitResponse - send query to peer
// * queryWaitResponse->queryWaitSubmit - query timeout with no acceptable response
...
//
// * queryAllPeers allows a single query to be broadcast to all peers, and then waits for as many peers as possible to
// answer that query within a timeout. This allows for doing things like checking cfilter checkpoints.
//
// * queryPeers allows a single query to be passed to one peer at a time until the query is deemed answered. This is
// good for getting a single piece of data, such as a filter or a block.
//
...
// ChainService, attempting to get them all answered unless the quit channel is closed.
//
...
//
// TODO(aakselrod): support for more than one in-flight query per peer to reduce effects of latency.
...
// options takes functional options for executing the query.
	...
	peerGoroutine := func(
		...
		// Track the last query our peer failed to answer and skip over it for the next attempt. This helps prevent most
		// instances of the same peer being asked for the same query every time.
		...
		for firstUnfinished < len(queryMsgs) {
			...
			for i := firstUnfinished; i < len(queryMsgs); i++ {
				// If this query is finished and we're at firstUnfinished, update firstUnfinished.
				...
				// being handled. If not, we move to the next one.
			...
			if handleQuery == -1 {
				...
				// We have nothing to work on but not all queries are answered yet. Wait for a query timeout, or a quit
				// signal, then see if anything needs our help.
				select {
				case <-queryQuit:
					return
				case <-s.quit.Wait():
	...
	// peerQuits holds per-peer quit channels so we can kill the goroutines when they disconnect.
	peerQuits := make(map[string]qu.C)
	// matchSignals holds per-peer answer channels that get a notice that the query got a match. If it's the peer's
	// match, the peer can mark the query a success and move on to the next query ahead of timeout.
	matchSignals := make(map[string]qu.C)
	// Clean up on exit.
	defer func() {
	...
	for {
		// Update our view of peers, starting new workers for new peers and removing disconnected/banned peers.
...
// options takes functional options for executing the query.
	...
	qo.numRetries = 1
	qo.applyQueryOptions(options...)
	// This is done in a single-threaded query because the peerState is held in a single thread. This is the only part
	...
	// Now we start a goroutine for each peer which manages the peer's message subscription.
	peerQuits := make(map[string]qu.C)
	...
	// Loop for any messages sent to us via our subscription channel and check them for whether they satisfy the query.
	// Break the loop when allQuit is closed.
checkResponses:
	for {
		select {
		case <-queryQuit.Wait():
...
// options takes functional options for executing the query.
	...
	qo := defaultQueryOptions()
	qo.applyQueryOptions(options...)
	// We get an initial view of our peers, to be updated each time a peer query times out.
	...
	// Loop for any messages sent to us via our subscription channel and check them for whether they satisfy the query.
	// Break the loop if it's time to quit.
...
checkResponses:
	for {
		...
		// The current peer we're querying has failed to answer the query. Time to select a new peer and query it.
		case <-peerTimeout.C:
			if queryPeer != nil {
				queryPeer.unsubscribeRecvMsgs(subscription)
			}
			...
			// If at this point, we don't yet have a query peer, then we'll exit now as all the peers are exhausted.
			if queryPeer == nil {
				break checkResponses
			}
		}
...
func (s *ChainService) getFilterFromCache(
	...
	if filterValue, e = s.FilterCache.Get(cacheKey); dbg.Chk(e) {
		return
	}
...
func (s *ChainService) GetCFilter(
	...
	// Only get one CFilter at a time to avoid redundancy from mutliple rescans running at once.
	s.mtxCFilter.Lock()
	...
	filter, e := s.getFilterFromCache(&blockHash, dbFilterType)
	if e == nil && filter != nil {
		return filter, nil
	}
	...
	if filter, e = s.FilterDB.FetchFilter(&blockHash, dbFilterType); !dbg.Chk(e) && filter != nil {
		return filter, nil
	}
	...
	block, height, e := s.BlockHeaders.FetchHeader(&blockHash)
	if e != nil {
		return
	}
	...
	if curHeader, e = getHeader(&blockHash); dbg.Chk(e) {
		return nil, fmt.Errorf(
			"couldn't get cfheader for block %s from database", blockHash,
		)
	}
	var prevHeader *chainhash.Hash
	if prevHeader, e = getHeader(&block.PrevBlock); dbg.Chk(e) {
		return nil, fmt.Errorf(
			"couldn't get cfheader for block %s from database", blockHash,
		)
	}
	...
	s.queryPeers(
		...
		// Chk responses and if we get one that matches, end the query early.
		func(sp *ServerPeer, resp wire.Message, quit chan<- struct{}) {
			...
			case *wire.MsgCFilter:
				...
				if gotFilter, e = gcs.FromNBytes(builder.DefaultP, builder.DefaultM, response.Data); dbg.Chk(e) {
				...
				if gotHeader, e = builder.MakeHeaderForFilter(gotFilter, *prevHeader); dbg.Chk(e) ||
					gotHeader != *curHeader {
					return
				}
	...
	if filter != nil {
		...
		qo.applyQueryOptions(options...)
		if qo.persistToDisk {
			if e = s.FilterDB.PutFilter(&blockHash, filter, dbFilterType); dbg.Chk(e) {
				return
			}
...
func (s *ChainService) GetBlock(
	...
	if e != nil || blockHeader.BlockHash() != blockHash {
		return nil, fmt.Errorf(
	...
	if blockValue, e = s.BlockCache.Get(*inv); !dbg.Chk(e) && blockValue != nil {
	...
	if e = getData.AddInvVect(inv); dbg.Chk(e) {
	}
	...
	// query is finished, so we can just write to it naively.
	s.queryPeers(
		...
		func(sp *ServerPeer, resp wire.Message, quit chan<- struct{}) {
			...
			case *wire.MsgBlock:
				...
				var pb *util.Block
				if pb, e = sp.server.GetBlock(block.MsgBlock().Header.PrevBlock); dbg.Chk(e) {
				}
...
// SendTransaction sends a transaction to all peers.
// It returns an error if any peer rejects the transaction.
//
...
func (s *ChainService) SendTransaction(
	...
	txHash := tx.TxHash()
	inv := wire.NewMsgInv()
	if e = inv.AddInvVect(wire.NewInvVect(invType, &txHash)); dbg.Chk(e) {
	}
		...
		func(
			...
			case *wire.MsgReject:
				if response.Hash == txHash {
					e = fmt.Errorf("transaction %s rejected by %s: %s", tx.TxHash(), sp.Addr(), response.Reason)
					close(quit)
				}
			}
		},
		// Default to 500ms timeout. Default for queryAllPeers is a single try.
		append(
