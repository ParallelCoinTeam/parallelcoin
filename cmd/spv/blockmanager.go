...
const (
	...
	// TODO: should this be more like 30 minutes?
...
newBlockManager(s *ChainService) (bm *blockManager, err error) {
	...
	if genesisHeader, err = s.RegFilterHeaders.FetchHeaderByHeight(0); slog.Check(err) {
		return
	}
	...
	var height uint32
	if header, height, err = s.BlockHeaders.ChainTip(); slog.Check(err) {
		return
	}
	...
	if _, bm.filterHeaderTip, err = s.RegFilterHeaders.ChainTip(); slog.Check(err) {
		return
	}
	// We must also ensure the the filter header tip hash is set to the
	// block hash at the filter tip height.
	var fh *wire.BlockHeader
	if fh, err = s.BlockHeaders.FetchHeaderByHeight(bm.filterHeaderTip); slog.Check(err) {
		return
	}
...
// Stop gracefully shuts down the block manager by stopping all
// asynchronous handlers and waiting for them to finish.
func (b *blockManager) Stop() (err error) {
	...
	slog.Info("Block manager shutting down")
	close(b.quit)
	b.wg.Wait()
	close(done)
	return
...
func (b *blockManager) handleNewPeerMsg(peers *list.List, sp *ServerPeer) {
	...
	slog.Debugf(
		"new valid peer %s (%s) %s", sp, sp.UserAgent(),
	)
	...
	var err error
	var height uint32
	if _, height, err = b.server.BlockHeaders.ChainTip(); slog.Check(err) {
		slog.Error("couldn't retrieve block header chain tip: %s", err)
		return
	}
	if height < uint32(sp.StartingHeight()) && b.BlockHeadersSynced() {
		var locator blockchain.BlockLocator
		if locator, err = b.server.BlockHeaders.LatestBlockLocator(); slog.Check(err) {
			slog.Error("couldn't retrieve latest block locator: %s", err)
			return
		}
		stopHash := &zeroHash
		if err = sp.PushGetHeadersMsg(locator, stopHash); slog.Check(err) {
		}
	}
...
func (b *blockManager) handleDonePeerMsg(peers *list.List, sp *ServerPeer) {
	...
	if b.SyncPeer() != nil && b.SyncPeer() == sp {
		...
		var height uint32
		var err error
		if header, height, err = b.server.BlockHeaders.ChainTip(); slog.Check(err) {
			return
		}
...
// cfHandler is the cfheader download handler for the block manager.
// It must be run as a goroutine.
// It requests and processes cfheaders messages in a separate goroutine from
// the peer handlers.
...
waitForHeaders:
	...
	// the sync, as the check pointed sync is faster, than fetching each header from each peer during the normal
	// "at tip" syncing.
	slog.Infof(
		"waiting for more block headers, then will start cfheaders sync from height %v...",
		b.filterHeaderTip,
	)
	// NOTE: We can grab the filterHeaderTip here without a lock, as this is the only goroutine that can modify this value.
	...
	var err error
	if lastHeader, lastHeight, err = b.server.BlockHeaders.ChainTip(); slog.Check(err) {
		return
	}
	lastHash := lastHeader.BlockHash()
	slog.Infof(
		"starting cfheaders sync from (block_height=%v, block_hash=%v) to (block_height=%v, block_hash=%v)",
		b.filterHeaderTip, b.filterHeaderTipHash, lastHeight, lastHeader.BlockHash(),
	)
	fType := wire.GCSFilterRegular
	store := b.server.RegFilterHeaders
	slog.Info("starting cfheaders sync for filter_type=", fType)
	...
	for len(goodCheckpoints) == 0 && lastHeight >= wire.CFCheckptInterval {
		...
		if minCheckpointHeight(allCFCheckpoints) < lastHeight {
			...
			slog.Debugf("getting filter checkpoints up to height=%v, hash=%v", bestHeight, bestHash)
			allCFCheckpoints = b.getCheckpts(&bestHash, fType)
			if len(allCFCheckpoints) == 0 {
				slog.Warnf("unable to fetch set of candidate checkpoints, trying again...")
				select {
		...
		if slog.Check(err) {
			slog.Debugf("got error attempting to determine correct cfheader checkpoints: %v, trying again", err)
		}
	...
	b.newHeadersMtx.RUnlock()
	slog.Infof("fully caught up with cfheaders at height %v, waiting at tip for new blocks", lastHeight)
	...
	for {
		...
		b.newHeadersSignal.L.Unlock()
		// At this point, we know that there are a set of new filter headers to fetch, so we'll grab them now.
		if err = b.getUncheckpointedCFHeaders(store, fType); slog.Check(err) {
			slog.Debugf("couldn't get uncheckpointed headers for %v: %v", fType, err)
			select {
			case <-time.After(QueryTimeout):
			case <-b.quit:
				return
...
func (b *blockManager) getUncheckpointedCFHeaders(
	...
	var filtHeight uint32
	if _, filtHeight, err = store.ChainTip(); slog.Check(err) {
		return fmt.Errorf("error getting filter chain tip: %v", err)
	}
	var blockHeader *wire.BlockHeader
	var blockHeight uint32
	if blockHeader, blockHeight, err = b.server.BlockHeaders.ChainTip(); slog.Check(err) {
		err = fmt.Errorf("error getting block chain tip: %v", err)
		slog.Check(err)
		return
	}
	...
	if blockHeight < filtHeight {
		err = fmt.Errorf("reorg in progress, waiting to get uncheckpointed cfheaders (block "+
			"height %d, filter height %d", blockHeight, filtHeight)
		slog.Debug(err)
		return
	}
	// If the heights match then we're fully synced so we don't need to do
	// anything from there.
	if blockHeight == filtHeight {
		return
	}
	slog.Infof("attempting to fetch set of un-checkpointed filters at height=%v, hash=%v",
	...
	if len(headers) == 0 {
		err = fmt.Errorf("couldn't get cfheaders from peers")
		slog.Debug(err)
		return
	}
	...
	for i := 0; i < wire.MaxCFHeadersPerMsg; i++ {
		if checkForCFHeaderMismatch(headers, i) {
			...
			slog.Warnf("detected cfheader mismatch at height=%v!!!", targetHeight)
			// Get the block header for this height, along with the
			// block as well.
			var header *wire.BlockHeader
			if header, err = b.server.BlockHeaders.FetchHeaderByHeight(targetHeight); slog.Check(err) {
				return
			}
			var block *util.Block
			if block, err = b.server.GetBlock(header.BlockHash()); slog.Check(err) {
				return
			}
			slog.Warnf("attempting to reconcile cfheader mismatch amongst %v peers", len(headers))
			...
			filtersFromPeers := b.fetchFilterFromAllPeers(targetHeight, header.BlockHash(), fType)
			var badPeers []string
			if badPeers, err = resolveCFHeaderMismatch(block.MsgBlock(), fType, filtersFromPeers); slog.Check(err) {
				return
			}
			slog.Warnf("banning %v peers due to invalid filter headers", len(badPeers))
			for _, peer := range badPeers {
				slog.Infof("banning peer=%v for invalid filter headers", peer)
	...
	// set of peers.
	var pristineHeaders *wire.MsgCFHeaders
	var ok bool
	if pristineHeaders, ok = headers[key]; !ok {
		err = fmt.Errorf("All peers served bogus headers! Retrying with new set")
		slog.Debug(err)
		return
	}
	if _, err = b.writeCFHeadersMsg(pristineHeaders, store); slog.Check(err) {
	}
	return err
}
...
func (b *blockManager) getCheckpointedCFHeaders(checkpoints []*chainhash.Hash, store *headerfs.FilterHeaderStore,
	...
	var err error
	if curHeader, curHeight, err = store.ChainTip(); slog.Check(err) {
		panic(fmt.Sprintf("failed getting chaintip from filter store: %v", err))
	}
	initialFilterHeader := curHeader
	slog.Infof("fetching set of checkpointed cfheaders filters from height=%v, hash=%v", curHeight, curHeader)
	...
	slog.Info("starting to query for cfheaders from checkpoint_interval =", startingInterval)
	...
	for currentInterval < uint32(len(checkpoints)) {
		...
		slog.Tracef("checkpointed cfheaders request start_range=%v, end_range=%v", startHeightRange, endHeightRange)
		// In order to fetch the range, we'll need the block header for
		// the end of the height range.
		var stopHeader *wire.BlockHeader
		if stopHeader, err = b.server.BlockHeaders.FetchHeaderByHeight(endHeightRange); slog.Check(err) {
			panic(fmt.Sprintf("failed getting block header at height %v: %v", endHeightRange, err))
		}
		stopHash := stopHeader.BlockHash()
		// Once we have the stop hash, we can construct the query
		// message itself.
		queryMsg := wire.NewMsgGetCFHeaders(fType, uint32(startHeightRange), &stopHash)
	...
	slog.Infof("attempting to query for %v cfheader batches", len(queryMsgs))
	...
	b.server.queryBatch(queryMsgs,
		...
		func(sp *ServerPeer, query wire.Message,
			...
			// Use either the genesis header or the previous checkpoint index
			// as the previous checkpoint when verifying that the filter
			// headers in the response match up.
			...
			if !verifyCheckpoint(prevCheckpoint, nextCheckpoint, r) {
				slog.Warnf("checkpoints at index %v don't match response!!!\nTODO: WHERE IS THIS INDEX VALUE FOR THE DEBUG?")
				return false
			}
			// At this point, the response matches the query,
			// and the relevant checkpoint we got earlier,
			// so we should always return true so that the peer looking for
			// the answer to this query can move on to the next query.
			// We still have to check that these headers are next before we
			// write them; otherwise, we cache them if they're too far ahead,
			// or discard them if we don't need them.
			// Find the first and last height for the blocks represented by
			// this message.
			startHeight := checkPointIndex*wire.CFCheckptInterval + 1
			lastHeight := (checkPointIndex + 1) * wire.CFCheckptInterval
			slog.Debugf("got cfheaders from height=%v to height=%v, prev_hash=%v", startHeight, lastHeight,
				r.PrevFilterHeader)
			...
			if startHeight > curHeight+1 {
				slog.Debugf("got response for headers at height=%v, only at height=%v, stashing",
			...
			if lastHeight <= curHeight {
				slog.Debugf("received out of order reply end_height=%v, already written", lastHeight)
				return true
			}
			...
			if bytes.Equal(curHeader[:], initialFilterHeader[:]) {
				// So we'll set the prev header to our best known header, and seek within the header
				// range a bit so we don't write any duplicate headers.
			...
			for {
				checkPointIndex++
				// We'll also update the current height of the last written
				// set of cfheaders.
				curHeight = checkPointIndex * wire.CFCheckptInterval
				// If we don't yet have the next response,
				// then we'll break out so we can wait for the peers to
				// respond with this message.
				if r, ok = queryResponses[checkPointIndex]; !ok {
					break
				}
				// We have another response to write,
				// so delete it from the cache and write it.
				delete(queryResponses, checkPointIndex)
				slog.Debugf("writing cfheaders at height=%v to next checkpoint", curHeight)
				// As we write the set of headers to disk,
				// we also obtain the hash of the last filter header we've
				// written to disk so we can properly set the
				// PrevFilterHeader field of the next message.
				if curHeader, err = b.writeCFHeadersMsg(r, store); slog.Check(err) {
					panic(fmt.Sprintf("couldn't write cfheaders msg: %v", err))
				}
			}
			return true
		},
...
// writeCFHeadersMsg writes a cfheaders message to the specified store.
// It assumes that everything is being written in order.
// The hints are required to store the correct block heights for the filters.
// We also return final constructed cfheader in this range as this lets
// callers populate the prev filter header field in the next message range
// before writing to disk.
func (b *blockManager) writeCFHeadersMsg(msg *wire.MsgCFHeaders, store *headerfs.FilterHeaderStore) (
	lastHeader *chainhash.Hash, err error) {
	...
	if tip, tipHeight, err = store.ChainTip(); slog.Check(err) {
		return
	}
	if *tip != msg.PrevFilterHeader {
		err = fmt.Errorf("attempt to write cfheaders out of order! Tip=%v (height=%v), prev_hash=%v", *tip, tipHeight, msg.PrevFilterHeader)
		slog.Debug(err)
		return
	}
	...
	for _, hash := range msg.FilterHashes {
		*lastHeader = chainhash.DoubleHashH(append(hash[:], lastHeader[:]...))
	...
	if matchingBlockHeaders, startHeight, err =
		blockHeaders.FetchHeaderAncestors(uint32(numHeaders-1), &msg.StopHash); slog.Check(err) {
		return
	}
	...
	slog.Debugf("writing filter headers up to height=%v, hash=%v, new_tip=%v", lastHeight, lastHash, lastHeader)
	// Write the header batch.
	if err = store.WriteHeaders(headerBatch...); slog.Check(err) {
		return
	}
	...
	for i, header := range matchingBlockHeaders {
		...
		b.server.sendSubscribedMsg(&blockMessage{msgType: msgType, header: &header})
	}
...
// minCheckpointHeight returns the height of the last filter checkpoint
// for the shortest checkpoint list among the given lists.
...
// verifyHeaderCheckpoint verifies that a CFHeaders message matches the
// passed checkpoints. It assumes everything else has been checked,
// including filter type and stop hash matches,
...
// resolveConflict finds the correct checkpoint information, rewinds the header store if it's incorrect,
// and bans any peers giving us incorrect header information.
func (b *blockManager) resolveConflict(checkpoints map[string][]*chainhash.Hash, store *headerfs.FilterHeaderStore,
	fType wire.FilterType) (checkpts []*chainhash.Hash, err error) {
	var heightDiff int
	if heightDiff, err = checkCFCheckptSanity(checkpoints, store); slog.Check(err) {
		return
	}
	...
	slog.Warnf("detected mismatch at index=%v for checkpoints!!!", heightDiff)
	...
	if len(checkpoints) == 0 {
		err = fmt.Errorf("no peer is serving good cfheaders")
		slog.Debug(err)
		return
	}
	...
	for _, msg := range headers {
		if hash == zeroHash {
			...
			err = fmt.Errorf("mismatch between filter headers expected to be the same")
			slog.Debug(err)
			return
		}
	}
	...
	for i := 0; i < wire.MaxCFHeadersPerMsg; i++ {
		if checkForCFHeaderMismatch(headers, i) {
			...
			slog.Infof("attempting to reconcile cfheader mismatch amongst %v peers", len(headers))
			// We'll also fetch each of the filters from the peers that reported check points, as we may need this in
			// order to determine which peers are faulty.
			filtersFromPeers := b.fetchFilterFromAllPeers(targetHeight, header.BlockHash(), fType)
			var badPeers []string
	...
	// Check sanity again. If we're sane, return a matching checkpoint list. If not, return an error and download
	...
	err = fmt.Errorf("got mismatched checkpoints")
	slog.Debug(err)
	return
}

// checkForCFHeaderMismatch checks all peers' responses at a specific
// position and detects a mismatch. It returns true if a mismatch has occurred.
func checkForCFHeaderMismatch(headers map[string]*wire.MsgCFHeaders, idx int) bool {
...
// resolveCFHeaderMismatch will attempt to cross-reference each filter
// received by each peer based on what we can reconstruct and verify from the
...
func resolveCFHeaderMismatch(block *wire.MsgBlock, fType wire.FilterType,
	filtersFromPeers map[string]*gcs.Filter) (invalidPeers []string, err error) {
	badPeers := make(map[string]struct{})
	blockHash := block.BlockHash()
	filterKey := builder.DeriveKey(&blockHash)
	slog.Infof("attempting to pinpoint mismatch in cfheaders for block=%v", block.Header.BlockHash())
	...
	case wire.GCSFilterRegular:
		...
		peerVerification:
			...
			// TODO(roasbeef): eventually just do a comparison against decompressed filters
			for _, tx := range block.Transactions {
				for _, txOut := range tx.TxOut {
					...
					// circular dependency.
					case txOut.PkScript[0] == txscript.OP_RETURN && txscript.IsPushOnlyScript(txOut.PkScript[1:]):
						continue
					}
					var match bool
					if match, err = filter.Match(filterKey, txOut.PkScript); slog.Check(err) {
	...
	default:
		err = fmt.Errorf("unknown filter: %v", fType)
		slog.Debug(err)
		return
	}
...
// getCFHeadersForAllPeers runs a query for cfheaders at a specific
// height and returns a map of responses from all peers.
func (b *blockManager) getCFHeadersForAllPeers(height uint32, fType wire.FilterType) (headers map[string]*wire.MsgCFHeaders) {
	...
	// larger.
	var (
		stopHeader *wire.BlockHeader
		stopHeight uint32
		err        error
	)
	if stopHeader, stopHeight, err = b.server.BlockHeaders.ChainTip(); slog.Check(err) {
	}
	if stopHeight-height >= wire.MaxCFHeadersPerMsg {
		if stopHeader, err = b.server.BlockHeaders.FetchHeaderByHeight(height + wire.MaxCFHeadersPerMsg - 1); slog.Check(err) {
			return
		}
	}
	...
	b.server.queryAllPeers(
		msg,
		func(sp *ServerPeer, resp wire.Message, quit chan<- struct{}, peerQuit chan<- struct{}) {
			switch m := resp.(type) {
...
// responded with.
func (b *blockManager) fetchFilterFromAllPeers(height uint32, blockHash chainhash.Hash, filterType wire.FilterType,
) (filterResponses map[string]*gcs.Filter) {
	...
	b.server.queryAllPeers(filterReqMsg,
		func(sp *ServerPeer, resp wire.Message, quit chan<- struct{}, peerQuit chan<- struct{}) {
			...
			case *wire.MsgCFilter:
				...
				if gcsFilter, err = gcs.FromNBytes(builder.DefaultP, builder.DefaultM, response.Data); slog.Check(err) {
...
func (b *blockManager) getCheckpts(lastHash *chainhash.Hash, fType wire.FilterType) (checkpoints map[string][]*chainhash.Hash) {
	checkpoints = make(map[string][]*chainhash.Hash)
	getCheckptMsg := wire.NewMsgGetCFCheckpt(fType, lastHash)
	b.server.queryAllPeers(getCheckptMsg,
		func(sp *ServerPeer, resp wire.Message, quit chan<- struct{}, peerQuit chan<- struct{}) {
			switch m := resp.(type) {
...
// checkCFCheckptSanity checks whether all peers which have responded
// agree.
...
func checkCFCheckptSanity(cp map[string][]*chainhash.Hash, headerStore *headerfs.FilterHeaderStore) (v int, err error) {
	// Get the known best header to compare against checkpoints.
	var storeTip uint32
	if _, storeTip, err = headerStore.ChainTip(); slog.Check(err) {
		return
	}
	...
	for i := 0; i < maxLen; i++ {
		...
		for _, checkpoints := range cp {
			...
			if checkpoint != *checkpoints[i] {
				slog.Warnf("mismatch at %v, expected %v got %v", i, checkpoint, checkpoints[i])
				return i, nil
			}
		}
		...
		if ckptHeight <= storeTip {
			...
			if header, err = headerStore.FetchHeaderByHeight(ckptHeight); slog.Check(err) {
				return
			}
			if *header != checkpoint {
				slog.Warnf("mismatch at height %v, expected %v got %v", ckptHeight, header, checkpoint)
				return i, nil
			}
		}
	}
...
// blockHandler is the main handler for the block manager.  It must be run as a goroutine. It processes block and inv
// messages in a separate goroutine from the peer handlers so the block (MsgBlock) messages are handled by a single
// thread without needing to lock memory data structures.
...
func (b *blockManager) findNextHeaderCheckpoint(height int32) *config.Checkpoint {
...
// network. This is used for resetting state when a malicious peer sends us headers that don't lead up to a known checkpoint.
func (b *blockManager) findPreviousHeaderCheckpoint(height int32) (prevCheckpoint *config.Checkpoint) {
...
func (b *blockManager) startSync(peers *list.List) {
	...
	var err error
	if _, bestHeight, err = b.server.BlockHeaders.ChainTip(); slog.Check(err) {
		slog.Error("failed to get hash and height for the latest block:", err)
		return
	}
	...
	for e := peers.Front(); e != nil; e = enext {
		...
		// equal, it will likely have one soon so it is a reasonable choice.  It also allows the case where both are at
		// 0 such as during regression test.
	...
	if bestPeer != nil {
		var locator blockchain.BlockLocator
		if locator, err = b.server.BlockHeaders.LatestBlockLocator(); slog.Check(err) {
			slog.Error("failed to get block locator for the latest block:", err)
			return
		}
		slog.Infof("syncing to block height %d from peer %s", bestPeer.LastBlock(), bestPeer.Addr())
		...
		if b.nextCheckpoint != nil && int32(bestHeight) < b.nextCheckpoint.Height {
			slog.Infof("downloading headers for blocks %d to %d from peer %s",
				bestHeight+1, b.nextCheckpoint.Height, bestPeer.Addr())
			stopHash = b.nextCheckpoint.Hash
		} else {
			slog.Infof("fetching set of headers from tip (height=%v) from peer %s", bestHeight, bestPeer.Addr())
		}
		// With our stop hash selected, we'll kick off the sync from this peer with an initial GetHeaders message.
		if err = b.SyncPeer().PushGetHeadersMsg(locator, stopHash); slog.Check(err) {
			return
		}
	} else {
		slog.Warn("no sync peer candidates available")
	}
}

// IsFullySynced returns whether or not the block manager believed it is fully synced to the connected peers,
// meaning both block headers and filter headers are current.
func (b *blockManager) IsFullySynced() (is bool) {
	var blockHeaderHeight uint32
	var err error
	if _, blockHeaderHeight, err = b.server.BlockHeaders.ChainTip(); slog.Check(err) {
		return
	}
	var filterHeaderHeight uint32
	if _, filterHeaderHeight, err = b.server.RegFilterHeaders.ChainTip(); slog.Check(err) {
		return
	}
...
// peers.
func (b *blockManager) BlockHeadersSynced() (is bool) {
...
// current set of filter headers. This allows the caller to execute an action that depends on the current filter
// header state, thereby ensuring that the state would shift from underneath them. Each execution of the closure will
// have the current filter header tip passed in to ensue that the caller gets a consistent view.
func (b *blockManager) SynchronizeFilterHeaders(f func(uint32) error) (err error) {
...
func (b *blockManager) handleInvMsg(imsg *invMsg) {
	...
	if lastBlock != -1 && b.BlockHeadersSynced() {
		var height uint32
		if height, err = b.server.BlockHeaders.HeightFromHash(&invVects[lastBlock].Hash); slog.Check(err) {
	...
	if lastBlock != -1 && (imsg.peer == b.SyncPeer() || b.BlockHeadersSynced()) {
		...
		if lastHash != invVects[lastBlock].Hash && lastEl != nil &&
			...
			var knownLocator blockchain.BlockLocator
			if knownLocator, err = b.server.BlockHeaders.LatestBlockLocator(); !slog.Check(err) {
				locator = append(locator, knownLocator...)
			}
			// Get headers based on locator.
			if err = imsg.peer.PushGetHeadersMsg(locator, &invVects[lastBlock].Hash); slog.Check(err) {
				slog.Warnf("failed to send getheaders message to peer %s: %s", imsg.peer.Addr(), err)
				return
			}
			b.lastRequested = invVects[lastBlock].Hash
		}
...
func (b *blockManager) handleHeadersMsg(hdrsMsg *headersMsg) {
	var err error
	msg := hdrsMsg.headers
	...
	maxTimestamp := b.server.timeSource.AdjustedTime().Add(maxTimeOffset)
	...
	for i, blockHeader := range msg.Headers {
		...
		if prevNodeEl == nil {
			slog.Warn("header list does not contain a previous element as expected -- disconnecting peer")
			hdrsMsg.peer.Disconnect()
			return
		}
		...
		if prevHash.IsEqual(&blockHeader.PrevBlock) {
			if err = b.checkHeaderSanity(blockHeader, maxTimestamp, false, prevNode.Height+1); slog.Check(err) {
				slog.Warnf("header doesn't pass sanity check: %s -- disconnecting peer", err)
				hdrsMsg.peer.Disconnect()
				return
			}
			...
			hdrsMsg.peer.UpdateLastBlockHeight(node.Height)
			b.blkHeaderProgressLogger.LogBlockHeight(blockHeader.Timestamp, node.Height)
			...
			//
			// We will need to do some additional checks to process possible reorganizations or incorrect chain on
			...
			// headers.
			if hdrsMsg.peer != b.SyncPeer() && !b.BlockHeadersSynced() {
				return
			}
			// Check if this is the last block we know of. This is a shortcut for sendheaders so that each redundant
			// header doesn't cause a disk read.
			if blockHash == prevHash {
				continue
			}
			// Check if this block is known. If so, we continue to the next one.
			if _, _, err := b.server.BlockHeaders.FetchHeader(&blockHash); !slog.Check(err) {
				continue
			}
			// Check if the previous block is known. If it is, this is probably a reorg based on the estimated latest
			// block that matches between us and the peer as derived from the block locator we sent to request
			// these headers. Otherwise, the headers don't connect to anything we know and we should disconnect the
			// peer.
			var (
				backHead   *wire.BlockHeader
				backHeight uint32
			)
			if backHead, backHeight, err = b.server.BlockHeaders.FetchHeader(&blockHeader.PrevBlock); slog.Check(err) {
				slog.Warnf("received block header that does not properly connect to the chain from peer %s (%s)"+
					" -- disconnecting", hdrsMsg.peer.Addr(), err)
				hdrsMsg.peer.Disconnect()
				return
			}
			...
			if backHeight < uint32(prevCheckpoint.Height) {
				slog.Errorf("attempt at a reorg earlier than a checkpoint past which we've already synchronized "+
					"-- disconnecting peer %s", hdrsMsg.peer)
				hdrsMsg.peer.Disconnect()
				return
			}
			// Check the sanity of the new branch. If any of the blocks don't pass sanity checks, disconnect the
			// peer.  We also keep track of the work represented by these headers so we can compare it to the work in
			// the known good chain.
			b.reorgList.ResetHeaderState(headerlist.Node{Header: *backHead, Height: int32(backHeight)})
			totalWork := big.NewInt(0)
			for j, reorgHeader := range msg.Headers[i:] {
				if err = b.checkHeaderSanity(reorgHeader, maxTimestamp, true, prevNode.Height+1); slog.Check(err) {
					slog.Warnf("header doesn't pass sanity check: %s -- disconnecting peer", err)
					hdrsMsg.peer.Disconnect()
					return
				}
				totalWork.Add(totalWork, blockchain.CalcWork(reorgHeader.Bits, prevNode.Height+1, reorgHeader.Version))
				b.reorgList.PushBack(headerlist.Node{Header: *reorgHeader, Height: int32(backHeight+1) + int32(j)})
			}
			slog.Trace("sane reorg attempted. Total work from reorg chain:", totalWork)
			...
			for j := uint32(prevNode.Height); j > backHeight; j-- {
				if knownEl != nil {
					...
					if knownHead, _, err = b.server.BlockHeaders.FetchHeader(&knownHead.PrevBlock); slog.Check(err) {
						slog.Fatalf("can't get block header for hash %s: %v", knownHead.PrevBlock, err)
						// Should we panic here?
					}
				}
			}
			slog.Trace("total work from known chain:", knownWork)
			...
			case 1:
				slog.Warnf("reorg attempt that has less work than known chain from peer %s -- disconnecting",
					hdrsMsg.peer)
				hdrsMsg.peer.Disconnect()
				fallthrough
			case 0:
			...
			// At this point, we have a valid reorg, so we roll back the existing chain and add the new block
			// header.  We also change the sync peer. Then we can continue with the rest of the headers in the message
			// as if nothing has happened.
			b.syncPeerMutex.Lock()
			b.syncPeer = hdrsMsg.peer
			b.syncPeerMutex.Unlock()
			if _, err = b.server.rollBackToHeight(backHeight); slog.Check(err) {
				panic(fmt.Sprintf("Rollback failed: %s", err))
				// Should we panic here?
			}
			hdrs := headerfs.BlockHeader{BlockHeader: blockHeader, Height: backHeight + 1}
			if err = b.server.BlockHeaders.WriteHeaders(hdrs); slog.Check(err) {
				slog.Fatal("Couldn't write block to database:", err)
				// Should we panic here?
			}
			b.headerList.ResetHeaderState(headerlist.Node{Header: *backHead, Height: int32(backHeight)})
			b.headerList.PushBack(headerlist.Node{Header: *blockHeader, Height: int32(backHeight + 1)})
		}
		...
		if b.nextCheckpoint != nil && node.Height == b.nextCheckpoint.Height {
			...
			if nodeHash.IsEqual(b.nextCheckpoint.Hash) {
				...
				slog.Infof("verified downloaded block header against checkpoint at height %d/hash %s",
					node.Height, nodeHash)
			} else {
				slog.Warnf("block header at height %d/hash %s from peer %s does NOT match expected checkpoint "+
					"hash of %s -- disconnecting", node.Height, nodeHash, hdrsMsg.peer.Addr(), b.nextCheckpoint.Hash)
				prevCheckpoint := b.findPreviousHeaderCheckpoint(node.Height)
				slog.Infof("rolling back to previous validated checkpoint at height %d/hash %s",
					prevCheckpoint.Height, prevCheckpoint.Hash,
				)
				if _, err = b.server.rollBackToHeight(uint32(prevCheckpoint.Height)); slog.Check(err) {
					slog.Fatal("rollback failed:", err)
					// Should we panic here?
				}
				hdrsMsg.peer.Disconnect()
				return
	...
	slog.Tracef("writing header batch of %v block headers", len(headerWriteBatch))
	...
	if b.server.chainParams.Net == config.SimNetParams.Net || !b.
		BlockHeadersSynced() {
		...
		if err = hdrsMsg.peer.PushGetHeadersMsg(locator, &nextHash); slog.Check(err) {
			slog.Errorf("failed to send getheaders message to peer %s: %s", hdrsMsg.peer.Addr(), err)
			return
		}
	}
...
func (b *blockManager) checkHeaderSanity(blockHeader *wire.BlockHeader, maxTimestamp time.Time, reorgAttempt bool,
	height int32) (err error) {
	var diff uint32
	if diff, err = b.calcNextRequiredDifficulty(blockHeader.Timestamp, reorgAttempt); slog.Check(err) {
		return
	}
...
func (b *blockManager) calcNextRequiredDifficulty(newBlockTime time.Time, reorgAttempt bool) (dif uint32, err error) {
	hList := b.headerList
	if reorgAttempt {
		hList = b.reorgList
	}
	...
	// Get the block node at the previous retarget
	// (targetTimespan days worth of blocks).
	...
	// Log new target difficulty and return it.
	// The new target logging is intentionally converting the bits back to a
	// number instead of using newTarget since conversion to the compact
	// representation loses precision.
	newTargetBits := blockchain.BigToCompact(newTarget)
	slog.Debug(func() string {
		return fmt.Sprintf(
			"difficulty retarget at block height %d old"+
				" target %08x ("+
				"%064x) new target %08x (%064x) actual timespan %v, "+
				"adjusted timespan %v, target timespan %v",
			lastNode.Height+1,
...
func (b *blockManager) findPrevTestNetDifficulty(hList headerlist.Chain) (lastBits uint32, err error) {
	...
	for iterNode != nil && iterHeight%b.blocksPerRetarget != 0 && iterNode.Bits == b.server.chainParams.PowLimitBits {
		...
		if el != nil {
			...
			var node *wire.BlockHeader
			if node, err = b.server.BlockHeaders.FetchHeaderByHeight(uint32(iterHeight)); slog.Check(err) {
				return
			}
			iterNode = node
		}
