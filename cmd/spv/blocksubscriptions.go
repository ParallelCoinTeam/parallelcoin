...
type (
	...
	// notifications.
	//
	// TODO(aakselrod): Move this to its own package so that the subscriber can't access internals, in particular the
	//  notifyBlock and intQuit members.
...
const (
	// connectBasic is a type of notification sent whenever we connect a new set of basic filter headers to the end of
	// the main chain.
	connectBasic messageType = iota
	// disconnect is a type of filter notification that is sent whenever a block is disconnected from the end of the
	// main chain.
	disconnect
)

// sendSubscribedMsg sends all block subscribers a message if they request this type.
//
// TODO(aakselrod): Refactor so we're able to handle more message types in new package.
...
//
// The best known height to the caller should be passed in, such that we can send a backlog of notifications to the
...
// unsubscribeBlockMsgs handles removing block subscriptions from the ChainService.
//
// TODO(aakselrod): move this to its own package and refactor so that we're not depending on the caller to not modify
//  the argument between subscribe and unsubscribe.
...
// subscriptionHandler must be run as a goroutine and queues notification messages from the chain service to the
// subscriber.
func (s *blockSubscription) subscriptionHandler() {
	...
	// Try to send on the specified channel. If a new message arrives while we try to send, queue it and continue with
	// the loop. If a quit signal is sent, let the loop know.
	...
	for {
		if next != nil {
			// If selectChan returns false, we were signalled on s.quit or s.intQuit.
			switch next.msgType {
			case connectBasic:
			...
			// Next notification is nil, so see if we can get a notification from the queue. If not, we wait for a
...
// sendMsgToSubscriber is a helper function that sends the target message to the subscription client over the proper
// channel based on the type of the new block notification.
func sendMsgToSubscriber(sub *blockSubscription, bm *blockMessage) {
	...
	// If the subscription channel was found for this subscription based on the new update, then we'll wait to either
	// send this notification, or quit from either signal.
	if subChan != nil {
		select {
