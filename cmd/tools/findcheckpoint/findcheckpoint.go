...
func loadBlockDB() (database.DB, error) {
	...
	log.L.Infof("Loading block database from '%s'\n", dbPath)
...
func showCandidate(
	...
	if cfg.UseGoOutput {
		log.L.Infof("Candidate %d -- {%d, newShaHashFromStr(\"%v\")},\n",
			candidateNum, checkpoint.Height, checkpoint.Hash)
		return
	}
	log.L.Infof("Candidate %d -- Height: %d, Hash: %v\n", candidateNum,
...
func main() {
	...
	db, err := loadBlockDB()
	if err != nil {
		log.L.Error("failed to load database:", err)
		return
	}
	defer db.Close()
	...
	if err != nil {
		log.L.Error("failed to initialize chain: %v\n", err)
		return
	}
	// Get the latest block hash and height from the database and report status.
	best := chain.BestSnapshot()
	log.L.Infof("Block database loaded with block height %d\n", best.Height)
	// Find checkpoint candidates.
	candidates, err := findCandidates(chain, &best.Hash)
	if err != nil {
		log.L.Error("Unable to identify candidates:", err)
		return
	}
	// No candidates.
	if len(candidates) == 0 {
		log.L.Error("No candidates found.")
		return
	}
	// Show the candidates.
