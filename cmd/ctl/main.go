...
func Main(args []string, cx *conte.Xt) {
	...
	var e error
	if usageFlags, e = btcjson.MethodUsageFlags(method); dbg.Chk(e) {
	...
	var result []byte
	if result, e = ctl.Call(cx, *cx.Config.Wallet, method, params...); dbg.Chk(e) {
		return
	}
	...
	case strings.HasPrefix(strResult, "{") || strings.HasPrefix(strResult, "["):
		...
		if e = js.Indent(&dst, result, "", "  "); dbg.Chk(e) {
	...
	case strings.HasPrefix(strResult, `"`):
		var str string
		if e = js.Unmarshal(result, &str); dbg.Chk(e) {
