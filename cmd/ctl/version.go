...
func normalizeVerString(str string) string {
	...
	for _, r := range str {
		if strings.ContainsRune(semanticAlphabet, r) {
			// Ignoring the error here since it can only fail if the the
			// system is out of memory and there are much bigger issues at
			// that point.
...
func Version() (version string) {
	...
	// The plus called for by the semantic versioning spec is automatically
	// appended and should not be contained in the build metadata string.
	// The build metadata string is not appended if it contains invalid
	// characters.
