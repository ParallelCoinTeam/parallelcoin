...
func initDictionary(cfg *pod.Config) {
	...
	log.L.Debug("lang set to", *cfg.Language)
}
...
func initDataDir(cfg *pod.Config) {
	...
	log.L.Debug("datadir set to", *cfg.DataDir)
}
...
func initWalletFile(cx *conte.Xt) {
	...
	log.L.Debug("walletfile set to", *cx.Config.WalletFile)
}
...
func initConfigFile(cfg *pod.Config) {
	...
	log.L.Debug("using config file:", *cfg.ConfigFile)
}
...
func initLogDir(cfg *pod.Config) {
	if *cfg.LogDir != "" {
		...
		interrupt.AddHandler(func() {
			log.L.Debug("initLogDir interrupt")
	...
	*cfg.Controller = net.JoinHostPort("0.0.0.0", fmt.Sprint(fP))
	...
	if len(*cfg.WalletRPCListeners) < 1 && !*cfg.DisableRPC {
		*cfg.WalletRPCListeners = append(*cfg.WalletRPCListeners,
		...
		log.L.Debug("setting save flag because wallet rpc listeners is empty and rpc is not disabled")
		cx.StateCfg.Save = true
	}
	if len(*cfg.RPCListeners) < 1 {
		*cfg.RPCListeners = append(*cfg.RPCListeners,
			":"+cx.ActiveNet.RPCClientPort)
		log.L.Debug("setting save flag because rpc listeners is empty and rpc is not disabled")
		cx.StateCfg.Save = true
	}
	...
	if *cfg.RPCConnect == "" {
		*cfg.RPCConnect = "127.0.0.1:" + cx.ActiveNet.RPCClientPort
		log.L.Debug("setting save flag because rpcconnect was not configured")
		cx.StateCfg.Save = true
	}
...
func initTLSStuffs(cfg *pod.Config, st *state.Config) {
	...
	if *cfg.RPCCert == "" {
		*cfg.RPCCert =
		...
		log.L.Debug("setting save flag because rpc cert path was not set")
		st.Save = true
		isNew = true
	}
	if *cfg.RPCKey == "" {
		*cfg.RPCKey =
		...
		log.L.Debug("setting save flag because rpc key path was not set")
		st.Save = true
		isNew = true
	}
	if *cfg.CAFile == "" {
		*cfg.CAFile =
		...
		log.L.Debug("setting save flag because CA cert path was not set")
		st.Save = true
		isNew = true
	}
	if isNew {
		...
		log.L.Warn("rpc tls ", *cfg.RPCCert, " ", *cfg.RPCKey)
...
func initLogLevel(cfg *pod.Config) {
	...
	case "trace", "debug", "info", "warn", "error", "fatal", "off":
		log.L.Trace("log level", loglevel)
	default:
		log.L.Info("unrecognised loglevel", loglevel, "setting default info")
		*cfg.LogLevel = "info"
	}
	color := true
...
func normalizeAddresses(cfg *pod.Config) {
	log.L.Trace("normalising addresses")
...
func setAlgo(cfg *pod.Config) {
	...
	log.L.Trace("mining algorithm ", *cfg.Algo)
}
...
func validateProfilePort(cfg *pod.Config) {
	...
	log.L.Trace("validating profile port number")
	if *cfg.Profile != "" {
...
func validateWhitelists(cfg *pod.Config, st *state.Config) {
	...
	log.L.Trace("validating whitelists")
...
func validatePeerLists(cfg *pod.Config) {
	log.L.Trace("checking addpeer and connectpeer lists")
...
func configListener(cfg *pod.Config, params *netparams.Params) {
	...
	log.L.Trace("checking proxy/connect for disabling listening")
	if (*cfg.Proxy != "" ||
	...
	log.L.Trace("checking if listener was set")
...
func validateUsers(cfg *pod.Config) {
	...
	log.L.Trace("checking admin and limited username is different")
	if *cfg.Username != "" &&
	...
	log.L.Trace("checking limited and admin passwords are not the same")
	if *cfg.Password != "" &&
...
func configRPC(cfg *pod.Config, params *netparams.Params) {
	...
	log.L.Trace("checking rpc server has a login enabled")
	if (*cfg.Username == "" || *cfg.Password == "") &&
		(*cfg.LimitUser == "" || *cfg.LimitPass == "") {
		*cfg.DisableRPC = true
	}
	if *cfg.DisableRPC {
		log.L.Trace("RPC service is disabled")
	}
	log.L.Trace("checking rpc server has listeners set")
	if !*cfg.DisableRPC && len(*cfg.RPCListeners) == 0 {
		log.L.Debug("looking up default listener")
		...
		*cfg.RPCListeners = make([]string, 0, len(addrs))
		log.L.Debug("setting listeners")
		for _, addr := range addrs {
			addr = net.JoinHostPort(addr, params.RPCClientPort)
			*cfg.RPCListeners = append(*cfg.RPCListeners, addr)
		}
	}
	log.L.Trace("checking rpc max concurrent requests")
	...
	log.L.Trace("checking rpc listener addresses")
...
func validatePolicies(cfg *pod.Config, stateConfig *state.Config) {
	...
	log.L.Trace("checking min relay tx fee")
	...
	log.L.Trace("checking max block size")
	...
	log.L.Trace("checking max block weight")
	...
	log.L.Trace("checking max orphan limit")
	...
	log.L.Trace("validating block priority and minimum size/weight")
	...
	*cfg.BlockMinWeight = int(apputil.MinUint32(
	...
	log.L.Trace("checking user agent comments", cfg.UserAgentComments)
	...
	log.L.Trace("checking the checkpoints")
...
func validateOnions(cfg *pod.Config) {
	...
	if *cfg.Onion && *cfg.OnionProxy != "" {
		log.L.Error("onion enabled but no onionproxy has been configured")
		log.L.Fatal("halting to avoid exposing IP address")
		os.Exit(1)
	}
...
func validateMiningStuff(cfg *pod.Config, state *state.Config,
	...
	log.L.Trace("checking mining addresses")
	...
	if (*cfg.Generate) && len(state.ActiveMiningAddrs) == 0 {
		log.L.Error("the generate flag is set, " +
...
func setDiallers(cfg *pod.Config, stateConfig *state.Config) {
	...
	log.L.Trace("setting network dialer and lookup")
	stateConfig.Dial = net.DialTimeout
	stateConfig.Lookup = net.LookupIP
	if *cfg.Proxy != "" {
		log.L.Trace("we are loading a proxy!")
		...
		if *cfg.TorIsolation &&
			*cfg.OnionProxy == "" &&
			(*cfg.ProxyUser != "" ||
				*cfg.ProxyPass != "") {
			torIsolation = true
			log.L.Warn("Tor isolation set -- overriding specified" +
	...
	log.L.Trace("setting up tor proxy if enabled")
	if *cfg.OnionProxy != "" {
		...
		if *cfg.TorIsolation &&
			...
			log.L.Warn("Tor isolation set - overriding specified onionproxy user" +
				" credentials")
		}
	}
	log.L.Trace("setting onion dialer")
	...
	// proxy.
	log.L.Trace("setting proxy lookup")
	if *cfg.Proxy != "" {
		stateConfig.Lookup = func(host string) ([]net.IP, error) {
