...
// NewFilter creates a new bloom filter instance, mainly to be used by SPV clients. The tweak parameter is a random
// value added to the seed value. The false positive rate is the probability of a false positive where 1.0 is "match
// everything" and zero is unachievable.
//
...
func NewFilter(elements, tweak uint32, fprate float64, flags wire.BloomUpdateType) *Filter {
	...
	// Calculate the size of the filter in bytes for the given number of elements and false positive rate. Equivalent to
	// m = -(n*ln(p) / ln(2)^2), where m is in bits. Then clamp it to the maximum filter size and convert to bytes.
	dataLen := uint32(-1 * float64(elements) * math.Log(fprate) / ln2Squared)
	dataLen = minUint32(dataLen, wire.MaxFilterLoadFilterSize*8) / 8
	// Calculate the number of hash functions based on the size of the filter calculated above and the number of
	// elements. Equivalent to k = (m/n) * ln(2) Then clamp it to the maximum allowed hash funcs.
...
// hash returns the bit offset in the bloom filter which corresponds to the passed data for the given indepedent hash
// function number.
func (bf *Filter) hash(hashNum uint32, data []byte) uint32 {
	// bitcoind: 0xfba4c795 chosen as it guarantees a reasonable bit difference between hashNum values. Note that << 3
	// is equivalent to multiplying by 8, but is faster. Thus the returned hash is brought into range of the number of
	// bits the filter has and returned.
...
func (bf *Filter) matches(data []byte) bool {
	...
	// The bloom filter does not contain the data if any of the bit offsets which result from hashing the data using
	// each independent hash are not set. The shifts and masks below are a faster equivalent of:
	//
...
// matchesOutPoint returns true if the bloom filter might contain the passed outpoint and false if it definitely does
// not.
//
// This function MUST be called with the filter lock held.
...
func (bf *Filter) add(data []byte) {
	...
	// Adding data to a bloom filter consists of setting all of the bit offsets which result from hashing the data using
	// each independent hash function. The shifts and masks below are a faster equivalent of:
	//
	//   arrayIndex := idx / 8    (idx >> 3)
	//   bitOffset := idx % 8     (idx & 7)
...
//
// This function MUST be called with the filter
// lock held.
