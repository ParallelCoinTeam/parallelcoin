...
const (
	// defaultScriptAlloc is the default size used for the backing array for a script being built by the ScriptBuilder.
	// The array will dynamically grow as needed, but this figure is intended to provide enough space for vast majority
	// of scripts without needing to grow the backing array multiple times.
...
// AddFullData should not typically be used by ordinary users as it does not include the checks which prevent data
// pushes larger than the maximum allowed txsizes which leads to scripts that can't be executed. This is provided for
// testing purposes such as regression tests where txsizes are intentionally made larger than allowed. Use AddData
// instead.
...
// AddData pushes the passed data to the end of the script. It automatically chooses canonical opcodes depending on the
// length of the data. A zero length buffer will lead to a push of empty data onto the stack (OP_0) and any push of data
// greater than MaxScriptElementSize will not modify the script since that is not allowed by the script engine. Also,
// the script will not be modified if pushing the data would cause the script to exceed the maximum allowed script
// engine size.
func (b *ScriptBuilder) AddData(data []byte) *ScriptBuilder {
	if b.err != nil {
		return b
	...
	// Pushes larger than the max script element size would result in a script that is not canonical.
	dataLen := len(data)
...
// AddInt64 pushes the passed integer to the end of the script. The script will not be modified if pushing the data
...
// Script returns the currently built script. When any errors occurred while building the script, the script will be
// returned up the point of the first error along with the error.
